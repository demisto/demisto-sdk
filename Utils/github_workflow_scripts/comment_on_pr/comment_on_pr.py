...
def obtain_validate_summary_msg():
    ...
    if (artifacts_folder := os.getenv("ARTIFACTS_FOLDER")) and Path(
        ...
        logger.info(
            f"reading from the validate summary results to a txt file at {artifacts_validate_summary_path}."
        )
        with open(artifacts_validate_summary_path, "ר") as f:
            validate_summary_msg = f.read()
    else:
        raise Exception("could not find validate summary file.")
...
def arguments_handler():
    ...
    parser.add_argument(
        "-vs",
        "--validate_summary",
        help="Boolean.Whether to run validate summary",
        required=False,
