...
if [[ $CI_COMMIT_BRANCH = master ]] || [[ -n "${NIGHTLY}" ]] || [[ -n "${BUCKET_UPLOAD}" ]] || [[ -n "${DEMISTO_SDK_NIGHTLY}" ]]; then
    if [[ -n "${PACKS_TO_UPLOAD}" ]]; then
        ...
        for item in $PACKS_TO_UPLOAD_SPACED; do
            python3 -m demisto_sdk validate -i Packs/"$item" --post-commit --graph --skip-pack-dependencies --run-new-validate --config-path sdk_validation_config.toml
        done       
    else
        ...
        python3 -m demisto_sdk validate -a --graph --skip-pack-dependencies --prev-ver $PREV_VER --run-new-validate --config-path sdk_validation_config.toml
    fi
elif [[ $CI_COMMIT_BRANCH =~ pull/[0-9]+ ]]; then
    python3 -m demisto_sdk validate -g --post-commit --graph --skip-pack-dependencies --run-new-validate --config-path sdk_validation_config.toml
elif [[ $CI_COMMIT_BRANCH = demisto/python3 ]]; then
    python3 -m demisto_sdk validate -g --post-commit --no-conf-json --allow-skipped --graph --skip-pack-dependencies --run-new-validate --config-path sdk_validation_config.toml
else
    python3 -m demisto_sdk validate -g --post-commit --graph --skip-pack-dependencies --run-new-validate --config-path sdk_validation_config.toml
fi