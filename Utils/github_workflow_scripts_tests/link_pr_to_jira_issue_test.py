...
@pytest.mark.parametrize('pr_body, is_merged, expected_ids, expected_actions',
                         PR_TEST_CASE)
def test_find_fixed_issue_in_body(pr_body, is_merged, expected_ids,
                                  expected_actions):
    """
...
def test_trigger_generic_webhook(requests_mock, is_merged, expected):
    ...
    post_mock = requests_mock.post(
        f'http://test.com/instance/execute/{link_pr_to_jira_issue.GENERIC_WEBHOOK_NAME}',
        status_code=200, json=[{"id": "1"}])
    option_mock = OptionMock('pr_link_example', '1', 'dummy pr',
                             PR_WITH_BOTH_BY_NEWLINE, is_merged)
