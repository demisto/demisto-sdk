...
def main(config, version):
    ...
    if last_release and cur_version != last_release:
        print_warning(
            f"You are using demisto-sdk {cur_version}, however version {last_release} is available.\n"
...
@click.help_option("-h", "--help")
@click.option("-i", "--input", help="The yml file to extract from", required=True)
@click.option(
    "-o",
    "--output",
    required=True,
...
@click.help_option("-h", "--help")
@click.option("-i", "--input", help="The path to the files to unify", required=True)
@click.option(
...
# ====================== validate ====================== #
@main.command(name="validate", short_help="Validate your content files.")
@click.help_option("-h", "--help")
@click.option(
    "-j",
    "--conf-json",
...
@click.option(
    "-p",
    "--path",
    help="Path of file to validate specifically, outside of a git directory.",
    hidden=True,
)
@click.option(
    "-a",
...
def validate(config, **kwargs):
    ...
    if file_path and not os.path.isfile(file_path) and not os.path.isdir(file_path):
        ...
        validator = FilesValidator(
            ...
            validate_conf_json=kwargs["conf_json"],
...
@click.option(
    ...
    "--content_version",
    default="",
    help="The content version which you want to appear in CommonServerPython.",
)
@click.option(
    "-p",
    "--preserve_bundles",
...
@click.option(
    "-i",
    "--input",
    help="Specify directory of integration/script",
...
@click.option(
    "-i",
    "--input",
    help="The path of an integration file or a package directory to upload",
    required=True,
)
...
@click.option(
    "-i",
    "--input",
    help="Valid JSON file path. If not specified then script will wait for user input in the terminal",
    required=False,
)
@click.option(
    "-p",
...
@click.option("-o", "--output", required=False, help="Specify output directory")
@click.option("-n", "--name", required=True, help="Specify test playbook name")
@click.option(
    "--no-outputs",
    is_flag=True,
...
def generate_test_playbook(**kwargs):
    ...
    if file_type not in ["integration", "script"]:
        print_error(
            f"Generating test playbook is possible only for an Integration or a Script."
        )
        return 1
...
@main.command(
    name="init",
    short_help="Initiate a new Pack, Integration or Script."
...
@click.option(
    "--integration",
    is_flag=True,
    help="Create an Integration based on HelloWorld example",
)
@click.option(
...
@click.option(
    "-e",
    "--examples",
    help="Path for file containing command or script examples."
...
@click.help_option("-h", "--help")
@click.option("-p", "--pack", help="Name of the pack.")
@click.option(
    "-u",
    "--update_type",
    help="The type of update being done. [major, minor, revision]",
    type=RNInputValidation(),
)
@click.option("--all", help="Update all changed packs", is_flag=True)
@click.option(
    "--pre_release",
...
@click.option(
    "-p",
    "--pack_folder_name",
    help="Pack folder name to find dependencies.",
    required=True,
)
@click.option("-i", "--id_set_path", help="Path to id set json file.", required=False)
