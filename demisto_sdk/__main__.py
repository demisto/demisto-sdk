...
from demisto_sdk.commands.common.tools import (find_type, get_content_path, get_demisto_tenants,
                                               get_last_remote_release_version, get_release_note_entries,
...
class DemistoSDK:
    ...
    def __init__(self):
        ...
        base_dir = get_content_path() or "."
        dotenv.load_dotenv(Path(base_dir) / '.env', override=True)  # Load a .env file from the cwd.
...
def main(config: "DemistoSDK", version, release_notes, host_only: bool, all_tenants: bool, **kwargs):
    ...
    if not os.getenv('DEMISTO_SDK_SKIP_VERSION_CHECK') or version:  # If the key exists/called to version
        try:
            ...
            __version__ = 'dev'
            print_warning('Could not find the version of the demisto-sdk. This usually happens when running in a development environment.')
        else:
