            ...
            if command in config.sections():
                for key in config[command]:
                    if key in args:
                        ...
                        if (
                            args[key] is False
                            and config[command][key] in true_synonyms
                        ):
                            args[key] = True

                        elif (
                            args[key] is None
                            and config[command][key] is not None
                        ):
...
def main(config, version, release_notes):
    ...
    dotenv.load_dotenv(
        Path(get_content_path()) / '.env', override=True
    )  # Load a .env file from the cwd.
    if (
...
def split(config, **kwargs):
    ...
    file_type: FileType = find_type(
        kwargs.get('input', ''), ignore_sub_categories=True
    )
    if file_type not in [
    ...
    if file_type in [
        ...
        json_splitter = JsonSplitter(
            ...
            new_module_file=kwargs.get('new_module_file'),
...
@click.help_option('-h', '--help')
@click.option(
    '--input', '-i', help='The yml file to extract from', required=True
)
@click.option(
    '--output',
    '-o',
...
def unify(**kwargs):
    ...
    if file_type == FileType.GENERIC_MODULE:
        ...
        rule_dir in os.path.abspath(kwargs['input'])
        for rule_dir in [PARSING_RULES_DIR, MODELING_RULES_DIR]
    ):
...
@click.option(
    '--zip-all',
    is_flag=True,
    help='Zip all the packs in one zip file.',
    default=False,
)
...
def validate(config, **kwargs):
    ...
    if kwargs['post_commit'] and kwargs['staged']:
        print_error(
            'Could not supply the staged flag with the post-commit flag'
        )
        sys.exit(1)
    try:
        ...
        if (
            not kwargs.get('validate_all')
            and not kwargs['use_git']
            and not file_path
        ):
            kwargs['use_git'] = True
...
@click.option(
    '--zip/--no-zip', help='Zip content artifacts folders', default=True
)
@click.option(
    '--packs',
...
@click.option(
    '--no-xsoar-linter', is_flag=True, help='Do NOT run XSOAR linter'
)
@click.option(
...
@click.option(
    '--no-pwsh-analyze', is_flag=True, help='Do NOT run powershell analyze'
)
@click.option(
    '--no-pwsh-test', is_flag=True, help='Do NOT run powershell test'
)
@click.option(
    '-kc', '--keep-container', is_flag=True, help='Keep the test container'
)
@click.option(
    '--prev-ver',
...
@click.option(
    '--no-coverage', is_flag=True, help='Do NOT run coverage report.'
)
@click.option(
    '--coverage-report',
...
def coverage_analyze(**kwargs):
    ...
    try:
        ...
        cov_report = CoverageReport(
            ...
            previous_coverage_report_url=kwargs[
                'previous_coverage_report_url'
            ],
        )
    ...
    * Note: Uploading classifiers to Cortex XSOAR is available from version 6.0.0 and up. *
    ...
    if kwargs['zip'] or kwargs['input_config_file']:
        if kwargs.pop('zip', False):
            ...
            config_file_to_parse = ConfigFileParser(
...
@click.option(
    '--playbook_id', '-p', help='The playbook ID to run.', required=True
)
@click.option(
    '--wait',
    '-w',
...
@click.option(
    '-i', '--input', help='Valid YAML integration file path.', required=False
)
@click.option(
    '-e',
    '--examples',
...
@click.option(
    '--script',
    is_flag=True,
    help='Create a Script based on BaseScript example',
)
@click.option(
...
def generate_docs(**kwargs):
    ...
    if (output_path := kwargs.get('output')) and not Path(
        output_path
    ).is_dir():
...
def _generate_docs_for_file(kwargs: Dict[str, Any]):
    ...
    if old_version and not old_version.lower().endswith('.yml'):
        print_error(
            f'Input old version {old_version} is not a valid yml file.'
        )
        return 1
...
def update_release_notes(**kwargs):
    ...
    try:
        ...
        print_error(
            f'An error occurred while updating the release notes: {str(e)}'
        )
        sys.exit(1)
...
def find_dependencies(**kwargs):
    ...
    input_paths = kwargs.get(
        'input'
    )  # since it can be multiple, received as a tuple
    ...
    output_path = kwargs.get(
        'output_path', ALL_PACKS_DEPENDENCIES_DEFAULT_PATH
    )
...
def postman_codegen(
    ...
    if config_out:
        ...
        yml_path = postman_config.generate_integration_package(
            output, is_unified=True
        )
        if package:
            yml_splitter = YmlSplitter(
                configuration=config.configuration,
                file_type=FileType.INTEGRATION.value,
                input=str(yml_path),
                output=str(output),
            )
...
@click.option(
    '-i',
    '--input_file',
    help='The swagger file to load in JSON format',
    required=True,
)
@click.option(
    '-cf',
...
def openapi_codegen(**kwargs):
    ...
    if not os.path.isdir(output_dir):
        print_error(
            f'The directory provided "{output_dir}" is not a directory'
        )
        sys.exit(1)
...
@click.option(
    '--xsiam-servers-path', help='Path to secret xsiam server metadata file.'
)
...
@click.option(
    '--templates',
    is_flag=True,
    help='Whether to print release notes templates',
)
@click.option(
    '-g',
    '--use-git',
...
@click.option(
    ...
    '--release-notes',
    is_flag=True,
    help='Will run only on release notes files',
)
@click.option(
    '-pkw',
...
@click.option(
    ...
    '--version',
    required=True,
    help='Version the input to be compatible with.',
)
@pass_config
...
@click.option(
    '-i', '--input_path', help='Valid integration file path.', required=True
)
@click.option(
    '-q',
    '--quiet',
...
@click.option(
    '-d',
    '--use_demisto',
    help='Run commands at Demisto automatically.',
    is_flag=True,
)
@click.option('--insecure', help='Skip certificate validation', is_flag=True)
@click.option(
