...
class Errors:
    ...
    @staticmethod
    def feed_wrong_from_version(
        file_path, given_fromversion, needed_from_version="5.5.0"
    ):
        return "{} is a feed and has wrong fromversion. got `{}` expected `{}`".format(
            file_path, given_fromversion, needed_from_version
        )

    @staticmethod
    def pwsh_wrong_version(file_path, given_fromversion, needed_from_version="5.5.0"):
        return (
            f"{file_path}: detected type: powershell and fromversion less than {needed_from_version}."
    ...
    def empty_display_configuration(file_path, field_name):
        return "No display details were entered for the field {} in the file {}.".format(
            field_name, file_path
        )
    ...
    def wrong_version(file_path, expected="-1"):
        return "{}: The version for our files should always be {}, please update the file.".format(
            file_path, expected
        )

    @staticmethod
    def wrong_version_reputations(file_path, object_id, version):
        return "{} Reputation object with id {} must have version {}".format(
            file_path, object_id, version
        )
    ...
    def dbot_invalid_output(file_path, command_name, missing_outputs, context_standard):
        return (
            ...
            "Fix according to context standard {} ".format(
                file_path, command_name, missing_outputs, context_standard
            )
        )

    @staticmethod
    def dbot_invalid_description(
        file_path, command_name, missing_descriptions, context_standard
    ):
        return (
    ...
    @staticmethod
    def missing_reputation(
        file_path, command_name, reputation_output, context_standard
    ):
        return (
    ...
    def added_required_fields(file_path, field):
        return "You've added required fields in the file '{}', the field is '{}'".format(
            file_path, field
        )
    ...
    def breaking_backwards_subtype(cls, file_path):
        return "{}: {}, You've changed the subtype, please undo.".format(
            file_path, cls.BACKWARDS
        )
    ...
    def breaking_backwards_docker(cls, file_path, old_docker, new_docker):
        return (
            "{}: {}, You've changed the docker for the file,"
            " this is not allowed. Old: {}, New: {} ".format(
                file_path, cls.BACKWARDS, old_docker, new_docker
            )
        )
    ...
    def not_latest_docker(file_path, current_docker, latest_docker):
        return (
            "{}: You're not using latest docker for the file,"
            " please update to latest version. Current: {}, Latest: {} ".format(
                file_path, current_docker, latest_docker
            )
        )
    ...
    def breaking_backwards_command_arg_changed(cls, file_path, command):
        return (
            "{}: {}, You've changed the name of a command or its arg in"
            " the file, please undo, the command was:\n{}".format(
                file_path, cls.BACKWARDS, command
            )
        )
    ...
    def missing_release_notes(file_path, rn_path):
        return "{}:  is missing releaseNotes, Please add it under {}".format(
            file_path, rn_path
        )

    @staticmethod
    def display_param(param_name, param_display):
        return "The display name of the {} parameter should be '{}'".format(
            param_name, param_display
        )

    @staticmethod
    def wrong_file_extension(file_extension, accepted_extensions):
        return "File extension {} is not valid. accepted {}".format(
            file_extension, accepted_extensions
        )

    @staticmethod
    def might_need_release_notes(file_path):
        return "{}: You might need RN in file, please make sure to check that.".format(
            file_path
        )
    ...
    def wrong_default_argument(file_path, arg_name, command_name):
        return "{}: The argument '{}' of the command '{}' is not configured as default".format(
            file_path, arg_name, command_name
        )
    ...
    def wrong_default_parameter_not_empty(param_name, default_value):
        return "The default value of the {} parameter should be {}".format(
            param_name, default_value
        )

    @staticmethod
    def wrong_required_value(param_name):
        return "The required field of the {} parameter should be False".format(
            param_name
        )
    ...
    def beta_field_not_found(file_path):
        return '{}: Beta integration yml file should have the field "beta: true", but was not found in the file.'.format(
            file_path
        )
    ...
    def no_default_arg(file_path, command_name):
        return "{}: Could not find default argument {} in command {}".format(
            file_path, command_name, command_name
        )
    ...
    def invalid_v2_integration_name(file_path):
        return (
            f"The display name of the v2 integration : {file_path} is incorrect , should be **name** v2.\n"
    ...
    def invalid_v2_script_name(file_path):
        return (
            f"The name of the v2 script : {file_path} is incorrect , should be **name**V2."
    ...
    def found_hidden_param(parameter_name):
        return (
            f'Parameter: "{parameter_name}" can\'t be hidden. Please remove this field.'
        )
...
SCRIPT_TEST_PY_REGEX = r"{}{}/([^\\/]+)/\1_test.py$".format(
    CAN_START_WITH_DOT_SLASH, SCRIPTS_DIR
)
SCRIPT_JS_REGEX = r"{}{}/([^\\/]+)/\1.js$".format(CAN_START_WITH_DOT_SLASH, SCRIPTS_DIR)
SCRIPT_PS_REGEX = r"{}{}/([^\\/]+)/\1.ps1$".format(
    CAN_START_WITH_DOT_SLASH, SCRIPTS_DIR
)
SCRIPT_YML_REGEX = r"{}{}/([^\\/]+)/([^\\/]+).yml$".format(
    CAN_START_WITH_DOT_SLASH, SCRIPTS_DIR
)
TEST_SCRIPT_REGEX = r"{}{}.*script-.*\.yml$".format(
    CAN_START_WITH_DOT_SLASH, TEST_PLAYBOOKS_DIR
)
SCRIPT_REGEX = r"{}{}/(script-[^\\/]+)\.yml$".format(
    CAN_START_WITH_DOT_SLASH, SCRIPTS_DIR
)

INTEGRATION_PY_REGEX = r"{}{}/([^\\/]+)/\1.py$".format(
    CAN_START_WITH_DOT_SLASH, INTEGRATIONS_DIR
)
INTEGRATION_TEST_PY_REGEX = r"{}{}/([^\\/]+)/\1_test.py$".format(
    CAN_START_WITH_DOT_SLASH, INTEGRATIONS_DIR
)
INTEGRATION_JS_REGEX = r"{}{}/([^\\/]+)/\1.js$".format(
    CAN_START_WITH_DOT_SLASH, INTEGRATIONS_DIR
)
INTEGRATION_PS_REGEX = r"{}{}/([^\\/]+)/\1.ps1$".format(
    CAN_START_WITH_DOT_SLASH, INTEGRATIONS_DIR
)
INTEGRATION_YML_REGEX = r"{}{}/([^\\/]+)/([^\\/]+).yml$".format(
    CAN_START_WITH_DOT_SLASH, INTEGRATIONS_DIR
)
INTEGRATION_REGEX = r"{}{}/(integration-[^\\/]+)\.yml$".format(
    CAN_START_WITH_DOT_SLASH, INTEGRATIONS_DIR
)
INTEGRATION_README_REGEX = r"{}{}/([^\\/]+)/README.md$".format(
    CAN_START_WITH_DOT_SLASH, INTEGRATIONS_DIR
)
INTEGRATION_OLD_README_REGEX = r"{}{}/integration-([^\\/]+_README.md)$".format(
    CAN_START_WITH_DOT_SLASH, INTEGRATIONS_DIR
)

INTEGRATION_CHANGELOG_REGEX = r"{}{}/([^\\/]+)/CHANGELOG.md$".format(
    CAN_START_WITH_DOT_SLASH, INTEGRATIONS_DIR
)
...
PACKS_INTEGRATION_YML_REGEX = r"{}{}/([^/]+)/{}/([^/]+)/([^.]+)\.yml".format(
    CAN_START_WITH_DOT_SLASH, PACKS_DIR, INTEGRATIONS_DIR
)
PACKS_INTEGRATION_REGEX = r"{}{}/([^/]+)/{}/([^/]+)\.yml$".format(
    CAN_START_WITH_DOT_SLASH, PACKS_DIR, INTEGRATIONS_DIR
)
PACKS_SCRIPT_NON_SPLIT_YML_REGEX = r"{}{}/([^/]+)/{}/script-([^/]+)\.yml".format(
    CAN_START_WITH_DOT_SLASH, PACKS_DIR, SCRIPTS_DIR
)
PACKS_SCRIPT_YML_REGEX = r"{}{}/([^/]+)/{}/([^/]+)/\2\.yml".format(
    CAN_START_WITH_DOT_SLASH, PACKS_DIR, SCRIPTS_DIR
)
PACKS_SCRIPT_PY_REGEX = r"{}{}/([^/]+)/{}/([^/]+)/\2\.py".format(
    CAN_START_WITH_DOT_SLASH, PACKS_DIR, SCRIPTS_DIR
)
PACKS_SCRIPT_TEST_PY_REGEX = r"{}{}/([^/]+)/{}/([^/]+)/\2_test\.py".format(
    CAN_START_WITH_DOT_SLASH, PACKS_DIR, SCRIPTS_DIR
)
PACKS_PLAYBOOK_YML_REGEX = r"{}{}/([^/]+)/{}/([^.]+)\.yml".format(
    CAN_START_WITH_DOT_SLASH, PACKS_DIR, PLAYBOOKS_DIR
)
PACKS_TEST_PLAYBOOKS_REGEX = r"{}{}/([^/]+)/{}/([^.]+)\.yml".format(
    CAN_START_WITH_DOT_SLASH, PACKS_DIR, TEST_PLAYBOOKS_DIR
)
PACKS_CLASSIFIERS_REGEX = r"{}{}/([^/]+)/{}/([^.]+)\.json".format(
    CAN_START_WITH_DOT_SLASH, PACKS_DIR, CLASSIFIERS_DIR
)
PACKS_DASHBOARDS_REGEX = r"{}{}/([^/]+)/{}/([^.]+)\.json".format(
    CAN_START_WITH_DOT_SLASH, PACKS_DIR, DASHBOARDS_DIR
)
PACKS_INCIDENT_TYPES_REGEX = r"{}{}/([^/]+)/{}/([^.]+)\.json".format(
    CAN_START_WITH_DOT_SLASH, PACKS_DIR, INCIDENT_TYPES_DIR
)
PACKS_INCIDENT_FIELDS_REGEX = r"{}{}/([^/]+)/{}/([^.]+)\.json".format(
    CAN_START_WITH_DOT_SLASH, PACKS_DIR, INCIDENT_FIELDS_DIR
)
PACKS_INDICATOR_FIELDS_REGEX = r"{}{}/([^/]+)/{}/([^.]+)\.json".format(
    CAN_START_WITH_DOT_SLASH, PACKS_DIR, INDICATOR_FIELDS_DIR
)
PACKS_LAYOUTS_REGEX = r"{}{}/([^/]+)/{}/([^.]+)\.json".format(
    CAN_START_WITH_DOT_SLASH, PACKS_DIR, LAYOUTS_DIR
)
PACKS_WIDGETS_REGEX = r"{}{}/([^/]+)/{}/([^.]+)\.json".format(
    CAN_START_WITH_DOT_SLASH, PACKS_DIR, WIDGETS_DIR
)
PACKS_REPORTS_REGEX = r"{}/([^/]+)/{}/([^.]+)\.json".format(PACKS_DIR, REPORTS_DIR)
PACKS_CHANGELOG_REGEX = r"{}{}/([^/]+)/CHANGELOG\.md$".format(
    CAN_START_WITH_DOT_SLASH, PACKS_DIR
)
PACKS_RELEASE_NOTES_REGEX = r"{}{}/([^/]+)/{}/([^/]+)\.md$".format(
    CAN_START_WITH_DOT_SLASH, PACKS_DIR, RELEASE_NOTES_DIR
)
PACKS_README_REGEX = r"{}{}/([^/]+)/README\.md".format(
    CAN_START_WITH_DOT_SLASH, PACKS_DIR
)
PACKS_README_REGEX_INNER = r"{}{}/([^/]+)/([^/]+)/([^/]+)/README\.md".format(
    CAN_START_WITH_DOT_SLASH, PACKS_DIR
)

PACKS_PACKAGE_META_REGEX = r"{}{}/([^/]+)/package-meta\.json".format(
    CAN_START_WITH_DOT_SLASH, PACKS_DIR
)

BETA_SCRIPT_REGEX = r"{}{}/(script-[^\\/]+)\.yml$".format(
    CAN_START_WITH_DOT_SLASH, BETA_INTEGRATIONS_DIR
)
BETA_INTEGRATION_REGEX = r"{}{}/(integration-[^\\/]+)\.yml$".format(
    CAN_START_WITH_DOT_SLASH, BETA_INTEGRATIONS_DIR
)
BETA_INTEGRATION_YML_REGEX = r"{}{}/([^\\/]+)/\1.yml$".format(
    CAN_START_WITH_DOT_SLASH, BETA_INTEGRATIONS_DIR
)
BETA_PLAYBOOK_REGEX = r"{}{}.*playbook-.*\.yml$".format(
    CAN_START_WITH_DOT_SLASH, BETA_INTEGRATIONS_DIR
)

PLAYBOOK_REGEX = r"{}(?!Test){}/playbook-.*\.yml$".format(
    CAN_START_WITH_DOT_SLASH, PLAYBOOKS_DIR
)
PLAYBOOK_CHANGELOG_REGEX = r"{}(?!Test){}/*_CHANGELOG.md$".format(
    CAN_START_WITH_DOT_SLASH, PLAYBOOKS_DIR
)

TEST_PLAYBOOK_REGEX = r"{}{}/playbook-.*\.yml$".format(
    CAN_START_WITH_DOT_SLASH, TEST_PLAYBOOKS_DIR
)
TEST_NOT_PLAYBOOK_REGEX = r"{}{}/(?!playbook).*-.*\.yml$".format(
    CAN_START_WITH_DOT_SLASH, TEST_PLAYBOOKS_DIR
)

INCIDENT_TYPE_REGEX = r"{}{}/incidenttype-.*\.json$".format(
    CAN_START_WITH_DOT_SLASH, INCIDENT_TYPES_DIR
)
INCIDENT_TYPE_CHANGELOG_REGEX = r"{}{}/*_CHANGELOG.md$".format(
    CAN_START_WITH_DOT_SLASH, INCIDENT_TYPES_DIR
)

INDICATOR_FIELDS_REGEX = r"{}{}/incidentfield-.*\.json$".format(
    CAN_START_WITH_DOT_SLASH, INDICATOR_FIELDS_DIR
)
INDICATOR_FIELD_CHANGELOG_REGEX = r"{}{}/*_CHANGELOG.md$".format(
    CAN_START_WITH_DOT_SLASH, INDICATOR_FIELDS_DIR
)
INCIDENT_FIELD_REGEX = r"{}{}/incidentfield-.*\.json$".format(
    CAN_START_WITH_DOT_SLASH, INCIDENT_FIELDS_DIR
)
INCIDENT_FIELD_CHANGELOG_REGEX = r"{}{}/*_CHANGELOG.md$".format(
    CAN_START_WITH_DOT_SLASH, INCIDENT_FIELDS_DIR
)

WIDGETS_REGEX = r"{}{}/widget-.*\.json$".format(CAN_START_WITH_DOT_SLASH, WIDGETS_DIR)
WIDGETS_CHANGELOG_REGEX = r"{}{}/*_CHANGELOG.md$".format(
    CAN_START_WITH_DOT_SLASH, WIDGETS_DIR
)

DASHBOARD_REGEX = r"{}{}.*dashboard-.*\.json$".format(
    CAN_START_WITH_DOT_SLASH, DASHBOARDS_DIR
)
DASHBOARD_CHANGELOG_REGEX = r"{}{}.*_CHANGELOG.md$".format(
    CAN_START_WITH_DOT_SLASH, DASHBOARD_REGEX
)

CONNECTIONS_REGEX = r"{}{}.*canvas-context-connections.*\.json$".format(
    CAN_START_WITH_DOT_SLASH, CONNECTIONS_DIR
)

CLASSIFIER_REGEX = r"{}{}.*classifier-.*\.json$".format(
    CAN_START_WITH_DOT_SLASH, CLASSIFIERS_DIR
)
CLASSIFIER_CHANGELOG_REGEX = r"{}{}.*_CHANGELOG.md$".format(
    CAN_START_WITH_DOT_SLASH, CLASSIFIERS_DIR
)

LAYOUT_REGEX = r"{}{}.*layout-.*\.json$".format(CAN_START_WITH_DOT_SLASH, LAYOUTS_DIR)
LAYOUT_CHANGELOG_REGEX = r"{}{}.*_CHANGELOG.md$".format(
    CAN_START_WITH_DOT_SLASH, LAYOUTS_DIR
)

REPORT_REGEX = r"{}{}.*report-.*\.json$".format(CAN_START_WITH_DOT_SLASH, REPORTS_DIR)
REPORT_CHANGELOG_REGEX = r"{}{}.*_CHANGELOG.md$".format(
    CAN_START_WITH_DOT_SLASH, REPORTS_DIR
)

MISC_REGEX = r"{}{}.*reputations\.json$".format(CAN_START_WITH_DOT_SLASH, MISC_DIR)
MISC_REPUTATIONS_REGEX = r"{}{}.reputations.json$".format(
    CAN_START_WITH_DOT_SLASH, MISC_DIR
)
REPUTATION_REGEX = r"{}{}/reputation-.*\.json$".format(
    CAN_START_WITH_DOT_SLASH, MISC_DIR
)
REPUTATION_CHANGELOG_REGEX = r"{}{}/*_CHANGELOG.md$".format(
    CAN_START_WITH_DOT_SLASH, MISC_DIR
)
...
PYTHON_SCRIPT_REGEXES = [SCRIPT_PY_REGEX, PACKS_SCRIPT_PY_REGEX]
...
YML_ALL_INTEGRATION_REGEXES: List[str] = sum(
    [YML_INTEGRATION_REGEXES, YML_BETA_INTEGRATIONS_REGEXES, ], []
)

YML_BETA_SCRIPTS_REGEXES: List[str] = [
    BETA_SCRIPT_REGEX,
]
YML_SCRIPT_REGEXES: List[str] = [SCRIPT_REGEX, PACKS_SCRIPT_YML_REGEX, SCRIPT_YML_REGEX]

YML_ALL_SCRIPTS_REGEXES: List[str] = sum(
    [YML_BETA_SCRIPTS_REGEXES, YML_SCRIPT_REGEXES], []
)
...
YML_ALL_PLAYBOOKS_REGEX: List[str] = sum(
    [YML_PLAYBOOKS_NO_TESTS_REGEXES, YML_TEST_PLAYBOOKS_REGEXES, ], []
)
...
PATHS_TO_VALIDATE: List[str] = sum(
    [PYTHON_ALL_REGEXES, JSON_ALL_REPORTS_REGEXES, JSON_ALL_MISC_REGEXES, BETA_REGEXES],
    [],
)
...
TESTS_DIRECTORIES = ["testdata", "test_data", "data_test"]
...
SCHEMA_TO_REGEX = {
    ...
    "changelog": [
        INTEGRATION_CHANGELOG_REGEX,
        PACKS_CHANGELOG_REGEX,
        REPUTATION_CHANGELOG_REGEX,
        INCIDENT_TYPE_CHANGELOG_REGEX,
        INCIDENT_FIELD_CHANGELOG_REGEX,
        INDICATOR_FIELD_CHANGELOG_REGEX,
        DASHBOARD_CHANGELOG_REGEX,
        CLASSIFIER_CHANGELOG_REGEX,
        LAYOUT_CHANGELOG_REGEX,
        REPORT_CHANGELOG_REGEX,
        WIDGETS_CHANGELOG_REGEX,
...
FEED_REQUIRED_PARAMS = [
    {"display": "Fetch indicators", "name": "feed", "type": 8, "required": False},
    {
    ...
    {"display": "", "name": "feedExpirationInterval", "type": 1, "required": False},
    {
