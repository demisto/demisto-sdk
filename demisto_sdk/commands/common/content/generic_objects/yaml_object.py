...
class YAMLObject:
    def __init__(self, path: Union[Path, str]):
        self._path = YAMLObject._fix_path(path)
    ...
    @property
    def id(self):
        return self.__getitem__('commonfields')['id']
    ...
    def docker_image(self):
        return self.__getitem__('script')['dockerimage']
    ...
    def _unserialize(self) -> dict:
        try:
            yaml_to_dict = ruamel.yaml.load(self._path.open())
        except Exception as e:
    ...
    def _serialize(self, dest):
        try:
            ruamel.yaml.dump(self._yaml_as_dict, Path(dest).open())
        except Exception as e:
            raise BaseException(f"{self._path} unable to dump yaml object: {e}")

    def __getitem__(self, item) -> Any:
        if not self._yaml_as_dict:
            self._yaml_as_dict = self._unserialize()

        return self._yaml_as_dict[item]

    def __setitem__(self, key, value) -> None:
        if not self._yaml_as_dict:
            self._yaml_as_dict = self._unserialize()

        self._yaml_as_dict[key] = value
