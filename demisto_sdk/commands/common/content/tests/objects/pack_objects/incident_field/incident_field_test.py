...
def mock_incident_field(repo, incident_field_data=None):
    pack = repo.create_pack('Temp')
    return pack.create_incident_field(name='MyIncidentField', content=incident_field_data if incident_field_data
                                      else INCIDENT_FIELD)


def test_objects_factory(repo):
    incident_field = mock_incident_field(repo)
    obj = path_to_pack_object(incident_field.path)
    assert isinstance(obj, IncidentField)


def test_prefix(repo):
    incident_field = mock_incident_field(repo)
    obj = IncidentField(incident_field.path)
...
def test_is_valid_name_sanity(current_file, answer, repo, capsys):
    incident_field_data = INCIDENT_FIELD.copy()
    for key, val in current_file.items():
        incident_field_data[key] = val
    incident_field = mock_incident_field(repo, incident_field_data)
    incident_field_obj = IncidentField(incident_field.path)
