...
def add_text_to_incident(incident, text_fields):
    incident[INCIDENT_TEXT_FIELD] = get_texts_from_incident(
        incident, text_fields
    )
...
def get_incidents_by_time(
    ...
    query = '{0}:>="{1}" and {0}:<="{2}" and type:"{3}"'.format(
        time_field, min_date.isoformat(), max_date.isoformat(), incident_type
    )
...
def pre_process_nlp(text_data):
    ...
    tokenized_text_data = map(
        lambda x: x.get('tokenizedText'), processed_text_data
    )
...
def main():
    ...
    TEXT_FIELDS = set(
        map(lambda x: x.lower(), demisto.args()['textFields'].split(','))
    )
    ...
    candidates = get_incidents_by_time(
        incident[TIME_FIELD],
        incident['type'],
        incident['id'],
        HOURS_TIME_FRAME,
    ...
    candidates = [
        x
        for x in candidates
        if len(x.get(INCIDENT_TEXT_FIELD, 0)) >= MIN_TEXT_LENGTH
    ]
    ...
    if len(similar_incidents or []) > 0:
        similar_incidents_rows = map(
            lambda incident: incident_to_record(incident, TIME_FIELD),
            similar_incidents,
        )
        similar_incidents_rows = sorted(
            similar_incidents_rows, key=lambda x: x['Time']
        )
        context = {
            'similarIncidentList': similar_incidents_rows[
                :MAX_CANDIDATES_IN_LIST
            ],
            'similarIncident': similar_incidents_rows[0],
            'isSimilarIncidentFound': True,
        }
        markdown_result = tableToMarkdown(
            'Similar incidents',
