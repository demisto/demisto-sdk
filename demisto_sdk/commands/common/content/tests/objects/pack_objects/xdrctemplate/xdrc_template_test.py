...
class TestXDRCTemplate:
    ...
    def test_prefix(self, pack):
        xdrc_template = pack.create_xdrc_template('external-xdrctemplate-xdrc_template_name')
        ...
        xdrc_template = pack.create_xdrc_template('xdrc_template_name')
        obj = XDRCTemplate(xdrc_template.xdrc_template_tmp_path)
        assert obj.normalize_file_name() == f"external-xdrctemplate-{xdrc_template.xdrc_template_tmp_path.name}"
