...
ERROR_CODE = {
    ...
    'playbook_condition_has_no_else_path': {
        'code': 'PB112',
        'ui_applicable': False,
        'related_field': 'nexttasks',
    },
...
class Errors:
    ...
    def file_cannot_be_deleted(file_path: str):
        return (
            f'The file {file_path} cannot be deleted. Please restore the file.'
        )
    ...
    def file_name_include_spaces_error(file_name):
        return "Please remove spaces from the file's name: '{}'.".format(
            file_name
        )
    ...
    def no_default_value_in_parameter(param_name):
        return 'The {} parameter should have a default value'.format(
            param_name
        )
    ...
    def wrong_required_type(param_name):
        return 'The type field of the {} parameter should be 8'.format(
            param_name
        )
    ...
    def fromlicense_in_parameters(param_name):
        return 'The "fromlicense" field of the {} parameter is not allowed for contributors'.format(
            param_name
        )
    ...
    def removed_integration_parameters(field):
        return "You've removed integration parameters, the removed parameters are '{}'".format(
            field
        )
    ...
    def empty_display_configuration(field_name):
        return 'No display details were entered for the field {}'.format(
            field_name
        )
    ...
    def feed_wrong_from_version(
        given_fromversion, needed_from_version='5.5.0'
    ):
    ...
    @staticmethod
    @error_code_decorator
    def parameter_missing_from_yml_not_community_contributor(
        name, correct_format
    ):
        """
    ...
    def error_starting_mdx_server(line):
        return (
            f'Failed starting mdx server. stdout: {line}.\n'
    ...
    @staticmethod
    @error_code_decorator
    def is_valid_integration_file_path_in_integrations_folder(
        integration_file,
    ):
        return (
            f'The integration file name: {integration_file} is invalid, '
    ...
    @staticmethod
    @error_code_decorator
    def integration_is_deprecated_and_used(
        integration_name: str, commands_dict: dict
    ):
    ...
    @staticmethod
    @error_code_decorator
    def invalid_hidden_attribute_for_param(
        param_name: str, invalid_value: str
    ):
    ...
    def breaking_backwards_context(cls):
        return (
            "{}, You've changed the context in the file,"
    ...
    def remove_field_from_widget(field, widget):
        return (
            f'The field {field} needs to be removed from the widget: {widget}.'
        )
    ...
    def image_in_package_and_yml():
        return (
            "Image in both yml and package, remove the 'image' "
    ...
    def cant_parse_pack_file_to_list(file_name):
        return (
            f'Could not parse the contents of "{file_name}" file into a list'
        )
    ...
    @staticmethod
    @error_code_decorator
    def pack_timestamp_field_not_in_iso_format(
        field_name, value, changed_value
    ):
        return (
    ...
    def invalid_readme_insert_image_link_error(path):
        return (
            f'Image link was not found, either insert it or remove it:\n{path}'
        )
    ...
    def file_id_changed(old_version_id, new_file_id):
        return (
            f'The file id has changed from {old_version_id} to {new_file_id}'
        )
    ...
    def invalid_package_dependencies(pack_name):
        return (
            f'{pack_name} depends on NonSupported / DeprecatedContent packs.'
        )
    ...
    def invalid_from_version_in_pre_process_rules():
        return (
            'fromVersion field in Pre Process Rule needs to be at least 6.5.0'
        )
    ...
    @staticmethod
    @error_code_decorator
    def integration_is_skipped(
        integration_id, skip_comment: Optional[str] = None
    ):
    ...
    def breaking_backwards_no_old_script(cls, e):
        return '{}\n{}, Could not find the old file.'.format(
            cls.BACKWARDS, str(e)
        )
    ...
    def xsoar_config_file_is_not_json(file_path):
        return (
            f'Could not load {file_path} as a JSON XSOAR configuration file.'
        )
    ...
    @staticmethod
    @error_code_decorator
    def entity_name_contains_excluded_word(
        entity_name: str, excluded_words: List[str]
    ):
    ...
    @staticmethod
    @error_code_decorator
    def empty_outputs_common_paths(
        paths: Dict[str, List[str]], yaml_path: str
    ):
        commands_str = '\n'.join(
            f'{command}:\t' + ', '.join(outputs)
            for command, outputs in paths.items()
        )
