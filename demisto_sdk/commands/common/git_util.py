...
class GitUtil:
    ...
    def added_files(
        ...
        untracked = untracked_added.union(
            untracked_modified.intersection(committed)
        )

        staged = staged.union(committed_added_locally_modified).union(
            untracked
        )
    ...
    def renamed_files(
        ...
        if get_only_current_file_names:
            all_renamed_files_only_new = {
                file[1] for file in all_renamed_files
            }
    ...
    def _get_untracked_files(self, requested_status: str) -> set:
        ...
        for line in git_status:
            line = line.strip()
            file_status = (
                line.split()[0].upper() if not line.startswith('?') else 'A'
            )
    ...
    def _only_last_commit(
        self, prev_ver: str, requested_status: str
    ) -> Set:  # pragma: no cover
        ...
        # when checking branch against itself only return the last commit.
        if (
            self.get_current_working_branch()
            != self.handle_prev_ver(prev_ver)[1]
        ):
            return set()
    ...
    def _check_file_status(
        self, file_path: str, remote: str, branch: str
    ) -> str:
