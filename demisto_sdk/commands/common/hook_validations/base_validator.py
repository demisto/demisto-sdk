...
class BaseValidator:
    ...
    @staticmethod
    def is_error_not_allowed_in_pack_ignore(
    ...
    def handle_error(
        ...
        is_error_not_allowed_in_pack_ignore = (
            self.is_error_not_allowed_in_pack_ignore(
                error_code=error_code,
                ignored_errors_pack_ignore=ignored_errors_pack_ignore,
            )
        )
        ...
        if should_print and not self.suppress_print:
            if suggested_fix and not is_error_not_allowed_in_pack_ignore:
                ...
                if error_code == 'ST109':
                    click.secho(
                        'Please add to the root of the yml.\n', fg='bright_red'
                    )
        ...
        self.json_output(file_path, error_code, error_message, warning)
        self.add_to_report_error_list(
            error_code, file_path, FOUND_FILES_AND_ERRORS
        )
    ...
    def add_flag_to_ignore_list(self, file_path, flag):
        if flag in PRESET_ERROR_TO_IGNORE:
            ...
            deprecated_ignored_errors = (
                self.create_reverse_ignored_errors_list(
    ...
    def json_output(
        ...
        if (
            entity_type == FileType.INTEGRATION.value
            and error_code.startswith('IM')
        ):
