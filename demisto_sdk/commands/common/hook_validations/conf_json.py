...
class ConfJsonValidator(BaseValidator):
    ...
    def is_valid_file_in_conf_json(self, current_file, file_type, file_path, ignored_errors=None):
        ...
        if file_type == FileType.SCRIPT:
            return self.has_unskipped_test_playbook(current_file=current_file, entity_id=entity_id, file_path=file_path)
        return True
    ...
    def integration_has_unskipped_test_playbook(self, integration_data, integration_id, file_path):
        ...
        for test in conf_tests:
            if "integrations" in test:
                if (
                    isinstance(test["integrations"], str) and integration_id == test["integrations"]
