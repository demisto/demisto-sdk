...
class ConfJsonValidator(BaseValidator):
    ...
    def is_valid_conf_json(self):
        ...
        unmockable_integrations_conf = self.conf_data[
    ...
    def is_valid_description_in_conf_dict(self, checked_dict):
        ...
        if problematic_instances:
            (
                error_message,
                error_code,
            ) = Errors.description_missing_from_conf_json(
                problematic_instances
            )
            if self.handle_error(
                error_message, error_code, file_path=CONF_PATH
            ):
    ...
    def has_unskipped_test_playbook(
        ...
        if not any(
            test_playbooks_unskip_status.values()
        ) and not self.has_unittest(file_path):
            (
                error_message,
                error_code,
            ) = Errors.all_entity_test_playbooks_are_skipped(entity_id)
            if self.handle_error(
