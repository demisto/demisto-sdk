...
class ContentEntityValidator(BaseValidator):
    ...
    def name_does_not_contain_excluded_word(self) -> bool:
        ...
        if any(excluded_word in lowercase_name for excluded_word in EXCLUDED_DISPLAY_NAME_WORDS):
            error_message, error_code = Errors.entity_name_contains_excluded_word(name, EXCLUDED_DISPLAY_NAME_WORDS)
            if self.handle_error(error_message, error_code, file_path=self.file_path):
                return False
        return True
    ...
    def should_run_fromversion_validation(self):
        ...
        if any(
            (feature_branch_name in self.prev_ver or feature_branch_name in self.branch_name)
    ...
    def are_fromversion_and_toversion_in_correct_format(self) -> bool:
        ...
        for field, name in ((from_version, "fromversion"), (to_version, "toversion")):
            if not FROM_TO_VERSION_REGEX.fullmatch(field):
                error_message, error_code = Errors.incorrect_from_to_version_format(name)
    ...
    def is_valid_fromversion(self):
        ...
        if LooseVersion(self.current_file.get(from_version_field, DEFAULT_CONTENT_ITEM_FROM_VERSION)) < LooseVersion(
    ...
    def is_valid_fromversion_for_generic_objects(self):
        ...
        if LooseVersion(self.current_file.get("fromVersion", DEFAULT_CONTENT_ITEM_FROM_VERSION)) < LooseVersion(
            GENERIC_OBJECTS_OLDEST_SUPPORTED_VERSION
        ):
            error_message, error_code = Errors.no_minimal_fromversion_in_file(
