...
class DeprecationValidator:
    ...
    def validate_integration_commands_deprecation(
        ...
        usage_dict = (
            self.collect_non_deprecated_playbooks_implementing_the_command_list(
                deprecated_commands_list, usage_dict, integration_id
            )
        )
        usage_dict = (
            self.collect_non_deprecated_scripts_implementing_integration_commands(
    ...
    def validate_script_deprecation(self, script_name: str):
        ...
        usage_list = self.collect_non_deprecated_scripts_implementing_another_script(
    ...
    def collect_non_deprecated_playbooks_implementing_the_command_list(
    ...
    def validate_integration_commands_not_in_playbook(
        ...
        for command, integration_name in command_to_integration.items():
            if command in deprecated_commands_list:
                if integration_name == integration_id or (
                    not integration_name and command not in COMMANDS_TO_IGNORE
                ):
    ...
    def collect_non_deprecated_scripts_implementing_integration_commands(
    ...
    def collect_non_deprecated_scripts_implementing_another_script(
    ...
    def collect_non_deprecated_playbooks_that_use_a_given_playbook_or_script(
        self, current_entity_name: str, usage_list: List[str], key_to_check: str
    ):
        """
