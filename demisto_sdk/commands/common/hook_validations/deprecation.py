...
class DeprecationValidator:
    ...
    def validate_integration_commands_deprecation(self, deprecated_commands_list: List[str], integration_id: str):
        """
        ...
        Return:
            dict: A dictionary where the keys are the integration's deprecated commands that are used in none-deprecated  scripts / playbooks.
            The values are the file names where they're being used
        """
        usage_dict: Dict[str, list] = {}

        usage_dict = self.collect_non_deprecated_playbooks_implementing_the_command_list(deprecated_commands_list, usage_dict, integration_id)
        usage_dict = self.collect_non_deprecated_scripts_implementing_integration_commands(deprecated_commands_list, usage_dict)
    ...
    def validate_script_deprecation(self, script_name: str):
        ...
        usage_list = self.collect_non_deprecated_scripts_implementing_another_script(script_name, usage_list)
        usage_list = self.collect_non_deprecated_playbooks_that_use_a_given_playbook_or_script(script_name, usage_list, key_to_check)

        return usage_list

    def collect_non_deprecated_playbooks_implementing_the_command_list(self, deprecated_commands_list, usage_dict: Dict[str, list], integration_id: str):
        """
        ...
        Args:
            deprecated_commands_list (list): A list of all the integration's deprecated commands.
            usage_dict (dict): A dictionary where the keys are the integration's deprecated commands
    ...
    def validate_integration_commands_not_in_playbook(self, usage_dict: Dict, deprecated_commands_list: List[str],
        ...
        Args:
            ...
            that are used in none-deprecated scripts / playbooks.
            The values are the file names where they're being used.
            command_to_integration(dict) A dict where the keys are the integrations commands that are being used
    ...
    def collect_non_deprecated_scripts_implementing_integration_commands(self, deprecated_commands_list: List[str], usage_dict: Dict):
        """
    ...
    def collect_non_deprecated_scripts_implementing_another_script(self, script_name: str, usage_list: List[str]):
        """
        ...
        for script in self.script_section:
            for script_val in script.values():
                if script_val.get("deprecated"):
                    continue
                depends_commands_list = script_val.get("depends_on")
                if depends_commands_list and script_name in depends_commands_list:
                    usage_list.append(script_val.get("file_path"))
        return usage_list

    def collect_non_deprecated_playbooks_that_use_a_given_playbook_or_script(self, current_entity_name: str, usage_list: List[str], key_to_check: str):
        """
        Collect from id_set all the playbooks that are non-deprecated and are implementing the given entity (script/playbook).

        Args:
            current_entity_name (str): The name of the script / playbook currently being checked.
        ...
        for playbook in self.playbook_section:
            for playbook_val in playbook.values():
                ...
                if implementing_entities:
                    usage_list = self.find_implementing_playbooks_and_scripts(usage_list, current_entity_name, implementing_entities, playbook_val)
        return usage_list

    def find_implementing_playbooks_and_scripts(self, usage_list: List[str], current_entity_name: str, implementing_entities: List[str], playbook: Dict):
        """
        ...
        Args:
            usage_list (list): A list of all the file paths that are using the script / playbook currently being checked.
            current_entity_name (str): The name of the script / playbook currently being checked.
            implementing_entities(dict) A list of all the entities that this currently checked playbook is using.
            playbook (dict): The playbook currently being checked.
        """
        for implementing_entity in implementing_entities:
            if implementing_entity == current_entity_name:
