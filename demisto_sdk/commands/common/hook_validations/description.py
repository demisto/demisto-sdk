...
class DescriptionValidator(BaseValidator):
    ...
    def contains_contrib_details(self):
        ...
        if contrib_details:
            (
                error_message,
                error_code,
            ) = Errors.description_contains_contrib_details()
        ...
        if not is_unified_integration:
            try:
                md_file_path = glob.glob(
                    os.path.join(
                        os.path.dirname(self.file_path), '*_description.md'
                    )
                )[0]
            except IndexError:
                (
            ...
            if BETA_INTEGRATION_DISCLAIMER not in description:
                (
                    error_message,
                    error_code,
                ) = Errors.no_beta_disclaimer_in_description()
                if self.handle_error(
                    error_message, error_code, file_path=self.file_path
                ):
                    self._is_valid = False
    ...
    def is_duplicate_description(self):
        ...
        if not re.match(
            PACKS_INTEGRATION_YML_REGEX, self.file_path, re.IGNORECASE
        ):
            package_path = os.path.dirname(self.file_path)
            try:
                base_name_without_extension: str = os.path.basename(
                    os.path.splitext(self.file_path)[0].replace(
                        '_description', ''
                    )
                )
                ...
                is_unified_integration = self.data_dictionary.get(
                    'script', {}
                ).get('script', '') not in {'-', ''}
                if not (
                    self.data_dictionary.get('deprecated')
                    or is_unified_integration
                ):
                    (
                        error_message,
                        error_code,
                    ) = Errors.no_description_file_warning()
                    self.handle_error(
                        error_message,
                        error_code,
                        file_path=self.file_path,
        ...
        if is_description_in_package and is_description_in_yml:
            ...
            if self.handle_error(
                error_message, error_code, file_path=package_path
            ):
                self._is_valid = False
                return False
    ...
    def is_valid_description_name(self):
        ...
        md_paths = glob.glob(
            os.path.join(os.path.dirname(self.file_path), '*.md')
        )

        description_file_path = self.file_path
        integrations_folder = os.path.basename(
            os.path.dirname(description_file_path)
        )
    ...
    def verify_demisto_in_description_content(self):
        ...
        # running on a description file so the file path is the description path
            ...
            integration_path = self.file_path.replace(
                '_description.md', '.yml'
            )
            yml_or_file = 'in the description file'
        ...
        if invalid_lines:
            (
                error_message,
                error_code,
            ) = Errors.description_contains_demisto_word(
