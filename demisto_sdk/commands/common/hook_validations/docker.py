...
class DockerImageValidator(BaseValidator):
    def __init__(
        ...
        ) = self.parse_docker_image(self.yml_docker_image)
    ...
    def get_python_version(self):
        if self.is_integration:
            python_version = self.yml_file.get('script', {}).get(
                'subtype', 'python2'
            )
        else:
    ...
    def get_docker_image_from_yml(self):
        if self.is_integration:
            docker_image = self.yml_file.get('script', {}).get(
                'dockerimage', ''
            )
        else:
    ...
    def parse_www_auth(www_auth):
        ...
        """
        match = re.match(
            r'.*realm="(.+)",service="(.+)".*', www_auth, re.IGNORECASE
        )
        if not match:
            return ()
        return match.groups()
    ...
    def clear_non_numbered_tags(tags):
        ...
        return [
            tag
            for tag in tags
            if re.match(r'^(?:\d+\.)*\d+$', tag) is not None
        ]
        ...
        if docker_image:
            ...
            try:
                ...
                if ':' in image:
                    ...
                    error_message, error_code = Errors.no_docker_tag(
                        docker_image
                    )
                    self.handle_error(
                        error_message, error_code, file_path=self.file_path
                ...
                (
                    error_message,
                    error_code,
                ) = Errors.docker_not_formatted_correctly(docker_image)
                self.handle_error(
                    error_message, error_code, file_path=self.file_path
                )
