...
class FieldBaseValidator(ContentEntityValidator):
    ...
    def is_valid_name(self):
        ...
        if found_words:
            error_message, error_code = Errors.invalid_incident_field_name(
                found_words
            )
    ...
    def is_valid_content_flag(self):
        ...
        if not self.current_file.get('content'):
            (
                error_message,
                error_code,
            ) = Errors.invalid_field_content_key_value()
            if self.handle_error(
                error_message,
                error_code,
                file_path=self.file_path,
    ...
    def is_valid_system_flag(self):
        ...
        if self.current_file.get('system'):
            (
                error_message,
                error_code,
            ) = Errors.invalid_incident_field_system_key_value()
            if self.handle_error(
                error_message,
                error_code,
                file_path=self.file_path,
    ...
    def does_cli_name_match_id(self):
        ...
        if cli_name != cli_name_expected and cli_name != _id:
            error_message, error_code = Errors.cli_name_and_id_do_not_match(
                _id
            )
            if self.handle_error(
                error_message, error_code, file_path=self.file_path
    ...
    def is_matching_cli_name_regex(self) -> bool:
        ...
        (
            error_message,
            error_code,
        ) = Errors.invalid_incident_field_cli_name_regex(
    ...
    def is_cli_name_is_builtin_key(self) -> bool:
        ...
        if not is_valid:
            (
                error_message,
                error_code,
            ) = Errors.invalid_incident_field_cli_name_value(cli_name)
        ...
        if old_from_version:
            ...
            if old_from_version != current_from_version:
                (
                    error_message,
                    error_code,
                ) = Errors.from_version_modified_after_rename()
                if self.handle_error(
                    error_message,
                    error_code,
                    file_path=self.file_path,
    ...
    def is_valid_field_name_prefix(self) -> bool:
        ...
        if pack_name and pack_name not in ignored_packs:
            ...
            error_message, error_code = Errors.invalid_incident_field_prefix(
                field_name
            )
            if self.handle_error(
                error_message,
    ...
    def is_aliased_fields_are_valid(self) -> bool:
        ...
        if not self.id_set_file:
            print_warning(
                'Validate will skip since an id set file was not provided'
            )
            return True
    ...
    def _get_incident_fields_by_aliases(self, aliases: List[dict]):
        ...
        """
        alias_ids: set = {
            f'incident_{alias.get("cliName")}' for alias in aliases
        }
