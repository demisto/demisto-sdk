...
class ImageValidator:
        ...
        if checked_type(file_path, INTEGRATION_REGXES) or re.match(
            ...
            if checked_type(file_path, YML_INTEGRATION_REGEXES):
                try:
                    ...
                    self.file_path = ""
                    print_error(
                        "You've created/modified a package but failed to provide an image as a .png file, "
    ...
    def oversize_image(self):
        ...
        if re.match(IMAGE_REGEX, self.file_path, re.IGNORECASE):
            if (
                os.path.getsize(self.file_path) > self.IMAGE_MAX_SIZE
            ):  # disable-secrets-detection
                print_error(
                    "{} has too large logo, please update the logo to be under 10kB".format(
                        self.file_path
                    )
                )
                self._is_valid = False
    ...
    def is_existing_image(self):
        ...
        if not (is_image_in_package or is_image_in_yml):
            print_error(
                "You have failed to add an image in the yml/package for {}".format(
                    self.file_path
                )
            )
            self._is_valid = False
    ...
    def is_not_default_image(self):
        ...
        if image in [
            ...
            print_error(
                "{} is the default image, please change to the "
