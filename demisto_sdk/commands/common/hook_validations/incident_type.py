...
class IncidentTypeValidator(ContentEntityValidator):
    ...
    def is_id_equals_name(self):
        ...
        """
        return super(IncidentTypeValidator, self)._is_id_equals_name(
            'incident_type'
        )
                ...
                for field in fields_to_include:
                    ...
                    if not isinstance(int_field, int) or int_field < 0:
                        (
                            error_message,
                            error_code,
                        ) = Errors.incident_type_integer_field(field)
                        if self.handle_error(
    ...
    def is_valid_playbook_id(self):
        ...
        if playbook_id and re.search(INVALID_PLAYBOOK_ID, playbook_id):
            (
                error_message,
                error_code,
            ) = Errors.incident_type_invalid_playbook_id_field()
            if self.handle_error(
                error_message, error_code, file_path=self.file_path
            ):
                return False
    ...
    def is_valid_autoextract(self):
        ...
        auto_extract_fields = auto_extract_data.get(
        ...
        if auto_extract_fields:
            ...
            for (
                incident_field,
                extracted_settings,
            ) in auto_extract_fields.items():
                extracting_all = extracted_settings.get(
                    'isExtractingAllIndicatorTypes'
                )
                extract_as_is = extracted_settings.get(
                ...
                # False and the extracted indicators list is empty
                    ...
                    if (
                        extracting_all is True
                        or len(extracted_indicator_types) > 0
                    ):
                        invalid_incident_fields.append(incident_field)
        ...
        if auto_extract_mode not in ['All', 'Specific']:
            (
                error_message,
                error_code,
            ) = Errors.incident_type_invalid_auto_extract_mode()
