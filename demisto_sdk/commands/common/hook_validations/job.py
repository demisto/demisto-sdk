...
class JobValidator(ContentEntityValidator):
    ...
    def is_valid_fromversion(self):
        if not self.from_version or LooseVersion(
            self.from_version
        ) < LooseVersion(FILETYPE_TO_DEFAULT_FROMVERSION.get(FileType.JOB)):
        ...
        if is_feed:
            if selected_feeds and is_all_feeds:
                ...
                (
                    error_message,
                    error_code,
                ) = Errors.missing_field_values_in_feed_job()
                formatted_error = self.handle_error(
                    error_message, error_code, file_path=self.file_path
                )
                if formatted_error:
