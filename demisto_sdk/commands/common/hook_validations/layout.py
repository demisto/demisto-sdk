...
class LayoutBaseValidator(ContentEntityValidator, ABC):
    ...
    @staticmethod
    def get_invalid_incident_fields_from_tabs(
        layout_tabs, content_fields
    ) -> List[str]:
        """
    ...
    def is_valid_from_version(self) -> bool:
        ...
        if LooseVersion(self.from_version) < LooseVersion(
            ...
            (
                error_message,
                error_code,
            ) = Errors.invalid_version_in_layoutscontainer('fromVersion')
            if self.handle_error(
                error_message, error_code, file_path=self.file_path
            ):
                return False
    ...
    def is_valid_to_version(self) -> bool:
        ...
        if self.to_version and LooseVersion(self.to_version) < LooseVersion(
            ...
            (
                error_message,
                error_code,
            ) = Errors.invalid_version_in_layoutscontainer('toVersion')
            if self.handle_error(
                error_message, error_code, file_path=self.file_path
            ):
                return False
    ...
    def is_valid_file_path(self) -> bool:
        ...
        if not output_basename.startswith('layoutscontainer-'):
            (
                error_message,
                error_code,
            ) = Errors.invalid_file_path_layoutscontainer(output_basename)
            if self.handle_error(
                error_message, error_code, file_path=self.file_path
            ):
                return False
    ...
    def is_incident_field_exist(
        ...
        if invalid_incident_fields:
            (
                error_message,
                error_code,
            ) = Errors.invalid_incident_field_in_layout(
                invalid_incident_fields
            )
            if self.handle_error(
                error_message, error_code, file_path=self.file_path
...
class LayoutValidator(LayoutBaseValidator):
    ...
    def is_valid_to_version(self) -> bool:
        ...
        Returns:
            bool. True if to version field is valid, else False.
        """
        if not self.to_version or LooseVersion(
            self.to_version
        ) >= LooseVersion(FROM_VERSION_LAYOUTS_CONTAINER):
            error_message, error_code = Errors.invalid_version_in_layout(
                'toVersion'
            )
            if self.handle_error(
    ...
    def is_valid_file_path(self) -> bool:
        ...
        if not output_basename.startswith('layout-'):
            error_message, error_code = Errors.invalid_file_path_layout(
                output_basename
            )
            if self.handle_error(
                error_message, error_code, file_path=self.file_path
