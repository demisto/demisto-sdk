...
def get_relative_urls(content: str) -> Set[ReadmeUrl]:
    ...
    def get_not_empty_urls(urls, is_markdown):
        return {
            ReadmeUrl(url[0], url[1], is_markdown) for url in urls if url[1]
        }
...
class ReadMeValidator(BaseValidator):
    ...
    def is_html_doc(self) -> bool:
        ...
        return (
            ...
            or (
                '<thead>' in self.readme_content
                and '<tbody>' in self.readme_content
            )
        )
    ...
    @error_codes('RM112')
    def check_readme_relative_url_paths(
        self, is_pack_readme: bool = False
    ) -> list:
        ...
        for url_link in relative_urls:
            ...
            (
                error_message,
                error_code,
            ) = Errors.invalid_readme_relative_url_error(url_link.get_url())
    ...
    def check_readme_relative_image_paths(
        self, is_pack_readme: bool = False
    ) -> list:
            ...
            if 'Insert the link to your image here' in relative_path:
                ...
                if not os.path.isfile(
                    ...
                    (
                        error_message,
                        error_code,
                    ) = Errors.invalid_readme_image_error(
                        prefix + f'({relative_path})',
    ...
    def check_readme_absolute_image_paths(
        self, is_pack_readme: bool = False
    ) -> list:
    ...
    def verify_no_default_sections_left(self) -> bool:
        ...
        errors += self._find_section_in_text(
            DEFAULT_SENTENCES, PACKS_TO_IGNORE
        )
    ...
    def is_context_different_in_yml(self) -> bool:
        ...
        if (
            f'{self.file_path} - [{missing_from_readme_error_code}]'
            in ignored_errors
            or f'{self.file_path} - [{missing_from_readme_error_code}]'
            in errors
            or f'{yml_file_path} - [{missing_from_yml_error_code}]'
            in ignored_errors
        ...
        for command_name in difference_context_paths:
            if difference_context_paths[command_name].get('only in yml'):
                error, code = Errors.readme_missing_output_context(
                    command_name,
                    ', '.join(
                        difference_context_paths[command_name].get(
                            'only in yml'
                        )
                    ),
                )
    ...
    def verify_copyright_section_in_readme_content(self):
        ...
        if invalid_lines:
            (
                error_message,
                error_code,
            ) = Errors.copyright_section_in_readme_error(invalid_lines)
            if self.handle_error(
                error_message, error_code, file_path=self.file_path
            ):
                is_valid = False
    ...
    def start_mdx_server(
        ...
        with ReadMeValidator._MDX_SERVER_LOCK:
            if not ReadMeValidator._MDX_SERVER_PROCESS:
                mdx_parse_server = (
                    Path(__file__).parent.parent / 'mdx-parse-server.js'
                )
                ReadMeValidator._MDX_SERVER_PROCESS = subprocess.Popen(
                    ['node', str(mdx_parse_server)],
                ...
                if 'MDX server is listening on port' not in line:
                    ...
                    (
                        error_message,
                        error_code,
                    ) = Errors.error_starting_mdx_server(line=line)
