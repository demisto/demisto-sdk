...
class ReleaseNotesValidator(BaseValidator):
        ...
        if modified_added_files:
            for file in modified_added_files:
                ...
                if find_type(file) in SKIP_RELEASE_NOTES_FOR_TYPES:
                    ...
                    checked_file_pack_name
                    and checked_file_pack_name == self.pack_name
                ):
    ...
    def has_release_notes_been_filled_out(self):
        release_notes_comments = self.strip_exclusion_tag(
    ...
    def is_docker_image_same_as_yml(self) -> bool:
        ...
        error_list = []
        for type, field in zip(
            ['Integrations', 'Scripts'], ['display', 'name']
        ):
            if type in release_notes_categories:
                splited_release_notes_entities = (
                    self.get_entities_from_category(
                ...
                for modified_yml_file in modified_yml_list:
                    ...
                    if (
                        ...
                        in splited_release_notes_entities
                    ):
                        entity_conent = (
                        ...
                        docker_version = self.get_docker_version_from_rn(
                            entity_conent
                        )
                        yml_docker_version = (
    ...
    def validate_json_when_breaking_changes(self) -> bool:
        ...
        if 'breaking change' in self.latest_release_notes.lower():
            ...
            (
                error_message,
                error_code,
            ) = Errors.release_notes_bc_json_file_missing(json_path)
            try:
    ...
    def strip_exclusion_tag(release_notes_comments):
        ...
        """
        return re.sub(
            r'<\!--.*?-->', '', release_notes_comments, flags=re.DOTALL
        )
