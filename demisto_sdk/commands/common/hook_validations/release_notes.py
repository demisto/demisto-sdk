...
class ReleaseNotesValidator(BaseValidator):
    ...
    def validate_content_type_header(self, content_type: str) -> bool:
        ...
        if content_type not in rn_valid_headers:
            (
                error_message,
                error_code,
            ) = Errors.release_notes_invalid_content_type_header(content_type=content_type, pack_name=self.pack_name)
    ...
    def validate_content_item_header(self, content_type: str, content_items: List) -> bool:
        ...
        for header in set(content_items).difference(content_items_display_names):
            (error_message, error_code,) = Errors.release_notes_invalid_content_name_header(
                    ...
                    if file_name and file_type and file_type in RN_HEADER_BY_FILE_TYPE:
                        if (RN_HEADER_BY_FILE_TYPE[file_type] not in self.latest_release_notes) or (
                            ...
                            (
                                error_message,
                                error_code,
                            ) = Errors.missing_release_notes_entry(file_type, self.pack_name, file_name)
                            if self.handle_error(error_message, error_code, self.release_notes_file_path):
                                is_valid = False
        return is_valid
    ...
    def is_docker_image_same_as_yml(self) -> bool:
        ...
        if len(error_list) > 0:
            (
                error_message,
                error_code,
            ) = Errors.release_notes_docker_image_not_match_yaml(rn_file_name, error_list, self.pack_path)
            if self.handle_error(error_message, error_code, file_path=self.release_notes_file_path):
                return False
