...
class StructureValidator(BaseValidator):
    ...
    def is_valid_file(self):
        ...
        """
        if (
            self.check_for_spaces_in_file_name()
            and self.is_valid_file_extension()
        ):
            answers = [
            ...
            if self.old_file:  # In case the file is modified
                click.secho(
                    f'Validating backwards compatibility for {self.file_path}'
                )
        ...
        try:
            ...
            try:
                ...
                if self.handle_error(
                    error_message, error_code, self.file_path
                ):
                    self.is_valid = False
                    return False
        return True
    ...
    def get_file_id_from_loaded_file_data(loaded_file_data):
        ...
        try:
            ...
            if not file_id:
                ...
                file_id = loaded_file_data.get('commonfields', {}).get(
                    'id', ''
                )
            if not file_id:
    ...
    def is_valid_fromversion_on_modified(self):
        ...
        from_version_old = self.old_file.get(
            'fromversion'
        ) or self.old_file.get('fromVersion')

        # if in old file there was no fromversion ,format command will add from version key with 1.0.0
        if (
            not from_version_old
            and from_version_new == OLD_FILE_DEFAULT_1_FROMVERSION
        ):
            return True
    ...
    def parse_missing_key_line(
        ...
        # if no path found this is an error in root
            ...
            error_message, error_code = Errors.pykwalify_missing_in_root(
                str(error_key)
            )
            return error_message, error_code, True
    ...
    def parse_undefined_key_line(
        ...
        if error_path:
            ...
            (
                error_message,
                error_code,
            ) = Errors.pykwalify_field_undefined_with_path(
                str(error_key), error_path_str
            )
            return error_message, error_code, True

        else:
            error_message, error_code = Errors.pykwalify_field_undefined(
                str(error_key)
            )
            return error_message, error_code, True
    ...
    def check_for_spaces_in_file_name(self):
        ...
        if file_name.count(' ') > 0:
            error_message, error_code = Errors.file_name_include_spaces_error(
                file_name
            )
