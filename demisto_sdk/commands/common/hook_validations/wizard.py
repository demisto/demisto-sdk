...
class WizardValidator(ContentEntityValidator):
    ...
    def collect_packs_dependencies(self) -> set:
        ...
        for pack_metadata in self.current_file.get('dependency_packs', []):
            packs = [
                pack.get('name', '') for pack in pack_metadata.get('packs', [])
            ]
            wizard_dependencies.update(packs)
        return wizard_dependencies

    def collect_set_playbooks(self) -> dict:
        set_playbooks = self.current_file.get('wizard', {}).get(
            'set_playbook', []
        )
        return {
    ...
    def collect_integrations(self, integration_category: str) -> set:
        integrations = self.current_file.get('wizard', {}).get(
        ...
        for dep in self._pack_deps:
            if dep not in packs:
                (
                    error_message,
                    error_code,
                ) = Errors.invalid_dependency_pack_in_wizard(dep)
    ...
    def are_integrations_in_dependency_packs(
        self, id_set_file: Optional[dict]
    ):
        if not id_set_file:
            click.secho(
        ...
        def are_integrations_mapped_to_dependency_packs(
            integrations: set,
        ) -> bool:
            ...
            for integration in integrations:
                if integration not in integrations_to_pack:
                    (
                        error_message,
                        error_code,
                    ) = Errors.invalid_integration_in_wizard(integration)
                    if self.handle_error(
                        error_message, error_code, file_path=self.file_path
                    ):
                        integration_in_dependency_packs = False
                elif (
                    pack := integrations_to_pack[integration]
                ) not in self._pack_deps:
                    (
                        error_message,
                        error_code,
    ...
    def are_playbooks_in_dependency_packs(self, id_set_file: Optional[dict]):
        ...
        for playbook in self._set_playbooks:
            if playbook not in playbooks_to_pack:
                error_message, error_code = Errors.invalid_playbook_in_wizard(
                    playbook
                )
                if self.handle_error(
                    error_message, error_code, file_path=self.file_path
    ...
    def do_all_fetch_integrations_have_playbook(self):
        ...
        for link in self._set_playbooks.values():
            if (
                not link
            ):  # handle case that a playbook was mapped to all integration
                return True
        ...
        if len(integrations) != 0:
            (
                error_message,
                error_code,
            ) = Errors.wizard_integrations_without_playbooks(integrations)
            if self.handle_error(
                error_message, error_code, file_path=self.file_path
            ):
                all_fetch_integrations_have_playbook = False
