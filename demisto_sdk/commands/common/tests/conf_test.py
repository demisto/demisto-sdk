...
def test_conf_json_description_not_given(mocker):
    ...
    assert (
        validator.is_valid_description_in_conf_dict(
            checked_dict=MISSING_DESCRIPTION
        )
        is False
...
@pytest.mark.parametrize(
    'conf_dict, answer', INTEGRATION_IS_SKIPPED_TEST_INPUTS
)
def test_integration_has_unskipped_test_playbook(mocker, conf_dict, answer):
    ...
    """
    mocker.patch.object(
        ConfJsonValidator, 'load_conf_file', return_value=conf_dict
    )

    validator = ConfJsonValidator()
    current = {
        'commonfields': {'id': 'SomeIntegration'},
...
def test_script_has_unskipped_test_playbook(
    mocker, test_playbooks, conf_dict, answer
):
    """
    Given:
        - A Script.
...
@pytest.mark.parametrize(
    'conf_data, yml_data', NOT_SKIPPED_DYNAMIC_SECTION_INPUTS
)
def test_not_skipped_test_playbook_for_dynamic_section(
    mocker, conf_data, yml_data
):
    """
    Given:
    - Script data.
    ...
    """
    mocker.patch.object(
        ConfJsonValidator, 'load_conf_file', return_value=conf_data
    )

    validator = ConfJsonValidator()

    assert validator.is_valid_file_in_conf_json(
        current_file=yml_data,
        file_type=FileType.SCRIPT,
        file_path='SomeFilePath',
    )
...
def test_has_unittests(mocker, integration, has_tests):
    ...
    try:
        if has_tests:
            test_file: Path = Path(integration.path) / (
                integration.name + '_test.py'
            )
            test_file.touch()
