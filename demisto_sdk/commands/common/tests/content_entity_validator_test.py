...
def test_yml_has_test_key(file_path, schema, expected):
    ...
    structure_validator = StructureValidator(
        file_path, predefined_scheme=schema
    )
...
def test_find_test_match(
    test_config, integration_id, test_playbook_id, expected, file_type
):
    ...
    """
    assert (
        is_test_config_match(test_config, test_playbook_id, integration_id)
        == expected
    )
...
def test_get_not_registered_tests(
    ...
    assert (
        get_not_registered_tests(
            conf_json_data, content_item_id, schema, tests
        )
        == expected
    )
...
def test_entity_valid_name_valid(repo, mocker):
    ...
    assert (
        integration_content_entity_validator.name_does_not_contain_excluded_word()
    )
...
def test_entity_valid_name_invalid(repo, mocker):
    ...
    assert (
        not script_content_entity_validator.name_does_not_contain_excluded_word()
    )
