...
def mock_docker_image_validator():
    with mock.patch.object(
        DockerImageValidator, '__init__', lambda x, y, z, w: None
    ):
...
class TestDockerImage:
    ...
    def test_none_demisto_docker(self, docker, docker_tag, expected_output):
        ...
        assert (
            docker_image_validator.get_docker_image_latest_tag(
                ...
                yml_docker_image='{}:{}'.format(docker, docker_tag),
            )
            == expected_output
        )
    ...
    @pytest.mark.parametrize(
        'www_auth, expected',
        [('AAArealm="2",service="3"AAA', ('2', '3')), ('bbb', ())],
    )
    ...
    def test_parse_docker_image(self):
        ...
        assert (
            'demisto/slack'
        ), '1.2.3.4' == docker_image_validator.parse_docker_image(
    ...
    def test_non_existing_docker(
        self, integration, capsys, requests_mock, mocker
    ):
        ...
        mocker.patch.object(
            DockerImageValidator, 'docker_auth', return_value='auth'
        )
        requests_mock.get(
    ...
    class TestIronBankDockerParse:
        ...
        def test_get_latest_commit_fails(
            ...
            api_url = 'https://repo1.dso.mil/api/v4/projects/dsop%2Fopensource%2Fpalo-alto-networks%2Ftest%2Ftest_project/pipelines'
            requests_mock.get(
                api_url, status_code=mocked_status, json=mock_results
            )
    ...
    class TestDeprecatedDockerImage:
        ...
        def test_command_is_docker_image_deprecated(self, requests_mock):
            ...
            docker_image_validator = mock_docker_image_validator()
            print(
                docker_image_validator.is_docker_image_deprecated(
                    'demisto/aiohttp'
                )
            )
            assert (
