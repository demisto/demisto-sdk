...
def test_get_docker_image_latest_tag(image, mocker):
    tag = DockerImageValidator.get_docker_image_latest_tag(self=BaseValidator(),
...
def test_none_demisto_docker(docker, docker_tag, expected_output):
    assert DockerImageValidator.get_docker_image_latest_tag(
        self=BaseValidator(), docker_image_name=docker,
...
def test_parse_docker_image():
    assert 'demisto/python', '1.3-alpine' == DockerImageValidator.parse_docker_image(docker_image='demisto/python:1.3-alpine')
    assert 'demisto/slack', '1.2.3.4' == DockerImageValidator.parse_docker_image(docker_image='demisto/slack:1.2.3.4')
    assert 'demisto/python', '' == DockerImageValidator.parse_docker_image(docker_image='demisto/python/1.2.3.4')
    assert ('', '') == DockerImageValidator.parse_docker_image(
        self=BaseValidator(), docker_image='blah/blah:1.2.3.4')
