...
class TestGitContentConfig:
    ...
    def test_valid_githubs(self, mocker, requests_mock, url: str, repo_name):
        ...
        """
        mocker.patch.object(
            GitContentConfig, '_search_gitlab_repo', return_value=None
        )
    ...
    def test_valid_gitlabs(self, mocker, requests_mock, url, repo_name):
        ...
        """
        requests_mock.get(
            'https://code.pan.run/api/v4/projects', json=[{'id': 3606}]
        )
        mocker.patch.object(Repo, 'remote', return_value=Urls([url]))
        git_config = GitContentConfig()
        assert (
        ...
        assert (
            git_config.base_api
            == GitContentConfig.BASE_RAW_GITLAB_LINK.format(
        ...
        git_config = GitContentConfig(
            repo_hostname='my-gitlab-hostname.com',
    ...
    def test_custom_github_url(self, mocker, requests_mock):
        ...
        mocker.patch.dict(
            os.environ,
            {GitContentConfig.ENV_REPO_HOSTNAME_NAME: custom_github},
        )  # test with env var
        mocker.patch.object(Repo, 'remote', return_value=Urls([url]))
        git_config = GitContentConfig()
    ...
    def test_custom_github_url_invalid(self, mocker, requests_mock):
        ...
        requests_mock.get(
            f'https://api.github.com/repos/{repo_name}', status_code=404
        )
        url = f'https://{custom_github}/{repo_name}'
        mocker.patch.object(Repo, 'remote', return_value=Urls([url]))
        git_config = GitContentConfig()
    ...
    def test_get_repo_name_empty_case(self, mocker):
        ...
        mocker.patch.object(
            GitContentConfig, '_search_github_repo', return_value=None
        )
        git_config = GitContentConfig()
        assert (
            git_config.current_repository
    ...
    def test_provide_repo_name(self, mocker, requests_mock):
        ...
        git_config = GitContentConfig(
            custom_repo_name, git_provider=GitProvider.GitLab
        )
        assert git_config.current_repository == custom_repo_name
        assert (
            git_config.base_api
            == 'https://gitlab.com/api/v4/projects/3/repository'
        )
    ...
    def test_provide_project_id(self, requests_mock):
        ...
        git_config = GitContentConfig(
            project_id=3,
            git_provider=GitProvider.GitLab,
            repo_hostname='code.pan.run',
        )
        assert git_config.project_id == 3
        assert (
            git_config.base_api
            == 'https://code.pan.run/api/v4/projects/3/repository'
        )
