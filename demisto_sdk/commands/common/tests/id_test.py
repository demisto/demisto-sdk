...
LAYOUT_DATA = {
    'my-layout': {'typename': 'my-layout', 'scripts': ['script_to_test']}
}
...
def test_is_incident_type_using_real_playbook__no_matching_playbook_id():
    ...
    incident_type_data = {
        'Zimperium Event': {'playbooks': 'a fake playbook id'}
    }
    validator.playbook_set = [
        {
            'Zimperium Incident Enrichment': {
...
def test_is_layout_using_existing_script_no_scripts():
    ...
    assert (
        validator._is_layout_scripts_found(
            layout_data=layout_data_without_scripts
        )
        is True
...
def test_is_classifier_incident_types_found__exists():
    ...
    assert (
        validator._is_classifier_incident_types_found(
...
def test_is_mapper_incident_types_found__exists():
    ...
    assert (
        validator._is_mapper_incident_types_found(mapper_data=mapper_data)
        is True
...
def test_is_unique_file_valid_in_set(pack):
    ...
    is_valid, error = validator.is_unique_file_valid_in_set(
        pack_path=pack.path
    )
    assert is_valid
    assert not error
...
class TestPlaybookEntitiesVersionsValid:
    ...
    def test_are_playbook_entities_versions_valid_scripts_and_subplaybooks(
        self, repo
    ):
        """
        ...
        with ChangeCWD(repo.path):
            (
                ...
                self.playbook_with_sub_playbook_not_in_id_set,
                playbook.yml.path,
            )
    ...
    def test_playbook_sub_playbook_exist(self, repo):
        ...
        with ChangeCWD(repo.path):
            ...
            is_subplaybook_name_exist = (
                self.validator.is_subplaybook_name_valid(
...
def test_invalid_playbook_is_file_valid_in_id_set(mocker):
    ...
    validator = IDSetValidations(
        is_circle=True, is_test_run=True, configuration=CONFIG
    )
    ...
    mocker.patch.object(
        validator, 'is_subplaybook_name_valid', return_value=True
    )
