...
class TestIntegrationValidator:
    ...
    REQUIED_FIELDS_FALSE = {
        'configuration': [{'name': 'test', 'required': False}]
    }
    REQUIED_FIELDS_TRUE = {
        'configuration': [{'name': 'test', 'required': True}]
    }
    ...
    def test_no_changed_removed_yml_fields(
        self, current_file, old_file, answer
    ):
        """
        Given
    ...
    @pytest.mark.parametrize(
        'current_file, old_file, answer',
        NO_REMOVED_INTEGRATION_PARAMETERS_INPUTS,
    )
    def test_no_removed_integration_parameters(
        self, current_file, old_file, answer
    ):
        """
        Given
    ...
    def test_get_field_to_required_dict(self, input_json, expected):
        assert (
            IntegrationValidator._get_field_to_required_dict(input_json)
            == expected
        )
    ...
    IS_CONTEXT_CHANGED_ADDED_PATH = [
        {
            'name': 'test',
            'outputs': [{'contextPath': 'test'}, {'contextPath': 'test2'}],
        }
    ]
    ...
    @pytest.mark.parametrize(
        'current, old, answer', CHANGED_COMMAND_OR_ARG_INPUTS
    )
            ...
            [
                {
                    ...
                    'arguments': [
                        {'name': 'argument_test_name_1', 'required': True}
                    ],
                },
                {
                    'name': 'command_test_name_2',
                    'arguments': [
                        {'name': 'argument_test_name_2', 'required': True}
                    ],
                },
            ],
            [
    ...
    @pytest.mark.parametrize(
        'current, old, expected_error_msg',
        CHANGED_COMMAND_OR_ARG_MST_TEST_INPUTS,
    )
    ...
    @patch(
        'demisto_sdk.commands.common.hook_validations.integration.print_error'
    )
    ...
    WITH_DUP_ARGS = [
        {
            'name': 'testing',
            'arguments': [{'name': 'test1'}, {'name': 'test1'}],
        }
    ]
    ...
    NON_DEFAULT_INFO = [
        {'name': 'API key', 'additionalinfo': 'you know, the API key'}
    ]
    ...
    @pytest.mark.parametrize(
        'args, answer, expecting_warning', DEFAULT_INFO_INPUTS
    )
    def test_default_params_default_info(
        self, capsys, args: List[Dict], answer: str, expecting_warning: bool
    ):
        validator = IntegrationValidator(
            mock_structure('', {'configuration': args})
        )
        assert (
            validator.default_params_have_default_additional_info() is answer
        )
        ...
        if expecting_warning:
            ...
            expected_warning = (
                f'[WARNING]: : [{warning_code}] - {warning_message}'
            )
        ...
        (
            {
                'script': {
                    'commands': [
                        {'name': 'command1', 'arguments': [{'name': 'arg1'}]}
                    ]
                }
            },
            True,
    ...
    def test_is_valid_default_array_argument_in_reputation_command(
        ...
        assert (
            validator.is_valid_default_array_argument_in_reputation_command()
            is answer
        )
    ...
    @pytest.mark.parametrize(
        'current, name, answer', IS_OUTPUT_FOR_REPUTATION_INPUTS
    )
    def test_is_outputs_for_reputations_commands_valid(
        self, current, name, answer
    ):
        current = {
            'script': {'commands': [{'name': name, 'outputs': current}]}
        }
        structure = mock_structure('', current)
        validator = IntegrationValidator(structure)
        validator.current_file = current
    ...
    @pytest.mark.parametrize(
        'configuration_setting, answer', IS_VALID_DISPLAY_INPUTS
    )
    def test_is_valid_display_configuration(
        self, configuration_setting, answer
    ):
    ...
    def test_valid_integration_parameters_default_value(
        ...
        assert (
            validator.is_valid_parameter_url_default_value() is excepted_result
        )

    @pytest.mark.parametrize(
        'support_level, expected_result',
        [('XSOAR', True), ('community', False)],
    )
    def test_fromlicense_in_integration_parameters_fields(
        ...
        with ChangeCWD(pack.repo_path):
            ...
            result = (
                validator.has_no_fromlicense_key_in_contributions_integration()
            )
    ...
    def test_valid_integration_path(self, integration):
        ...
        validator = IntegrationValidator(structure_validator)
        validator.file_path = (
            'Packs/VirusTotal/Integrations/VirusTotal/Virus_Total.yml'
        )
    ...
    @pytest.mark.parametrize(
        'file_name', ['IntNameTest.py', 'IntNameTest_test.py']
    )
    ...
    @pytest.mark.parametrize(
        'support, parameter_type, expected_result',
        [
            ('xsoar', 4, False),
            ('xsoar', 9, True),
            ('community', 4, True),
    ...
    def test_is_api_token_in_credential_type(
        self, pack, support, parameter_type, expected_result
    ):
        """
        Given
        ...
        with ChangeCWD(pack.repo_path):
            ...
            assert (
                validator.is_api_token_in_credential_type() == expected_result
            )

    IS_SKIPPED_INPUTS = [
        ({'skipped_integrations': {'SomeIntegration': 'No instance'}}, False),
        (
            {'skipped_integrations': {'SomeOtherIntegration': 'No instance'}},
            True,
        ),
    ]
    ...
    def test_verify_reputation_commands_has_reliability(
        ...
        current = (
            {
                'script': {
                    'commands': [{'name': command} for command in commands]
                },
...
class TestIsFeedParamsExist:
    ...
    HIDDEN_ALLOWED_TRUE = {
        'configuration': [
            {'name': 'longRunning', 'required': 'false', 'hidden': True}
        ]
    }
    ...
    def test_verify_yml_commands_match_readme(
        ...
        struct = mock_structure(
            current_file=yml_data, file_path=integration.yml.path
        )
    ...
    def test_verify_yml_commands_match_readme_no_readme_file(
        ...
        integration_validator = IntegrationValidator(struct)
        assert (
            integration_validator.verify_yml_commands_match_readme(
                is_modified=True
            )
            is False
        )
