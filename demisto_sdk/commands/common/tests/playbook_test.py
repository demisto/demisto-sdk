...
class TestPlaybookValidator:
    ...
    CONDITION_EXIST_PARTIAL_1 = {
        ...
        'version': -1,
        'tasks': {
            '1': {'type': 'condition', 'conditions': [], 'nexttasks': {}}
        },
    }
    ...
    DEPRECATED_INVALID_DESC3 = {
        'deprecated': True,
        'description': 'Deprecated. The playbook uses an unsupported'
        ' scraping API.',
    }
    DEPRECATED_INPUTS = [
    ...
    @pytest.mark.parametrize(
        'playbook_json, expected_result', IS_NO_ROLENAME_INPUTS
    )
    ...
    @pytest.mark.parametrize(
        'playbook_json, expected_result', IS_CONDITIONAL_INPUTS
    )
    def test_is_condition_branches_handled(
        self, playbook_json, expected_result
    ):
        structure = mock_structure('', playbook_json)
        validator = PlaybookValidator(structure)
    ...
    @pytest.mark.parametrize(
        'playbook_json, expected_result', IS_ROOT_CONNECTED_INPUTS
    )
    def test_is_root_connected_to_all_tasks(
        self, playbook_json, expected_result
    ):
        structure = mock_structure('', playbook_json)
        validator = PlaybookValidator(structure)
    ...
    def test_skipping_test_playbooks(
        self, mocker, playbook_path, expected_result
    ):
        """
        Given
        - A playbook
    ...
    @pytest.mark.parametrize(
        'playbook_json, expected_result', IS_DELETECONTEXT
    )
    def test_is_delete_context_all_in_playbook(
        self, playbook_json, expected_result
    ):
        """
        Given
    ...
    @pytest.mark.parametrize(
        'playbook_json, expected_result', IS_USING_INSTANCE
    )
    ...
    @pytest.mark.parametrize(
        'playbook_json, expected_result', IS_TASK_ID_EQUALS_ID
    )
    ...
    @pytest.mark.parametrize(
        'playbook_json, expected_result',
        [(CONDITIONAL_SCRPT_WITH_NO_DFLT_NXT_TASK, True)],
    )
    def test_verify_all_conditional_tasks_has_else_path(
        ...
        validator = PlaybookValidator(structure)
        assert (
            validator.verify_condition_tasks_has_else_path() is expected_result
        )

    @pytest.mark.parametrize(
        'playbook_task_json, expected_result', IS_ELSE_IN_CONDITION_TASK
    )
    def test_verify_else_for_conditions_task(
        self, playbook_task_json, expected_result
    ):
        """
        Given
    ...
    def test_is_valid_with_indicators_input(
        self, playbook_json, expected_result
    ):
        """
        Given
    ...
    def test_validate_readme_exists(
        ...
        assert (
            playbook_validator.validate_readme_exists(
                playbook_validator.validate_all
            )
            is expected_result
        )
