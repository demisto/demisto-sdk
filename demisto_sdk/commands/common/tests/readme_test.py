...
INVALID3_MD = (
    f'{git_path()}/demisto_sdk/tests/test_files/README-short-invalid.md'
)
...
def test_is_file_valid(mocker, current, answer):
    ...
    with requests_mock.Mocker() as m:
        ...
        mocker.patch.dict(
            os.environ,
            {
                'DEMISTO_README_VALIDATION': 'yes',
                'DEMISTO_MDX_CMD_VERIFY': 'yes',
            },
        )
...
def test_is_image_path_valid():
    ...
    assert (
        images_paths[0]
        and alternative_images_paths[0] in captured_output.getvalue()
    )
    assert (
        images_paths[1]
        and alternative_images_paths[1] in captured_output.getvalue()
    )
...
def test_verify_no_default_sections_left(
    integration, capsys, file_input, section
):
    """
    Given
...
@pytest.mark.parametrize(
    'errors_found, errors_ignore, expected', ERROR_FOUND_CASES
)
...
@pytest.mark.parametrize(
    'difference_found, expected', DIFFERENCE_CONTEXT_RESULTS_CASE
)
def test_context_difference_created_is_valid(
    mocker, difference_found, expected
):
    """
    Given
        - README that contains changes and YML file
...
def test_verify_template_not_in_readme(repo):
    ...
    with open(readme_path, 'w') as f:
        f.write(
            'This checks if we have the sentence %%FILL HERE%% in the README.'
        )
