...
class TestStructureValidator:
    ...
    @pytest.mark.parametrize(
        'path, scheme, answer, error', SCHEME_VALIDATION_INPUTS
    )
    def test_scheme_validation_playbook(
        self, path, scheme, answer, error, mocker
    ):
        mocker.patch.object(
    ...
    @pytest.mark.parametrize(
        'path, scheme, answer', SCHEME_VALIDATION_INDICATORFIELDS
    )
    def test_scheme_validation_indicatorfield(
        self, path, scheme, answer, mocker
    ):
    ...
    def test_scheme_validation_reputation(self, path, answer):
        validator = StructureValidator(
            file_path=path, predefined_scheme='reputation'
        )
        assert validator.is_valid_scheme() is answer
    ...
    @pytest.mark.parametrize(
        'path, old_file_path, answer', INPUTS_VALID_FROM_VERSION_MODIFIED
    )
    ...
    @pytest.mark.parametrize(
        'current_file, old_file, answer, error', INPUTS_IS_ID_MODIFIED
    )
    ...
    NEGATIVE_ERROR = (
        'found a slash in the ID even though it not contains a slash.'
    )
    ...
    @pytest.mark.parametrize(
        'path, answer, error', INPUTS_IS_FILE_WITHOUT_SLASH
    )
    ...
    def test_is_valid_file_extension(self, mocker):
        ...
        mocker.patch.object(
            StructureValidator, 'load_data_from_file', return_value=''
        )
        image = 'image.png'
    ...
    def test_job_missing_field(
        self, repo, capsys, is_feed: bool, missing_field: str
    ):
        """
        Given
...
class TestGetMatchingRegex:
    ...
    test_packs_regex_params = [
        ...
        (
            [
                'Packs/XDR/Playbooks/playbook-Test.yml',
                'Packs/XDR/Playbooks/Test.yml',
            ],
    ...
    @pytest.mark.parametrize(
        'acceptable,non_acceptable,regex', test_packs_regex_params
    )
