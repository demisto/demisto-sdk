...
class TestGenericFunctions:
    ...
    @pytest.mark.parametrize(
        "dir_path", ["demisto_sdk", f"{git_path()}/demisto_sdk/tests/test_files"]
    )
    ...
    @pytest.mark.parametrize(
        "data, output", [({"a": {"b": {"c": 3}}}, 3), ("a", 0), ([1, 2], 1)]
    )
...
class TestGetRemoteFile:
    ...
    def test_get_remote_file_origin(self):
        hello_world_yml = tools.get_remote_file(
            "Packs/HelloWorld/Integrations/HelloWorld/HelloWorld.yml", "master"
        )
        assert hello_world_yml
        assert hello_world_yml["commonfields"]["id"] == "HelloWorld"
    ...
    def test_is_test_file(self):
        test_file = tools.is_test_file(
            "Packs/HelloWorld/Integrations/HelloWorld/test_data/search_alerts.json"
        )
        assert test_file
...
class TestGetMatchingRegex:
    INPUTS = [
        (
            ...
            [PACKS_PLAYBOOK_YML_REGEX, PACKS_TEST_PLAYBOOKS_REGEX],
            PACKS_PLAYBOOK_YML_REGEX,
        ),
        (
...
class TestServerVersionCompare:
    ...
    LEFT_IS_LATER = 1
    RIGHT_IS_LATER = -1
    INPUTS = [(V0, V5, RIGHT_IS_LATER), (V5, V0, LEFT_IS_LATER), (V5, V5, EQUAL)]
...
class TestEntityAttributes:
    ...
    @pytest.mark.parametrize(
        "data, entity",
        [({"typeId": "wow"}, LAYOUTS_DIR), ({"name": "wow"}, PLAYBOOKS_DIR)],
    )
