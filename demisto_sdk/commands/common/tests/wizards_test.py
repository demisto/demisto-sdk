...
class TestWizardValidator:
            ...
            (
                {
                    'dependency_packs': [{'packs': [{'name': 'not_exists'}]}],
                    'wizard': {
                        'fetching_integrations': [{'name': 'not_exists'}]
                    },
                },
    ...
    def test_are_integrations_in_dependency_packs(
        self, current_file, id_set, answer
    ):
        """
        Given
        - a valid wizard
    ...
    def test_are_playbooks_in_dependency_packs(
        self, current_file, id_set, answer
    ):
        validator = get_validator(current_file)
            ...
            (
                {
                    'wizard': {
                        'set_playbook': [
                            {
                                'name': 'exists',
                                'link_to_integration': 'not_exists',
                            }
                        ],
    ...
    def test_do_all_fetch_integrations_have_playbook(
        self, current_file, answer
    ):
        validator = get_validator(current_file)
