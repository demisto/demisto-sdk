...
def get_all_content_objects_paths_in_dir(project_dir_list: Optional[Iterable]):
    ...
    for file_path in project_dir_list:
        files.extend(get_files_in_dir(file_path, GRAPH_SUPPORTED_FILE_TYPES, ignore_test_files=True))
...
def get_demisto_version(client: demisto_client) -> Union[Version, LegacyVersion]:
    ...
    try:
        ...
        logger.warning("Could not parse Xsoar version, please make sure the environment is properly configured.")
        return parse("0")
