...
def get_script_data(file_path, script_code=None):
    ...
    script_executions = sorted(
        list(set(re.findall(r"demisto.executeCommand\(['\"](\w+)['\"].*", script_code)))
    )
    pack = get_pack_name(file_path)
...
def update_object_in_id_set(obj_id, obj_data, file_path, instances_set):
    ...
    is_added_from_version = (
        True if re.search(r"\+fromversion: .*", change_string) else False
    )
    is_added_to_version = (
        True if re.search(r"\+toversion: .*", change_string) else False
    )
    ...
    for instance in instances_set:
        ...
        if obj_id == instance_id:
            if is_added_from_version or (
                not is_added_from_version
                and file_from_version == integration_from_version
            ):
                if is_added_to_version or (
                    not is_added_to_version
                    and file_to_version == integration_to_version
                ):
...
def process_integration(file_path, print_logs):
    ...
    if os.path.isfile(file_path):
        if checked_type(
            file_path,
            (INTEGRATION_REGEX, BETA_INTEGRATION_REGEX, PACKS_INTEGRATION_REGEX),
        ):
            if print_logs:
...
def process_script(file_path, print_logs):
    ...
    if os.path.isfile(file_path):
        if checked_type(
            file_path,
            (SCRIPT_REGEX, PACKS_SCRIPT_YML_REGEX, PACKS_SCRIPT_NON_SPLIT_YML_REGEX),
        ):
            if print_logs:
                print("adding {} to id_set".format(file_path))
...
def process_playbook(file_path, print_logs):
    res = []
    if checked_type(
        file_path, (PACKS_PLAYBOOK_YML_REGEX, PLAYBOOK_REGEX, BETA_PLAYBOOK_REGEX)
    ):
        if print_logs:
            print("adding {} to id_set".format(file_path))
...
def process_test_playbook_path(file_path, print_logs):
    ...
    playbook = None
    if checked_type(
        file_path, (TEST_SCRIPT_REGEX, PACKS_TEST_PLAYBOOKS_REGEX, TEST_PLAYBOOK_REGEX)
    ):
...
def re_create_id_set(
    id_set_path="./Tests/id_set.json", objects_to_create=None, print_logs=True
):
    ...
    with click.progressbar(
        length=12, label="Progress of id set creation"
    ) as progress_bar:
        if "Integrations" in objects_to_create:
            print_color("\nStarting iterating over Integrations", LOG_COLORS.GREEN)
            for arr in pool.map(
                partial(process_integration, print_logs=print_logs),
        ...
        if "Playbooks" in objects_to_create:
            ...
            for arr in pool.map(
                partial(process_playbook, print_logs=print_logs), get_playbooks_paths()
            ):
        ...
        if "Scripts" in objects_to_create:
            ...
            for arr in pool.map(
                partial(process_script, print_logs=print_logs),
        ...
        if "TestPlaybooks" in objects_to_create:
            ...
            for pair in pool.map(
                partial(process_test_playbook_path, print_logs=print_logs),
        ...
        if "Classifiers" in objects_to_create:
            ...
            for arr in pool.map(
                partial(process_classifier, print_logs=print_logs),
        ...
        if "Dashboards" in objects_to_create:
            ...
            for arr in pool.map(
                partial(process_dashboards, print_logs=print_logs),
        ...
        if "IncidentFields" in objects_to_create:
            ...
            for arr in pool.map(
                partial(process_incident_fields, print_logs=print_logs),
        ...
        if "IncidentTypes" in objects_to_create:
            ...
            for arr in pool.map(
                partial(process_incident_types, print_logs=print_logs),
        ...
        if "IndicatorFields" in objects_to_create:
            ...
            for arr in pool.map(
                partial(process_indicator_fields, print_logs=print_logs),
        ...
        if "Layouts" in objects_to_create:
            ...
            for arr in pool.map(
                partial(process_layouts, print_logs=print_logs),
        ...
        if "Reports" in objects_to_create:
            ...
            for arr in pool.map(
                partial(process_reports, print_logs=print_logs),
        ...
        if "Widgets" in objects_to_create:
            ...
            for arr in pool.map(
                partial(process_widgets, print_logs=print_logs),
...
def find_duplicates(id_set, print_logs):
    ...
    for field_to_check in field_ids:
        if has_duplicate(
            fields, field_to_check, "Indicator and Incident Fields", print_logs
        ):
...
def has_duplicate(id_set, id_to_check, object_type=None, print_logs=True):
    ...
    for dup1, dup2 in itertools.combinations(duplicates, 2):
        ...
        if print_logs and dict1["name"] != dict2["name"]:
            print_warning(
                "The following {} have the same ID ({}) but different names: "
                '"{}", "{}".'.format(
                    object_type, id_to_check, dict1["name"], dict2["name"]
                )
            )
...
def update_id_set():
    ...
    added_files, modified_files, added_scripts, modified_scripts = get_changed_files(
        files_string + "\n" + second_files_string
    )
            ...
            try:
                ...
                if "Expecting property name" in str(ex):
                    ...
                    with open("./Tests/id_set.json", "r") as id_set_file_from_master:
                        ids_dict = json.load(
                            id_set_file_from_master, object_pairs_hook=OrderedDict
                        )
                else:
                    raise
    ...
    if added_files:
        for file_path in added_files:
            if re.match(INTEGRATION_REGEX, file_path, re.IGNORECASE) or re.match(
            ...
            if re.match(PLAYBOOK_REGEX, file_path, re.IGNORECASE):
                add_new_object_to_id_set(
                    collect_ids(file_path), get_playbook_data(file_path), playbook_set
                )
        ...
        for file_path in modified_files:
            if re.match(INTEGRATION_REGEX, file_path, re.IGNORECASE) or re.match(
                ...
                update_object_in_id_set(
                    id_, integration_data, file_path, integration_set
                )
            ...
            if re.match(TEST_PLAYBOOK_REGEX, file_path, re.IGNORECASE):
                ...
                playbook_data = get_playbook_data(file_path)
                update_object_in_id_set(
                    id_, playbook_data, file_path, test_playbook_set
                )
                print("updated {} in id_set".format(id_))
    ...
    if added_scripts:
        for added_script_package in added_scripts:
            ...
            add_new_object_to_id_set(
                ...
                get_script_data(yml_path, script_code=code),
                script_set,
            )
