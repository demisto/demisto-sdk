...
class Neo4jContentGraphInterface(ContentGraphInterface):
    ...
    def _get_nodes_set_from_result(
        self,
        result: List[Neo4jResult],
        pack_nodes: Set[graph.Node],
        content_items_nodes: Set[graph.Node],
    ) -> Set[graph.Node]:
        """
    ...
    def _add_all_level_dependencies(
        self, session: Session, marketplace: MarketplaceVersions, pack_nodes
    ):
        ...
        nodes_set = self._get_nodes_set_from_result(
            mandatorily_dependencies, set(), content_items_nodes
        )
    ...
    def _search(
        ...
        **properties,
        ...
        with self.driver.session() as session:
            ...
            nodes_set = self._get_nodes_set_from_result(
                result, pack_nodes, content_items_nodes
            )
            self._add_to_mapping(nodes_set)
