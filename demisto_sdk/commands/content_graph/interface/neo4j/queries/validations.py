...
def get_supported_modules_mismatch_dependencies(
    ...
    WHERE ({content_item_ids} IS NULL OR size({content_item_ids}) = 0 OR contentItemA.object_id IN {content_item_ids})
      ...
      AND NOT ALL(module IN contentItemA.supportedModules WHERE module IN coalesce(contentItemB.supportedModules,{[sm.value for sm in PlatformSupportedModules]}))
    RETURN collect(contentItemB) AS nodes_to
    """
    return {
