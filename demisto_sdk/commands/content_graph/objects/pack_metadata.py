...
class PackMetadata(BaseModel):
    ...
    _default_data_source_id: Optional[str]
    _default_data_source_name: Optional[str]
    ...
    def _format_metadata(
        ...
        if self._default_data_source_name and self._default_data_source_id:
            # in order to show only for XSIAM packs
            _metadata.update(
                {
                    "defaultDataSource": {
                        "name": self._default_data_source_name,
                        "id": self._default_data_source_id,
                    },
                }
            )
    ...
    def _get_content_items_and_displays_metadata(
        ...
        if (
            ...
            and self._default_data_source_name
    ...
    def is_data_source(self, content_items: PackContentItems) -> bool:
        ...
        if self._default_data_source_id and self._default_data_source_name:
            return True
    ...
    def _set_default_data_source(self, content_items: PackContentItems) -> None:
        ...
        if self._default_data_source_id and self._default_data_source_id in [
            data_source.get("id") for data_source in data_sources
        ]:
            # the provided _default_data_source_id is of a valid integration, keep it
            self._default_data_source_name = [
                data_source.get("name")
                for data_source in data_sources
                if data_source.get("id") == self._default_data_source_id
            ][0]
            logger.info(
                f"Keeping the provided {self._default_data_source_id=} with {self._default_data_source_name=}"
            )
            return
        ...
        logger.info(
            f"No defaultDataSource provided ({self._default_data_source_id}) or it is not a valid data source,"
