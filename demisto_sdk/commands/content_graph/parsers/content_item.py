...
class ContentItemParser(BaseContentParser, metaclass=ParserMetaclass):
    ...
    def __init__(
        ...
        self.pack_supported_modules: Optional[List[str]] = pack_supported_modules
        super().__init__(path)
    ...
    def _resolve_pack_defaults(
        ...
        """Resolve the pack's marketplaces and supported modules from the item's `pack_metadata.json`. Falls back to all marketplaces when not in a pack,
