...
def test_pack_metadata_xsoar(repo: Repo, tmp_path: Path, mocker):
    ...
    pack.create_integration(name="TestIntegration", yml={
        "script": {
            "commands": [
                {
                    "name": "test-command1",
    ...
    pack.create_script(name="TestScript", yml={"tags": ["transformer", "filter"], "fromversion": "6.5.0"})
    ...
    assert metadata.get("description") == "This is the Hello World integration for getting started."
    ...
    assert metadata.get("supportDetails", {}).get("url") == "https://www.paloaltonetworks.com/cortex"
    assert metadata.get("supportDetails", {}).get("email") == ""
    assert metadata.get("eulaLink") == "https://github.com/demisto/content/blob/master/LICENSE"
    ...
    assert metadata.get("contentItems", {}).get("integration", [{}])[0] == expected_integration_summary
