...
class ClassifierSixConverter(ClassifierBaseConverter):
    ...
    def create_classifier_from_old_classifier(
        ...
        new_classifier = dict(
            new_classifier,
            type='classification',
            name=f'{classifier_name_and_id} - Classifier',
            description='',
            fromVersion='6.0.0',
    ...
    def create_mapper_from_old_classifier(
        self, old_classifier: Classifier
    ) -> None:
        """
        ...
        if not classifier_name_and_id or not mapping:
            return
        mapper = dict(
            id=f'{classifier_name_and_id}-mapper',
            name=f'{classifier_name_and_id} - Incoming Mapper',
    ...
    def calculate_new_path(
        self, old_classifier_brand: str, is_mapper: bool
    ) -> str:
        """
        ...
        """
        fixed_brand_name = self.entity_separators_to_underscore(
