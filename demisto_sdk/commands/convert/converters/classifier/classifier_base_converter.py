...
class ClassifierBaseConverter(BaseConverter):
    ...
    def get_classifiers_schema_intersection_fields(self) -> Set[str]:
        ...
        """
        first_schema_data: dict = get_yaml(
            self.CLASSIFIER_UP_TO_5_9_9_SCHEMA_PATH
        ).get('mapping', dict())
        second_schema_data: dict = get_yaml(
            self.CLASSIFIER_6_0_0_SCHEMA_PATH
        ).get('mapping', dict())
        intersecting_fields = (
            first_schema_data.keys() & second_schema_data.keys()
        )
        return {
            field
