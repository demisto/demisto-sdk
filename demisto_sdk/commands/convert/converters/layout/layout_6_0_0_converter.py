...
class LayoutSixConverter(LayoutBaseConverter):
        ...
        for (
            ...
            for old_layout in old_corresponding_layouts:
                ...
                if not layout_kind:
                    continue
                layout_kind = (
                    'detailsV2' if layout_kind == 'details' else layout_kind
                )
    ...
    def group_layouts_needing_conversion_by_layout_id(
        ...
        for layout in self.get_entities_by_entity_type(
            ...
            layout_id_to_layouts_dict[
                layout_id
            ] = layout_id_to_layouts_dict.get(layout_id, []) + [layout]
        return layout_id_to_layouts_dict

    def calculate_new_layout_group(
        self, old_layouts: List[LayoutObject]
    ) -> str:
        """
        ...
        is_group_indicator = any(
            layout.get('kind') in self.layout_indicator_fields
    ...
    def calculate_new_layout_relative_path(self, layout_id: str) -> str:
        ...
        layout_file_name = (
            f'{FileType.LAYOUTS_CONTAINER.value}-{layout_base_name}.json'
        )
    ...
    def update_incident_and_indicator_types_related_to_old_layouts(
        self, old_layouts: List[LayoutObject], layout_id: str
    ) -> None:
        """
        ...
        bounded_incident_types = [
            incident_type
            for incident_type in self.pack.incident_types
            if incident_type.get('id') in old_layouts_type_ids
        ]
        bounded_indicator_types = [
            indicator_type
            for indicator_type in self.pack.indicator_types
