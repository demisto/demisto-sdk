...
class LayoutBelowSixConverter(LayoutBaseConverter):
    ...
    def convert_dir(self):
        ...
        for layout_to_convert in layouts_to_be_converted:
            layout_to_create = self.create_layout_dict(from_version='4.1.0', to_version='5.9.9', type_id='TODO',
                                                       kind='TODO')
    ...
    @staticmethod
    def get_layouts_needing_conversion(layouts: List[LayoutObject]):
        """
        Receives list of all Layouts of the given path, returns all layouts above
        Args:
            layouts:
        ...
        """
        return [layout for layout in layouts if layout.type() == FileType.LAYOUTS_CONTAINER and layout.layout_id()]
...
class LayoutConverter:
    ...
    def build_pack_layouts_object(self, input_pack: str, pack_layouts_tempdir_path: str) -> dict:
        ...
        for file_path in files:
            ...
            if find_type(path=file_path, _dict=file_data, file_type='json') in [FileType.LAYOUT,
                ...
                layout_id: str = self.layout_id(file_data, layout_version)
    ...
    @staticmethod
    def layout_id(layout_data: dict, layout_version: str):
