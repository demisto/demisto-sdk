...
class LayoutBelowSixConverter(LayoutBaseConverter):
    ...
    def convert_dir(self) -> int:
        ...
        """
        layout_id_to_incident_type = (
            self.layout_to_indicators_or_incidents_dict(
                self.pack.incident_types
            )
        )
        layout_id_to_indicators_dict = (
            self.layout_to_indicators_or_incidents_dict(
    ...
    def build_old_layout(
        self,
        layout_id: str,
        type_id: str,
        dynamic_field_key: str,
        dynamic_field_value: Any,
        from_version: str,
    ):
        """
        ...
        new_layout_dict = self.create_layout_dict(
            ...
            to_version='5.9.9',
            type_id=type_id,
            kind=dynamic_field_key,
        )
        new_layout_dict['layout'] = dict(
            id=layout_id,
            name=layout_id,
            version=-1,
            kind=dynamic_field_key,
    ...
    def calculate_from_version(
        ...
        layout_id: str,
        layout_kind: str,
        current_old_layouts: List[LayoutObject],
    ) -> str:
        """
