...
class TestLayoutSixConverter:
    ...
    def test_group_layouts_needing_conversion_by_layout_id(self, tmpdir):
        ...
        result = (
            layout_converter.group_layouts_needing_conversion_by_layout_id()
        )
        assert len(result) == 1 and 'ExtraHop Detection' in result
        layout_kinds = {
            layout['kind'] for layout in result['ExtraHop Detection']
        }
        assert all(
    ...
    def test_calculate_new_layout_relative_path(
        ...
        assert (
            layout_converter.calculate_new_layout_relative_path(layout_id)
            == expected
        )
