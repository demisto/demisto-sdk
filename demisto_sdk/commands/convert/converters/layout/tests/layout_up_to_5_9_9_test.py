...
class TestLayoutBelowSixConverter:
    ...
    def test_layout_to_incidents_dict(self, tmpdir):
        ...
        result = (
            LayoutBelowSixConverter.layout_to_indicators_or_incidents_dict(
    ...
    @pytest.mark.parametrize(
        'layout_id, incident_type_id, dynamic_field_key, dynamic_field_value, expected',
        BUILD_OLD_LAYOUT_INPUTS,
    )
    def test_build_old_layout(
        self,
        tmpdir,
        layout_id: str,
        incident_type_id: str,
        dynamic_field_key: str,
        dynamic_field_value: str,
        expected: Dict,
    ):
        """
        Given:
        ...
        """
        assert (
            LayoutBelowSixConverter(Pack(tmpdir)).build_old_layout(
                layout_id,
                incident_type_id,
                dynamic_field_key,
                dynamic_field_value,
    ...
    def test_calculate_from_version(
        ...
        assert (
            LayoutBelowSixConverter(Pack(tmpdir)).calculate_from_version(
                layout_id, layout_kind, [Layout(full_layout_path)]
            )
            == expected
        )
