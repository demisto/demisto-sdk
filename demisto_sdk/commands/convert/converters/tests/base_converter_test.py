...
class TestBaseConverter:
    ...
    def test_get_layouts_by_layout_container_type(self, tmpdir):
        ...
        assert all(
            layout.type() == FileType.LAYOUTS_CONTAINER for layout in layouts
        )
    ...
    def test_get_layouts_by_layout_type(self, tmpdir):
        ...
        fake_pack.create_layout(
            'quickView-ExtraHop_Detection',
            util_load_json(self.LAYOUT_QUICK_VIEW_PATH),
        )
        fake_pack.create_layout(
            'mobile-ExtraHop_Detection',
    ...
    ENTITY_SEPARATORS_TO_UNDERSCORE_INPUTS = [
        ('abcde', 'abcde'),
        ('axzjd-frl', 'axzjd_frl'),
        (
    ...
    @pytest.mark.parametrize(
        'name, expected', ENTITY_SEPARATORS_TO_UNDERSCORE_INPUTS
    )
    def test_entity_separators_to_underscore(
        self, tmpdir, name: str, expected: str
    ):
        """
        Given:
        ...
        assert (
            layout_converter.entity_separators_to_underscore(name) == expected
        )
