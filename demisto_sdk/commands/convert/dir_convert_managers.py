...
class AbstractDirConvertManager:
    ...
    def should_convert(self) -> bool:
        ...
        """
        return self.is_dir_convert_manager_path() or is_pack_path(
            self.input_path
        )
...
class LayoutsDirConvertManager(AbstractDirConvertManager):
    def __init__(self, pack: Pack, input_path: str, server_version: Version):
        super().__init__(
            pack, input_path, server_version, entity_dir_name='Layouts'
        )
...
class ClassifiersDirConvertManager(AbstractDirConvertManager):
    ...
    def convert(self) -> int:
        if self.server_version >= self.VERSION_6_0_0:
            classifier_converter: ClassifierBaseConverter = (
                ClassifierSixConverter(self.pack)
            )
            ...
            raise NotImplementedError(
                'Version requested to convert is not supported.'
            )
    ...
    def should_convert(self) -> bool:
        ...
        """
        return (
            self.server_version >= self.VERSION_6_0_0
            and super().should_convert()
        )
