...
class CoverageReport:
    def __init__(
        ...
        coverage_file: Optional[str] = os.path.join("coverage_report", ".coverage"),
    ...
    def coverage_diff_report(self) -> bool:
        ...
        for file_name, cov_precents in self.files.items():
            ...
            if min_cov > cov_precents:
                logger.error(
                    f"[red]Unit-tests for '{file_name}' must reach a coverage of at least {min_cov} percents "
                    f"(currently: {cov_precents} percents).[/red]"
                )
                coverage_ok = False
        return coverage_ok
