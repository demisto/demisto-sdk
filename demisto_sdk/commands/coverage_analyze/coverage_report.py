...
class CoverageReport:
    def __init__(
        self,
        default_min_coverage: float = 70.0,
        allowed_coverage_degradation_percentage: float = 1.0,
        coverage_file: Optional[str] = os.path.join(
            'coverage_report', '.coverage'
        ),
        no_cache: Optional[bool] = False,
        report_dir: str = 'coverage_report',
        ...
        cache_dir = (
            str(os.path.join(self.report_dir, 'cache'))
            if not no_cache
            else None
        )
    ...
    def coverage_report(self):
        ...
        if 'xml' in self.report_types:
            export_report(
                self.cov.xml_report, 'xml', self.cov.config.xml_output
            )
        if 'json' in self.report_types:
            export_report(
                self.cov.json_report, 'json', self.cov.config.json_output
            )
        if 'json-min' in self.report_types:
            json_min_dir = os.path.join(self.report_dir, 'coverage-min.json')
            logger.info(
                f'exporting json-min coverage report to {json_min_dir}'
            )
