...
class TestCoverageReport:
    ...
    def test_original_summary(self, tmpdir, monkeypatch, requests_mock):
        ...
        monkeypatch.chdir(tmpdir)
        assert (
            CoverageReport().original_summary
            == read_file(JSON_MIN_DATA_FILE)['files']
        )
...
class TestFileMinCoverage:
    ...
    @pytest.mark.parametrize(
        'file_path, default_min_cover', data_test_with_new_file
    )
    def test_with_new_file(
        self, file_path, default_min_cover, tmpdir, monkeypatch
    ):
        monkeypatch.chdir(tmpdir)
    ...
    @pytest.mark.parametrize(
        'file_path, current_cover, expected_min_cover',
        data_test_with_exist_file,
    )
    ...
    @pytest.mark.parametrize(
        'file_path, epsilon, expected_min_cover',
        data_test_with_custom_epsilon_file,
    )
...
class TestCoverageDiffReport:
    ...
    def test_with_degradated_files(self, caplog, tmpdir, monkeypatch, mocker):
        ...
        cov_report._report_str = Path(REPORT_STR_FILE).read_text()
        mocker.patch.object(
            cov_report, 'file_min_coverage', return_value=100.0
        )
        with caplog.at_level(logging.ERROR, logger='demisto-sdk'):
