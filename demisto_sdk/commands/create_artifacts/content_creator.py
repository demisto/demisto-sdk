...
class ContentCreator:
    def __init__(
        ...
        self.artifacts_path = (
            artifacts_path if artifacts_path else "/home/circleci/project/artifacts"
        )
        ...
        self.content_bundle = (
            ...
            else os.path.join(self.artifacts_path, "bundle_content")
        )
        self.test_bundle = (
            test_bundle_path
            if test_bundle_path
            else os.path.join(self.artifacts_path, "bundle_test")
        )
        self.packs_bundle = (
            packs_bundle_path
            if packs_bundle_path
            else os.path.join(self.artifacts_path, "bundle_packs")
        )
    ...
    def run(self):
        ...
        if self.long_file_names:
            print_error(
                f"The following files exceeded to file name length limit of {self.file_name_max_size}:\n"
    ...
    def create_unifieds_and_copy(self, package_dir, dest_dir="", skip_dest_dir=""):
        ...
        for package in scanned_packages:
            ...
            if any(
                package_to_skip in package for package_to_skip in self.packages_to_skip
            ):
    ...
    def add_tools_to_bundle(bundle):
        for directory in glob.glob(os.path.join(TOOLS_DIR, "*")):
            zipf = zipfile.ZipFile(
                os.path.join(bundle, f"tools-{os.path.basename(directory)}.zip"),
                "w",
    ...
    def copy_content_yml(path, out_path, yml_info):
        ...
        if parent_dir_name in DIR_TO_PREFIX and not os.path.basename(path).startswith(
            "playbook-"
        ):
            script_obj = yml_info
    ...
    def copy_dir_yml(self, dir_path, bundle):
        ...
        for path in scan_files:
            ...
            if dir_name in ["Playbooks", "TestPlaybooks"]:
                ...
                if dir_name == "TestPlaybooks" and os.path.basename(path).startswith(
                    "script-"
                ):
                    self.copy_content_yml(
                        path, os.path.join(bundle, os.path.basename(path)), yml_info
                    )
                self.copy_playbook_yml(
                    path, os.path.join(bundle, os.path.basename(path))
                )
            else:
                self.copy_content_yml(
                    path, os.path.join(bundle, os.path.basename(path)), yml_info
    ...
    def copy_dir_json(self, dir_path, bundle):
        ...
        for path in scan_files:
            ...
            new_path = dpath
            if dir_name == "IndicatorFields" and not dpath.startswith(
        ...
        for path in scan_files:
            if os.path.isdir(path):
                ...
                for new_path in non_circle_tests:
                    ...
                    shutil.copyfile(
                        new_path,
                        os.path.join(self.test_bundle, os.path.basename(new_path)),
                    )
    ...
    def copy_packs_content_to_packs_bundle(self, packs):
        ...
        for pack in packs:
            ...
            for file_path in pack_files:
                shutil.copy(
                    file_path, os.path.join(pack_dst, os.path.basename(file_path))
                )
                ...
                if dir_name in DIR_TO_PREFIX:
                    ...
                    if packages_dirs:  # split yml files directories
                        for package_dir in packages_dirs:
                            ...
                            if not ymls or (
                                len(ymls) == 1 and ymls[0].endswith("_unified.yml")
                            ):
                                msg = f"Skipping package: {package_dir} -"
                                if not ymls:
                                    print_warning(
                                        "{} No yml files found in the package directory".format(
                                            msg
                                        )
                                    )
                                else:
                                    print_warning(
                                        "{} Only unified yml found in the package directory".format(
                                            msg
                                        )
                                    )
                                continue
    ...
    def create_content(self):
        ...
        try:
            ...
            shutil.copyfile(
                "./Tests/id_set.json", os.path.join(self.artifacts_path, "id_set.json")
            )
            if os.path.exists("release-notes.md"):
                print(
                    'copying release-notes.md to artifacts directory "{}"'.format(
                        self.artifacts_path
                    )
                )
                shutil.copyfile(
            ...
            if os.path.exists("beta-release-notes.md"):
                print(
                    'copying beta-release-notes.md to artifacts directory "{}"'.format(
                        self.artifacts_path
                    )
                )
                shutil.copyfile(
                    "beta-release-notes.md",
                    os.path.join(self.artifacts_path, "beta-release-notes.md"),
                )
            else:
                print_warning(
                    "beta-release-notes.md was not found in the content directory and therefore not "
                    "copied over to the artifacts directory"
                )
            print(
