...
def test_use_alternative_fields(mock_single_pack_git):
    ...
    with temp_dir() as temp:
        ...
        assert exit_code == 0
        assert same_folders(
            temp, ARTIFACTS_EXPECTED_RESULTS / "content_with_alternative_fields"
        )
        ...
        integration_yml = dict(
            yaml.load(
                PosixPath(pack_path, "Integrations", "integration-sample_packs.yml")
            )
        )
        ...
        # Check Script
        script_yml = dict(
            yaml.load(PosixPath(pack_path, "Scripts", "script-sample_packs.yml"))
        )
        ...
        # Check Playbook
        playbook_yml = dict(
            yaml.load(PosixPath(pack_path, "Playbooks", "playbook-sample_packs.yml"))
        )
        assert not any(key for key in playbook_yml if key.endswith("_x2"))
        assert playbook_yml["name"] == "name_x2"
        assert (
            playbook_yml["tasks"]["task_num"]["task"]["scriptName"] == "scriptName_x2"
        )

        # Check IncidentField
        with open(
            os.path.join(pack_path, "IncidentFields", "incidentfield-sample_packs.json")
        ) as json_file:
