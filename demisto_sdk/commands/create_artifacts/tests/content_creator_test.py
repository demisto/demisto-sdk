...
class TestContentCreator:
    def setup(self):
        ...
        self.scripts_full_path = os.path.join(
            tests_dir, "test_files", "content_repo_example", "Scripts"
        )
        self.integrations_full_path = os.path.join(
            tests_dir, "test_files", "content_repo_example", "Integrations"
        )
        self.TestPlaybooks_full_path = os.path.join(
            tests_dir, "test_files", "content_repo_example", "TestPlaybooks"
        )
        self.Packs_full_path = os.path.join(
            tests_dir, "test_files", "content_repo_example", "Packs"
        )
        self._bundle_dir = mkdtemp()
        self._test_dir = mkdtemp()
        self.content_repo = os.path.join(
            tests_dir, "test_files", "content_repo_example"
        )
    ...
    def test_copy_dir_files(self):
        ...
        content_creator = ContentCreator(
            artifacts_path=self.content_repo,
        ...
        content_creator.copy_dir_files(
            self.scripts_full_path, content_creator.content_bundle
        )
        assert filecmp.cmp(
            f"{self.scripts_full_path}/script-Sleep.yml",
            f"{self._bundle_dir}/script-Sleep.yml",
        )

        # test Integrations repo copy
        content_creator.copy_dir_files(
            f"{self.integrations_full_path}/Securonix", content_creator.content_bundle
        )
        print(f"{self._bundle_dir}/Panorama.yml")
        assert filecmp.cmp(
            f"{self.integrations_full_path}/Securonix/Securonix_unified.yml",
            f"{self._bundle_dir}/Securonix_unified.yml",
        )

        # test TestPlaybooks repo copy
        content_creator.copy_dir_files(
            self.TestPlaybooks_full_path, content_creator.test_bundle
        )
        assert filecmp.cmp(
            f"{self.TestPlaybooks_full_path}/script-Sleep-for-testplaybook.yml",
    ...
    def test_copy_packs_content_to_packs_bundle(self):
        ...
        content_creator.copy_packs_content_to_old_bundles(
            [f"{self.Packs_full_path}/FeedAzure"]
        )

        # test Packs repo, TestPlaybooks repo copy without playbook- prefix
        assert filecmp.cmp(
            f"{self.Packs_full_path}/FeedAzure/TestPlaybooks/FeedAzure_test.yml",
            f"{self._test_dir}/playbook-FeedAzure_test.yml",
        )
        assert filecmp.cmp(
            f"{self.Packs_full_path}/FeedAzure/TestPlaybooks/playbook-FeedAzure_test_copy_no_prefix.yml",
        ...
        assert filecmp.cmp(
            f"{self.Packs_full_path}/FeedAzure/TestPlaybooks/just_a_test_script.yml",
            f"{self._test_dir}/script-just_a_test_script.yml",
        )
        assert filecmp.cmp(
            f"{self.Packs_full_path}/FeedAzure/TestPlaybooks/script-prefixed_automation.yml",
