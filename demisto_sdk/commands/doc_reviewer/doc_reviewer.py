...
class DocReviewer:
    ...
    def print_unknown_words(unknown_words):
        for word, corrections in unknown_words.items():
            if corrections:
                logger.info(f"[red]  - {word} - did you mean: {corrections}[/red]")
            else:
                logger.info(f"[red]  - {word}[/red]")
        logger.info(
    ...
    def print_file_report(self):
        if self.files_without_misspells:
            ...
            logger.info("[green]" + no_misspells_string + "[/green]")
        ...
        if self.files_with_misspells:
            ...
            logger.info("[red]" + misspells_string + "[/red]")
