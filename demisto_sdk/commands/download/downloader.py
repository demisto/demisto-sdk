...
class Downloader:
    ...
    def handle_file(self, string_to_write: str, member_name: str, scripts_id_name: dict):
        ...
        try:
            ...
            for member in tar.getmembers():
                ...
                if extracted_file:
                    ...
                    if "automation-" in member.name.lower() or "integration-" in member.name.lower():
    ...
    def handle_list_files_flag(self) -> bool:
        ...
        if self.list_files:
            ...
            list_files = [[cco["name"], cco["type"]] for cco in self.all_custom_content_objects if cco.get("name")]
            print_color(
    ...
    def build_custom_content(self) -> None:
        ...
        custom_content_objects = (
            self.all_custom_content_objects if self.all_custom_content_objects else self.get_custom_content_objects()
        )
        ...
        print_color(
            f"Demisto instance: Receiving objects: 100% ({number_of_files}/{number_of_files})," f" done.\n",
            LOG_COLORS.NATIVE,
        )
    ...
    def file_type_to_entity(file_data: dict, file_type: str) -> str:
        ...
        if file_type and file_type == "playbook":
            ...
            if name.endswith(("Test", "_test", "_Test", "-test", "-Test")) or name.lower().startswith("test"):
    ...
    def update_data(file_path_to_write: str, file_path_to_read: str, file_ending: str) -> None:
        ...
        preserved_data: dict = unflatten(
            {field: dictor(pack_obj_data, field) for field in fields if dictor(pack_obj_data, field)},
            splitter="dot",
        )
