...
class Environment:
    ...
    def setup(self):
        ...
        shutil.copytree(
            src='demisto_sdk/commands/download/tests/tests_env',
        ...
        self.CUSTOM_CONTENT_BASE_PATH = (
            f'{tests_path}/tests_data/custom_content'
        )
        ...
        self.SCRIPT_INSTANCE_PATH = (
            f'{self.PACK_INSTANCE_PATH}/Scripts/TestScript'
        )
        ...
        self.LISTS_INSTANCE_PATH = (
            f'{self.PACK_INSTANCE_PATH}/Lists/list-dummy.json'
        )
        self.JOBS_INSTANCE_PATH = (
            f'{self.PACK_INSTANCE_PATH}/Jobs/job-sample.json'
        )
...
class TestHelperMethods:
    ...
    def test_verify_code_lang(
        self, code_lang, file_type, file_name, err_msg, output
    ):
        with patch.object(Downloader, '__init__', lambda a, b, c: None):
            downloader = Downloader('', '')
            downloader.files_not_downloaded = []
            assert (
                downloader.verify_code_lang(code_lang, file_type, file_name)
                is output
            )
            if not output:
...
class TestFlagHandlers:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                None,
                False,
                'The item type option is just for downloading system '
                'items.',
            ),
            (
                True,
    ...
    def test_handle_all_custom_content_flag(self, tmp_path):
        ...
        with patch.object(Downloader, '__init__', lambda a, b, c: None):
            ...
            assert ordered(custom_content_names) == ordered(
                downloader.input_files
            )
    ...
    def test_handle_list_files_flag(self, capsys, tmp_path):
        ...
        with patch.object(Downloader, '__init__', lambda a, b, c: None):
            ...
            stdout, _ = capsys.readouterr()
            list_files = [
                [cco['name'], cco['type']] for cco in env.CUSTOM_CONTENT
            ]
...
class TestBuildPackContent:
    def test_build_pack_content(self, tmp_path):
        env = Environment(tmp_path)
        downloader = Downloader(
            output=env.PACK_INSTANCE_PATH, input='', regex=''
        )
...
class TestBuildCustomContent:
    def test_exist_in_pack_content(self, tmp_path):
        ...
        with patch.object(Downloader, '__init__', lambda a, b, c: None):
            ...
            for param in parameters:
                assert (
                    downloader.exist_in_pack_content(
                ...
                for path_part in dotted_path_list:
                    if path_part != dotted_path_list[-1]:
                        ...
                        if obj.get(path_part):
                            ...
                            assert False
            with open(
                env.INTEGRATION_PACK_OBJECT['Test Integration'][5]['path'], 'r'
            ) as description_file:
                description_data = description_file.read()
            assert 'Test Integration Long Description TEST' in description_data
            with open(
                env.INTEGRATION_PACK_OBJECT['Test Integration'][0]['path'], 'r'
            ) as code_file:
    ...
    def test_merge_existing_file(self, tmp_path):
        ...
        with patch.object(Downloader, '__init__', lambda a, b, c: None):
            ...
            for param in parameters:
                ...
                file_data = param['method'](
                    param['instance_path'], cache_clear=True
                )
        ...
        parameters = [
            {
                ...
                'pack_file_object': env.INTEGRATION_PACK_OBJECT[
                    'Test Integration'
                ][2],
            },
            {
...
class TestMergeNewFile:
    ...
    def test_merge_new_file(self, tmp_path):
        ...
        for param in parameters:
            ...
            new_file_path = (
                f'{output_dir_path}/{os.path.basename(old_file_path)}'
            )
...
class TestVerifyPackPath:
    ...
    def test_verify_output_path_is_pack(
        self, tmp_path, output_path, valid_ans
    ):
        env = Environment(tmp_path)
        downloader = Downloader(
...
def test_build_req_params(
    ...
    with patch.object(Downloader, '__init__', lambda x, y, z: None):
        monkeypatch.setenv(
            'DEMISTO_BASE_URL', 'http://demisto.instance.com:8080/'
        )
