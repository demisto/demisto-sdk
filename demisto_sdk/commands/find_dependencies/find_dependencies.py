...
from demisto_sdk.commands.create_id_set.create_id_set import (
    IDSetCreator,
    get_id_set,
)
...
def parse_for_pack_metadata(
    ...
    for dependency_id, additional_data in parsed_dependency_graph:
        ...
        if not complete_data:
            ...
            if dependency_data:
                ...
                additional_data['minVersion'] = dependency_data[
                    'current_version'
                ]
                additional_data['certification'] = dependency_data[
                    'certification'
                ]
            else:
                additional_data['display_name'] = pack_name
    ...
    if verbose:
        click.secho(
            f'All level dependencies are: {all_level_dependencies}', fg='white'
        )
...
def find_pack_display_name(pack_folder_name: str) -> str:
    ...
    pack_display_name = (
        ...
        if pack_metadata.get('name')
        else pack_folder_name
    )
...
def update_pack_metadata_with_dependencies(
    ...
    with open(pack_metadata_path, 'r+') as pack_metadata_file:
        pack_metadata = json.load(pack_metadata_file)
        pack_metadata = (
            {} if not isinstance(pack_metadata, dict) else pack_metadata
        )
        pack_metadata['dependencies'] = first_level_dependencies
        pack_metadata['displayedImages'] = list(
            first_level_dependencies.keys()
        )
...
class PackDependencies:
    ...
    def _search_for_pack_items(pack_id: str, items_list: list) -> list:
        ...
        return list(
            filter(
                lambda s: next(iter(s.values())).get('pack') == pack_id,
                items_list,
            )
        )
    ...
    def _should_add_item_as_dependency(
        ...
        is_relevant_pack = bool(
            ...
            and (
                ...
                or id_set_item_pack
                not in constants.IGNORED_DEPENDENCY_CALCULATION
            )
        )

        is_marketplace_match = (
            not marketplace
            or marketplace in item_details.get('marketplaces', [])
        )
    ...
    def _search_packs_by_integration_command(
        ...
        return {
            p
            for p in pack_names
            if p not in constants.IGNORED_DEPENDENCY_CALCULATION
    ...
    def _detect_generic_commands_dependencies(pack_ids: set) -> list:
        ...
        """
        return [
            (p, False) if len(pack_ids) > 1 else (p, True) for p in pack_ids
        ]
    ...
    def _update_optional_commontypes_pack_dependencies(
        ...
        mandatory_packs = (
            packs_found_from_incident_fields_or_types.intersection(
    ...
    def _collect_scripts_dependencies(
        ...
        for script_mapping in pack_scripts:
            ...
            for command in dependencies_commands:
                ...
                if (
                    pack_names
                ):  # found script dependency implementing pack name
                    pack_dependencies_data = (
                        PackDependencies._label_as_mandatory(pack_names)
                    )
    ...
    def _collect_layouts_dependencies(
        ...
        for layout in pack_layouts:
            ...
            related_fields = layout_data.get(
    ...
    def _collect_classifiers_dependencies(
        ...
        for classifier in pack_classifiers:
            ...
            related_scripts = classifier_data.get(
                'filters', []
            ) + classifier_data.get('transformers', [])
            (
                packs_found_from_scripts,
                packs_and_scripts_dict,
            ) = PackDependencies._search_packs_by_items_names_or_ids(
    ...
    def _collect_generic_modules_dependencies(
        ...
        for generic_module in pack_generic_modules:
            ...
            for view in related_views:
                related_dashboards = related_views.get(view, {}).get(
                    'dashboards', []
                )
                (
    ...
    def build_all_dependencies_graph(
        ...
        for pack in pack_ids:
            ...
            for (
                dependent_item,
                items_depending_on_item,
            ) in dependencies_items.items():
                for (
                    ...
                    for item_dependent_on in items_dependent_on:
                        ...
                        if (
                            ...
                            dependency_graph.nodes()[
                                pack_of_item_dependent_on
                            ]['mandatory_for_items'].setdefault(
                                item_dependent_on, {}
                            ).update(
                                {pack: [dependent_item]}
                            )
            ...
            dependency_graph.nodes()[pack]['depending_on_packs'] = list(
                dependencies
            )
    ...
    def build_dependency_graph_single_pack(
        ...
        while found_new_dependencies:
            ...
            found_new_dependencies = (
                graph.number_of_nodes() > current_number_of_nodes
            )
    ...
    def find_dependencies_manager(
        ...
        if get_dependent_on:
            dependent_packs, _ = get_packs_dependent_on_given_packs(
                input_paths,
                id_set_path,  # type: ignore[arg-type]
                output_path,
                verbose,
            )
    ...
    def find_dependencies(
        ...
        if not silent_mode:
            ...
            click.echo(
                click.style(
                    f'Found dependencies result for {pack_name} pack:',
                    bold=True,
                )
            )
    ...
    @staticmethod
    def update_dependencies_from_pack_metadata(
        pack_name, first_level_dependencies
    ):
        """
        ...
        """
        pack_meta_file_content = PackDependencies.get_metadata_from_pack(
            pack_name
        )
            ...
            for item, dependent_item_and_pack in pack_graph_node.get(
                ...
                for dep_pack, dep_item in dependent_item_and_pack.items():
                    if dep_pack == man_pack:
                        ...
                        if first_level_dependencies[man_pack].get(
                            'dependent_items'
                        ):
...
def calculate_single_pack_dependencies(
    ...
    try:
        ...
        for dependency_pack, additional_data in subgraph.nodes(data=True):
            if verbose:
                print(
                    f'Iterating dependency {dependency_pack} for pack {pack}'
                )
        ...
        (
            first_level_dependencies,
            all_level_dependencies,
        ) = parse_for_pack_metadata(subgraph, pack)
    except Exception:
...
def calculate_all_packs_dependencies(
    ...
    with ProcessPoolHandler() as pool:
        ...
        wait_futures_complete(
            futures=futures, done_fn=add_pack_metadata_results
        )
        print(
        ...
        with open(output_path, 'w') as pack_dependencies_file:
            json.dump(
                pack_dependencies_result, pack_dependencies_file, indent=4
            )
...
def get_packs_dependent_on_given_packs(
    ...
    with ProcessPoolHandler() as pool:
        ...
        # finished iteration over pack folders
        print_success(
            'Finished calculating the dependencies on the given packs.'
        )

        if output_path:
            with open(output_path, 'w') as pack_dependencies_file:
                json.dump(
                    dependent_on_results, pack_dependencies_file, indent=4
                )
...
def remove_unmandatory_items(
    packs_and_items_dict, pack_dependencies_data
) -> None:
    """
...
def remove_dependencies_from_id_set(
    ...
    save_dict_of_sets(
        'items_removed_manually_from_id_set.json', excluded_items_by_pack
    )

    print_success(
        'Starting to remove dependencies of excluded items from id_set'
    )
    ...
    save_dict_of_sets(
        'all_removed_items_from_id_set.json', excluded_items_by_pack
    )
...
def remove_items_from_packs_section(
    id_set: dict, excluded_items_by_pack: dict
) -> None:
    """
    ...
    for pack, pack_items in excluded_items_by_pack.items():
        pack_content_items = packs_section_from_id_set.get(pack, {}).get(
            'ContentItems'
        )

        if (
            not pack_content_items
        ):  # This pack has been excluded from the id_set
            continue
