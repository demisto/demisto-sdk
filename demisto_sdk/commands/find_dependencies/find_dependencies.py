...
def update_pack_metadata_with_dependencies(
    pack_folder_name, first_level_dependencies, all_level_dependencies
):
    """
...
class PackDependencies:
    ...
    def _search_packs_by_items_names(items_names, items_list):
        ...
        content_items = list(
            filter(
                lambda s: list(s.values())[0].get("name", "") in items_names
                and "pack" in list(s.values())[0],
                items_list,
            )
        )
    ...
    def _search_packs_by_integration_command(command, id_set):
        ...
        integrations = list(
            filter(
                lambda i: command in list(i.values())[0].get("commands", [])
                and "pack" in list(i.values())[0],
                id_set["integrations"],
            )
        )
    ...
    def _collect_scripts_dependencies(pack_scripts, id_set):
        ...
        for script_mapping in pack_scripts:
            ...
            for command in dependencies_commands:
                pack_names = PackDependencies._search_packs_by_items_names(
                    command, id_set["scripts"]
                )
    ...
    def _collect_playbooks_dependencies(pack_playbooks, id_set):
        ...
        for playbook in pack_playbooks:
            ...
            packs_found_from_scripts = PackDependencies._search_packs_by_items_names(
                implementing_script_names, id_set["scripts"]
            )
            ...
            for (
                ...
                packs_found_from_integration = (
                    PackDependencies._search_packs_by_items_names(
                        integration_name, id_set["integrations"]
                    )
                    if integration_name
                    else PackDependencies._search_packs_by_integration_command(
                        command, id_set
                    )
                )
            ...
            implementing_playbook_names = playbook_data.get(
                "implementing_playbooks", []
            )
            packs_found_from_playbooks = PackDependencies._search_packs_by_items_names(
                implementing_playbook_names, id_set["playbooks"]
            )
    ...
    def _collect_pack_items(pack_id, id_set):
        ...
        """
        pack_scripts = PackDependencies._search_for_pack_items(
            pack_id, id_set["scripts"]
        )
        pack_playbooks = PackDependencies._search_for_pack_items(
    ...
    def _find_pack_dependencies(pack_id, id_set):
        ...
        """
        pack_scripts, pack_playbooks = PackDependencies._collect_pack_items(
            pack_id, id_set
        )
    ...
    def find_dependencies(pack_name, id_set_path=None):
        ...
        dependency_graph = PackDependencies.build_dependency_graph(
            pack_id=pack_name, id_set=id_set
        )
        ...
        update_pack_metadata_with_dependencies(
            pack_name, first_level_dependencies, all_level_dependencies
        )
        # print the found pack dependency results
        click.echo(
            click.style(f"Found dependencies result for {pack_name} pack:", bold=True)
        )
