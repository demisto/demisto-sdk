...
def create_a_pack_entity(
    ...
    if entity_type == FileType.SCRIPT:
        ...
        pack.create_test_playbook(entity_id).create_default_test_playbook(
            entity_id
        )
...
def working_repo(repo):
    ...
    create_a_pack_entity(
        expanse, FileType.PLAYBOOK, 'Expanse_Incident_Playbook'
    )
    ...
    create_a_pack_entity(
        hello_world, FileType.CLASSIFIER, 'HelloWorld', 'HelloWorld'
    )
    ...
    create_a_pack_entity(
        slack, FileType.PLAYBOOK, 'Failed Login Playbook - Slack v2'
    )
    ...
    create_a_pack_entity(
        ipinfo, FileType.INTEGRATION, 'ipinfo', commands=['ip']
    )
    ...
    create_a_pack_entity(
        phishing, FileType.INCIDENT_FIELD, 'emailfrom', 'Email From'
    )
    create_a_pack_entity(
    ...
    for field_id, field_name in zip(
        ct_incident_field_ids, ct_incident_field_names
    ):
        create_a_pack_entity(
            common_types, FileType.INCIDENT_FIELD, field_id, field_name
        )
    for field_id, field_name in zip(
        ct_incident_type_ids, ct_incident_type_names
    ):
        create_a_pack_entity(
            common_types, FileType.INCIDENT_TYPE, field_id, field_name
        )
    for field_id, field_name in zip(
        ct_indicator_field_ids, ct_indicator_field_names
    ):
        create_a_pack_entity(
            common_types, FileType.INDICATOR_FIELD, field_id, field_name
        )
    create_a_pack_entity(
        common_types, FileType.REPUTATION, 'accountrep', 'Account Rep'
    )
    ...
    create_a_pack_entity(
        common_scripts, FileType.SCRIPT, 'AssignAnalystToIncident'
    )
    ...
    create_a_pack_entity(
        common_playbooks, FileType.PLAYBOOK, 'Block IP - Generic v2'
    )
    create_a_pack_entity(
    ...
    create_a_pack_entity(
        carbon_black_enterprise_response, FileType.SCRIPT, 'CBAlerts'
    )

    # Create a pack called 'Claroty' with 3 mappers and 1 incident_type.
    claroty = repo.create_pack('Claroty')
    create_a_pack_entity(
        claroty, FileType.MAPPER, 'CBAlerts-mapper', 'Claroty-mapper'
    )
    ...
    create_a_pack_entity(
        ews, FileType.MAPPER, 'EWS v2-mapper', 'EWS v2-mapper'
    )
    ...
    create_a_pack_entity(
        impossible_traveler, FileType.SCRIPT, 'CalculateGeoDistance'
    )
    create_a_pack_entity(
        impossible_traveler, FileType.PLAYBOOK, 'Impossible Traveler'
    )
    create_a_pack_entity(
        impossible_traveler,
    ...
    create_a_pack_entity(
        impossible_traveler, FileType.LAYOUT, 'Impossible Traveler'
    )
    create_a_pack_entity(
        impossible_traveler,
        FileType.INCIDENT_FIELD,
...
class TestIdSetFilters:
    ...
    def test_search_for_pack_item_with_no_result(
        self, item_section, module_repo
    ):
...
class TestDependsOnPlaybook:
    ...
    @pytest.mark.parametrize(
        'integration_command', ['ip', 'domain', 'url', 'cve']
    )
    ...
    def test_collect_playbooks_dependencies_on_incident_fields(
        ...
        """
        expected_result = {
            ('DigitalGuardian', False),
            ('EmployeeOffboarding', False),
        }
        test_input = [
            {
                'Dummy Playbook': {
    ...
    def test_collect_playbooks_dependencies_on_indicator_fields(
        self, module_repo
    ):
        """
        Given
...
class TestDependsOnLayout:
    ...
    def test_layouts_dependencies(
        self, pack_name, expected_dependencies, module_repo
    ):
        dependencies = PackDependencies.find_dependencies(
            pack_name,
            id_set_path=module_repo.id_set.path,
    ...
    def test_collect_indicator_layouts_dependencies_with_items(
        self, module_repo
    ):
        """
        Given
        ...
        expected_result = (
            ...
            },
            {
                ('layout', 'Dummy Layout'): {
                    ...
                    'CommonTypes': [
                        ('indicator_field', 'indicator_adminname')
                    ],
                    'CrisisManagement': [
                        ('indicator_field', 'indicator_jobtitle')
                    ],
                }
            },
        )
...
class TestDependsOnClassifiers:
    ...
    def test_collect_classifier_dependencies__commontypes_pack(
        self, module_repo
    ):
        """
        Given
...
class TestDependsOnJob:
    ...
    def test_collect_job_dependencies(
        self, module_repo, feed_dependency: bool
    ):
        """
        Given
        ...
        test_job_data = [
            {
                'jobby': {
                    ...
                    'fromVersion': FILETYPE_TO_DEFAULT_FROMVERSION.get(
                        FileType.JOB
                    ),
                }
            }
...
SEARCH_PACKS_INPUT = [
    ...
    (
        ...
        (
            {'Compliance'},
            {'Compliance': [('incident_field', 'incident_emailaddress')]},
        ),
        'incident_field',
    ),
    (
...
def test_find_dependencies_using_pack_metadata(mocker):
    ...
    assert not first_level_dependencies.get('dependency_pack2', {}).get(
        'mandatory'
    )
    assert not first_level_dependencies.get('dependency_pack3', {}).get(
        'mandatory'
    )
    assert first_level_dependencies.get('dependency_pack4', {}).get(
        'mandatory'
    )
...
class TestDependencyGraph:
    ...
    def test_build_all_dependencies_graph(self, mocker):
...

        {
            ...
            'Expanse Behavior': {
                'Expanse': 'Expanse Behavior Severity Update'
            },
        }
        mocker.patch(
        ...
        assert [n for n in dependency_graph.neighbors('pack1')] == [
            'pack2',
            'pack3',
        ]
...
class TestCalculateSinglePackDependencies:
    ...
    def setup_class(cls):
        ...
        (
            cls.first_level_dependencies,
            cls.all_level_dependencies,
            _,
        ) = dependencies
    ...
    def test_calculate_single_pack_dependencies_mandatory_dependencies(self):
        ...
        assert self.first_level_dependencies[expected_mandatory_dependency][
            'mandatory'
        ]
...
class TestGetDependentOnGivenPack:
    def test_get_dependent_on_given_pack(self, mocker):
        ...
        (
            dependent_packs_dict,
            dependent_packs,
        ) = get_packs_dependent_on_given_packs('pack3', '')
        assert 'pack2' in dependent_packs
        assert 'pack1' in dependent_packs
        assert dependent_packs_dict['pack3'][
        ...
        assert dependent_packs_dict['pack3'][
            'packsDependentOnThisPackMandatorily'
        ]['pack1']['dependent_items'] == [
            (('type_item_3', 'item3'), ('type_item_a', 'item_a'))
        ]
        assert dependent_packs_dict['pack3'][
            'packsDependentOnThisPackMandatorily'
        ]['pack2']['dependent_items'] == [
...
ID_SET = {
    ...
    'Packs': {
        'pack1': {
            'name': 'pack1',
            'ContentItems': {'playbooks': ['playbook1']},
        },
        'pack2': {
            'name': 'pack2',
            'ContentItems': {'scripts': ['script1', 'script2']},
        },
    },
}
...
def test_remove_items_from_content_entities_sections():
    ...
    assert IsEqualFunctions.is_dicts_equal(
        id_set, expected_id_set_entities_section
    )
