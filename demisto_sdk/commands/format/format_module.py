...
def run_format_on_file(
    input: str, file_type: str, from_version: str, **kwargs
    ...
    schema_path = os.path.normpath(
        os.path.join(
            __file__, "..", "..", "common", SCHEMAS_PATH, "{}.yml".format(file_type)
        )
    )
    UpdateObject = FILE_TYPE_AND_LINKED_CLASS[file_type](
        input=input, path=schema_path, from_version=from_version, **kwargs
    )
...
def logger(
    input: str, format_res: int, validate_res: int
) -> Tuple[List[str], List[str], List[str]]:
    info_list = []
    error_list = []
    skipped_list = []
