...
def get_files_to_format_from_git(
    ...
    for file_path in all_changed_files:
        ...
        if file_extension in supported_file_types and os.path.exists(
            str_file_path
        ):
    ...
    for file in files:
        ...
        with open(file_path, 'r+') as f:
            ...
            for (
                id_to_replace,
                updated_id,
            ) in CONTENT_ENTITY_IDS_TO_UPDATE.items():
...
def run_format_on_file(
    ...
    if (
        file_type not in ('integration', 'script')
        and 'update_docker' in kwargs
    ):
        # non code formatters don't support update_docker param. remove it
        del kwargs['update_docker']
    if (
        file_type not in ('integration', 'playbook', 'script')
        and 'add_tests' in kwargs
    ):
    ...
    if (
        not updater_class
    ):  # fail format so long as xsiam entities dont have formatters
