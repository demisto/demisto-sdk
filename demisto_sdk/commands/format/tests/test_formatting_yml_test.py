...
def test_basic_yml_updates(
    ...
    schema_path = os.path.normpath(
        os.path.join(
            __file__, "..", "..", "..", "common", "schemas", "{}.yml".format(file_type)
        )
    )
...
INTEGRATION_BANG_COMMANDS_ARGUMENTS_PACK = [
    ...
    (
        ...
        "integration",
        "email",
        [("default", True), ("isArray", True), ("required", True), ("description", "")],
    ),
]
...
def test_playbook_task_description_name(source_path):
    schema_path = os.path.normpath(
        os.path.join(
            __file__, "..", "..", "..", "common", "schemas", "{}.yml".format("playbook")
        )
    )
...
EQUAL_TEST = [
    (
        EQUAL_VAL_FORMAT_PLAYBOOK_SOURCE,
        EQUAL_VAL_FORMAT_PLAYBOOK_DESTINATION,
        EQUAL_VAL_PATH,
    ),
]
...
def test_pwsh_format(tmpdir, yml_file, yml_type):
    schema_path = os.path.normpath(
        os.path.join(
            __file__, "..", "..", "..", "common", "schemas", "{}.yml".format(yml_type)
        )
    )
