...
class TestFormatting:
    ...
    def test_default_outputs_filled(
        ...
        schema_path = (
            ...
            / 'common/schemas/integration.yml'
        )

        formatter = IntegrationYMLFormat(
            integration.yml.path, path=schema_path
        )
    ...
    def test_proxy_ssl_descriptions(
        ...
        base_yml = IntegrationYMLFormat(
            source_path, path=schema_path, verbose=True
        )
    ...
    def test_isarray_false(self, integration, capsys):
        ...
        assert (
            integration.yml.read_dict()['script']['commands'][0]['arguments'][
                0
            ]['isArray']
            is False
        )
    ...
    @pytest.mark.parametrize(
        'source_path', [PLAYBOOK_WITH_INCIDENT_INDICATOR_SCRIPTS]
    )
    ...
    @pytest.mark.parametrize(
        'source, target, path, answer', FORMAT_FILES_FETCH
    )
    ...
    def test_set_feed_params_in_config(
        self, mocker, source, target, path, answer
    ):
        """
        Given
    ...
    def test_run_format_on_tpb(self):
        ...
        formatter = TestPlaybookYMLFormat(
            input=SOURCE_FORMAT_TEST_PLAYBOOK,
            output=DESTINATION_FORMAT_TEST_PLAYBOOK,
        )
    ...
    def test_format_boolean_default_value(self):
        ...
        formatter = IntegrationYMLFormat(
            input=SOURCE_FORMAT_INTEGRATION_DEFAULT_VALUE
        )
    ...
    def test_update_tests_on_script_with_test_playbook(self):
        ...
        formatter.update_tests()
        assert formatter.data.get('tests') == [
            'playbook-checkEmailAuthenticity-test'
        ]
    ...
    def test_update_docker_format(self, tmpdir, mocker, monkeypatch):
        ...
        test_files_dir = (
            f'{GIT_ROOT}/demisto_sdk/tests/test_files/update-docker'
        )
    ...
    @pytest.mark.parametrize(
        argnames='docker_image',
        argvalues=['error:1.0.0.1', 'demisto/error:1.0.0.1'],
    )
    def test_update_docker_format_with_invalid_dockerimage(
        ...
        format_obj = ScriptYMLFormat(
            str(integration_yml_file_1), update_docker=True
        )
    ...
    def test_recursive_extend_schema(self):
        ...
        assert (
            modified_schema['mapping']['inputs']['sequence'][0] == sub_schema
        )
    ...
    def test_check_for_subplaybook_usages(self, repo):
        ...
        playbook_data = playbook.yml.read_dict()
        assert (
            playbook_data['tasks']['1']['task']['playbookId']
            == 'my-sub-playbook'
        )
    ...
    def test_tpb_name_format_change_new_tpb(self, repo):
        ...
        playbook_yml = TestPlaybookYMLFormat(
            test_playbook.yml.path,
            path=test_playbook.yml.path,
            assume_yes=True,
        )
        with ChangeCWD(repo.path):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                'Replacement entity',
                'Deprecated. Use Replacement entity instead.',
            ),
        ],
    )
    def test_update_deprecate_in_integration(
        ...
        base_update_yml = BaseUpdateYML(
            input=integration.yml.path, deprecate=True
        )
    ...
    def test_update_deprecate_in_playbook(
        ...
        base_update_yml = BaseUpdateYML(
            input=playbook.yml.path, deprecate=True
        )
    ...
    def test_remove_spaces_end_of_id_and_name(self, pack, mocker, name):
        ...
        integration.yml.write_dict(
            {'commonfields': {'id': name}, 'name': name}
        )
        mocker.patch.object(
            BaseUpdateYML,
    ...
    def test_sync_to_master_no_change(self, mocker, tmp_path):
        ...
        mocker.patch.object(
            update_generic, 'get_remote_file', return_value=yml_example
        )
    ...
    def test_equal_id_and_name_integartion(self, pack, mocker):
        ...
        mocker.patch.object(
            update_generic, 'get_remote_file', return_value=None
        )
    ...
    def test_equal_id_and_name_playbook_from_master(self, pack, mocker):
        ...
        mocker.patch.object(
            update_generic, 'get_remote_file', return_value={'id': uid}
        )
        base_yml = IntegrationYMLFormat(input=playbook.yml.path)
        base_yml.update_id_to_equal_name()
    ...
    def test_set_fetch_params_in_config_acording_marketplaces_value(
        ...
        assert all(
            config in base_yml.data['configuration']
