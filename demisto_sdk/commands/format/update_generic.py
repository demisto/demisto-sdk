...
class BaseUpdate:
            ...
            for key, value in current_schema.items():
                ...
                if isinstance(value, str) and key == 'include':
                    ...
                    if extended_schema is None:
                        click.echo(
                            f'Could not find sub-schema for {value}',
                            LOG_COLORS.YELLOW,
                        )
    ...
    def recursive_remove_unnecessary_keys(
    ...
    def initiate_file_validator(self) -> int:
        ...
        if self.no_validate:
            ...
            if self.is_old_file(self.output_file):
                validation_result = (
                    self.validate_manager.run_validation_using_git()
                )
            else:
                validation_result = (
    ...
    def check_server_version(self):
        ...
        current_from_server_version = self.data.get(
            self.json_from_server_version_key
        )
        current_from_version = self.data.get(self.from_version_key)
        old_from_server_version = self.old_file.get(
            self.json_from_server_version_key
        )
