...
class GenericFieldJSONFormat(BaseUpdateJSON):
    ...
    def update_id_field_if_needed(self):
        ...
        id = self.data.get("id")
        if not id.startswith(GENERIC_FIELD_DEFAULT_ID_PREFIX):
            updated_id = f'{GENERIC_FIELD_DEFAULT_ID_PREFIX}_{id}'
            if self.assume_yes:
                if self.verbose:
                ...
                set_from_version = str(
                    input(f"\nYour current id is: '{id}'. Do you want to set it to '{updated_id}'? Y/N ")).lower()
