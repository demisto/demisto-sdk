...
def append_or_replace_command_in_docs(old_docs: str, new_doc_section: str,
    ...
    if re.findall(regexp, old_docs, flags=re.DOTALL):
        ...
        print_color('New command docs has been replaced in README.md.',
                    LOG_COLORS.GREEN)
    else:
        ...
        if old_docs.endswith('\n'):
            ...
            old_docs = old_docs[:-1]
        new_docs = f'{old_docs}\n{new_doc_section}'
        print_color('New command docs has been added to the README.md.',
                    LOG_COLORS.GREEN)
    ...
    try:
        ...
        if command:
            ...
            for specific_command in specific_commands:
                ...
                doc_text, err = append_or_replace_command_in_docs(doc_text,
                                                                  command_section_str,
                                                                  specific_command)
                errors.extend(err)
        else:
...
def generate_commands_section(
    ...
    commands = filter(lambda cmd: not cmd.get('deprecated', False),
                      yaml_data['script']['commands'])
    command_sections: list = []
    if command:
        # for specific command, return it only.
        try:
            command_dict = \
            list(filter(lambda cmd: cmd['name'] == command, commands))[0]
        except IndexError:
            err = f'Could not find the command `{command}` in the .yml file.'
            print_error(err)
            raise IndexError(err)
        return generate_single_command_section(command_dict, example_dict,
                                               command_permissions_dict)
    for cmd in commands:
        cmd_section, cmd_errors = generate_single_command_section(cmd,
                                                                  example_dict,
                                                                  command_permissions_dict)
...
def generate_single_command_section(cmd: dict, example_dict: dict,
    ...
    if arguments is None:
        ...
        for arg in arguments:
            ...
            section.append('| {} | {} | {} | '.format(arg['name'],
                                                      string_escape_md(
...
def generate_versions_differences_section(input_path, old_version,
                                          display_name) -> list:
    """
    ...
    differences_section.extend(
        ['## Additional Considerations for this version', '%%FILL HERE%%',
...
def get_previous_version_differences(new_integration_path,
                                     previous_integration_path) -> list:
    """
    ...
    differences_detector = IntegrationDiffDetector(new=new_integration_path,
                                                   old=previous_integration_path)
...
def disable_md_autolinks(markdown: str) -> str:
    ...
    if not markdown:
        return markdown
    return re.sub(r'\b(?<!\)\[)(https?)://([\w\d]+?\.[\w\d]+?)\b',
                  r'\1:<span>//</span>\2', markdown, flags=re.IGNORECASE)
...
def generate_command_example(cmd, cmd_example=None):
    ...
    if cmd_example is not None:
        ...
        cmd_example = ' '
        errors.append(
            'did not get any example for {}. please add it manually.'.format(
                cmd['name']))
...
def add_access_data_of_type_credentials(access_data: List[Dict],
                                        credentials_conf: Dict) -> None:
    """
    ...
    access_data.append(
        {'Parameter': credentials_conf.get('displaypassword', 'Password'),
         'Description': '' if display_name else string_escape_md(
             credentials_conf.get('additionalinfo', '')),
         'Required': credentials_conf.get('required', '')})
