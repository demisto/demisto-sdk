...
def generate_integration_doc(
    ...
    try:
        ...
        if command:
            ...
            # breaking changes
            if integration_version and not skip_breaking_changes:
                docs.extend(generate_versions_differences_section(input_path, old_version, yml_data.get("display", "")))
            ...
            access_data.append(
                {
                    ...
                    "Description": string_escape_md(
                        conf.get("additionalinfo", "") or default_additional_info.get(conf.get("name", ""), "")
                    ),
...
def get_command_examples(commands_examples_input, specific_commands):
    ...
    if specific_commands:
        command_examples = [
            command_ex for command_ex in command_examples if command_ex.split(" ")[0].strip("!") in specific_commands
        ]
