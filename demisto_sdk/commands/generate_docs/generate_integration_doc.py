...
def append_or_replace_command_in_docs(
    ...
    if re.findall(regexp, old_docs, flags=re.DOTALL):
        ...
        print_color(
            'New command docs has been replaced in README.md.',
            LOG_COLORS.GREEN,
        )
    else:
        ...
        new_docs = f'{old_docs}\n{new_doc_section}'
        print_color(
            'New command docs has been added to the README.md.',
            LOG_COLORS.GREEN,
        )
...
def generate_integration_doc(
    ...
    try:
        ...
        if command:
            ...
            integration_version = re.findall(
                '[vV][2-9]$', yml_data.get('display', '')
            )
            ...
            access_data.append(
                {
                    ...
                    'Description': string_escape_md(
                        ...
                        or default_additional_info.get(
                            conf.get('name', ''), ''
                        )
                    ),
...
def generate_commands_section(
    ...
    commands = filter(
        lambda cmd: not cmd.get('deprecated', False),
    ...
    if command:
        ...
        try:
            command_dict = list(
                filter(lambda cmd: cmd['name'] == command, commands)
            )[0]
        except IndexError:
...
def generate_single_command_section(
    ...
    if command_permissions_dict:
        ...
        isinstance(command_permissions_dict, dict)
        and not command_permissions_dict
    ):
        cmd_permission_example = [
            '#### Required Permissions',
...
def get_previous_version_differences(
    ...
    differences_detector.missing_items_report = (
        differences_detector.get_differences()
    )
        ...
        for script_example, md_example, context_example in cmd_example:
            ...
            example.extend(
                [
                    ...
                    '{}'.format(
                        '>'.join(
                            f'\n{disable_md_autolinks(md_example)}'.splitlines(
                                True
                            )
                        )
                    ),
