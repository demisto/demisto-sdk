...
TEST_INTEGRATION_2_PATH = os.path.join(
    FILES_PATH,
    'integration-display-credentials-none/integration-display'
...
def test_generate_list_section_empty():
    ...
    section = generate_list_section(
        'Inputs', [], empty_message='No inputs found.'
    )
...
def test_generate_numbered_section():
    ...
    section = generate_numbered_section(
        'Use Cases', '* Drink coffee. * Write code.'
    )
...
def test_get_inputs():
    ...
    assert (
        errors[0]
        == 'Error! You are missing description in playbook input InputA'
    )
...
def test_get_outputs():
    ...
    assert (
        errors[0]
        == 'Error! You are missing description in playbook output FileData'
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            'playbook name',
            '',
            '![playbook name](../doc_files/playbook_name.png)',
        ),
        ('playbook name', 'custom_path', '![playbook name](custom_path)'),
    ],
)
def test_generate_image_link(
    playbook_name, custom_image_path, expected_result
):
    """
    Given
...
def test_get_script_inputs():
    ...
    assert (
        errors[0]
        == 'Error! You are missing description in script input InputA'
    )
...
def test_get_script_outputs():
    ...
    assert (
        errors[0]
        == 'Error! You are missing description in script output outputB'
    )
...
def test_generate_script_doc(tmp_path, mocker):
    ...
    expected_readme = os.path.join(
        FILES_PATH, 'docs_test', 'set_expected-README.md'
    )
...
class TestAppendOrReplaceCommandInDocs:
    ...
    def test_append_or_replace_command_in_docs_positive(
...
class TestGenerateIntegrationDoc:
    ...
    def test_generate_integration_doc(self, mocker):
        ...
        mocker.patch.object(
            common, 'execute_command', side_effect=handle_example
        )
        # Generate doc
        ...
        with open(fake_readme) as fake_file:
            with open(
                ...
                assert (
                    'Number of users to return. Max 300. Default is 30.'
                    in fake_data
                )
    ...
    def test_generate_integration_doc_new_contribution(self):
        ...
        """
        fake_readme = os.path.join(
            os.path.dirname(TEST_INTEGRATION_PATH),
            'fake_new_contribution_README.md',
        )
        # Generate doc
    ...
    def test_integration_doc_credentials_display_missing(self):
        ...
        """
        readme = os.path.join(
            os.path.dirname(TEST_INTEGRATION_2_PATH), 'README.md'
        )
        # Generate doc
        generate_integration_doc(
            TEST_INTEGRATION_2_PATH, skip_breaking_changes=True
        )
...
def test_generate_setup_section_with_additional_info(
    yml_input, expected_results
):
    """
    Given
...
def test_disable_md_autolinks():
    ...
    """
    assert (
        disable_md_autolinks('http://test.com')
        == 'http:<span>//</span>test.com'
    )
...
@pytest.mark.parametrize(
    'yml_content, expected_result', TEST_EMPTY_SCRIPTDATA_SECTION
)
