...
def test_json_body_to_code():
    ...
    request_body = {
        'test_id': '{test_id}',
        'test_name': '{test_name}',
        'test_filter': {'test_key': '{test_key}', 'test_const': 'CONST VALUE'},
    }

    body_code = json_body_to_code(request_body)
    assert (
        body_code
        == 'data={"test_filter":{"test_const":"CONST VALUE","test_key":test_key},"test_id":test_id,"test_name":test_name}'
    )
...
class TestCodeGenerator:
    ...
    def test_py_code_generated_from_config(self, mocker, tmpdir):
        ...
        with open(
            os.path.join(self.test_integration_dir, 'VirusTotalTest.py'),
            mode='r',
        ) as f:
    ...
    def test_yml_generated_from_config(self, mocker):
        ...
        with open(
            os.path.join(self.test_integration_dir, 'VirusTotalTest.yml'),
            mode='r',
        ) as f:
    ...
    def test_generate_integration_package(self, tmpdir, mocker):
        ...
        assert os.path.isfile(
            Path(tmpdir, 'VirusTotalTest', 'VirusTotalTest.py')
        )
        assert os.path.isfile(
            Path(tmpdir, 'VirusTotalTest', 'VirusTotalTest.yml')
        )
    ...
    def test_generate_unified_integration_yml(self, tmpdir, mocker):
        ...
        autogen_config.generate_integration_package(
            output_dir=tmpdir, is_unified=True
        )

        assert os.path.isfile(Path(tmpdir, 'integration-VirusTotalTest.yml'))
        with open(
            Path(tmpdir, 'integration-VirusTotalTest.yml'), mode='r'
        ) as f:
    ...
    def test_query_response_root_object(self):
        ...
        with open(
            os.path.join(
                self.test_files_path, 'VirusTotal-autogen-config.json'
            ),
            mode='r',
        ) as f:
            config_dict = json.load(f)
        ...
        assert (
            'contextPath: VirusTotalTest.TestScan.scans.field1'
            in integration_yml_str
        )
        assert (
            'contextPath: VirusTotalTest.TestScan.total_count'
            in integration_yml_str
        )
