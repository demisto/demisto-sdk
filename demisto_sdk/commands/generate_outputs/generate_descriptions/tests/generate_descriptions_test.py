...
def test_ai21_api_request(mocker):
    ...
    with requests_mock.Mocker() as m:
        ...
        assert (
            generate_descriptions.ai21_api_request(
                'test', {'prob_check': True}
            )
            == 'The ThreatStream **Platform**.'
        )
...
def test_build_description_with_probabilities():
    ...
    assert (
        generate_descriptions.build_description_with_probabilities(
            LOGPROB_INPUT_AI21
        )
        == 'The **Unkown**'
    )
...
def test_generate_ai_descriptions(mocker, tmp_path):
    ...
    with requests_mock.Mocker() as m:
        ...
        for output in (
            output_dict.get('script').get('commands')[0].get('outputs')
        ):
            assert (
                output.get('description') == 'The ThreatStream **Platform**.'
            )
...
def test_generate_ai_descriptions_interactive(mocker, tmp_path, monkeypatch):
    ...
    mocker.patch.object(
        generate_descriptions,
        'pinput',
        return_value='The brown fox is not brown',
    )
...
def test_generate_ai_descriptions_interactive_similar_path(mocker, tmp_path):
    ...
    # will be 222
    mocker.patch.object(
        generate_descriptions, 'pinput', side_effect=['111', '222']
    )
