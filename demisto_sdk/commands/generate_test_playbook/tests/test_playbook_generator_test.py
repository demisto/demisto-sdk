...
class TestGenerateTestPlaybook:
    ...
    def teardown_class(cls):
        ...
        if Path(TestGenerateTestPlaybook.TEMP_DIR).exists():
            shutil.rmtree(
                TestGenerateTestPlaybook.TEMP_DIR,
                ignore_errors=False,
                onerror=None,
            )
    ...
    def test_generate_test_playbook(self, use_all_brands, expected_yml):
        ...
        new_yml_path = (
            Path(TestGenerateTestPlaybook.TEMP_DIR)
    ...
    def test_generate_test_playbook__integration_under_packs(self, tmpdir):
        ...
        shutil.copy(
            TestGenerateTestPlaybook.DUMMY_INTEGRATION_YML_PATH,
            integration_path,
        )
...
def test_generate_test_playbook_with_command_examples(
    ...
    assert tpb_yml['tasks']['2']['scriptarguments']['first_name'] == {
        'simple': 'fname'
    }
    assert tpb_yml['tasks']['4']['scriptarguments']['user'] == {
        'simple': 'fname'
    }

    if commands:
        assert (
            tpb_yml['tasks']['4']['task']['script']
            == 'Zoom|||zoom-delete-user'
        )
    else:
        assert (
            tpb_yml['tasks']['4']['task']['script']
