...
class UnitTestsGenerator:
    ...
    def decision_maker(self, command_name: str):
        ...
        return (
            len(self.commands) == 0
            or command_name in self.commands_to_generate
        )
    ...
    def create_command_to_generate_dict(self):
        ...
        for command in self.command_examples:
            ...
            if command_name in self.commands_to_generate:
                self.commands_to_generate.get(command_name).append(
                    command_dict
                )
            else:
                self.commands_to_generate.update(
                    {command_name: [command_dict]}
                )
...
def run_generate_unit_tests(
    ...
    click.echo(
        '================= Running Unit Testing Generator ==================='
    )
    # validate inputs
    ...
    if source:
        try:
            ...
            print_error(
                f'An error occurred: {e.args if hasattr(e, "args") else e}'
            )
            return 1
    else:
