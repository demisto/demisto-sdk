...
class TestModule:
    ...
    def to_ast(self):
        ...
        if not self.to_concat:
            ...
            body.extend(
                [self.util_json_builder(), self.generate_test_client()]
            )
    ...
    def util_json_builder(self):
        ...
        io_open = ast_mod.Call(
            func=ast_mod.Attribute(
                value=ast_name('io'), attr=ast_name('open')
            ),
            args=[ast_name('path')],
            keywords=[
                ast_mod.keyword(arg='mode', value=ast_mod.Constant(value='r')),
                ast_mod.keyword(
                    arg='encoding', value=ast_mod.Constant(value='utf-8')
                ),
            ],
        )
        f_read = ast_mod.Call(
        ...
        body = [
            ast_mod.With(
                items=[
                    ast_mod.withitem(
                        context_expr=io_open, optional_vars=ast_name('f')
                    )
                ],
    ...
    def generate_test_client(self):
        ...
        for arg in client_init_args:
            arg_name = arg.arg
            arg_annotation = (
                str(arg.annotation) if hasattr(arg, 'annotation') else None
            )
            if 'url' in arg_name:
                keywords.append(
                ...
                keywords.append(
                    ast_mod.keyword(
                        arg=arg_name, value=ast_mod.Constant('test')
                    )
                )
            elif arg_name != 'self':
                keywords.append(
        ...
        func = ast_mod.FunctionDef(
            ...
            decorator_list=[
                ast_mod.Call(
                    func=ast_mod.Attribute(
                        value=ast_name('pytest'), attr='fixture'
                    ),
                    args=[],
                    keywords=[],
                )
    ...
    def build_imports(self, names_to_import: list):
        ...
        return ast_mod.ImportFrom(
            module=self.module_name, names=aliases, level=0
        )
