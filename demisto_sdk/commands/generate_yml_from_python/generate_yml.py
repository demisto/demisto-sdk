...
class YMLGenerator:
    ...
    def __init__(
        self, filename: str, verbose: bool = False, force: bool = False
    ):
        self.functions: list = []
    ...
    def import_the_metadata_collector(self):
        ...
        with mock.patch('builtins.__import__', side_effect=import_mock):
            try:
                ...
                if spec:
                    ...
                    if self.file_import:
                        ...
                        self.metadata_collector = (
                            self.file_import.metadata_collector
                        )
                        if self.verbose:
                            click.secho(
                ...
                click.secho(
                    f'No metadata collector found in {self.filename}', fg='red'
                )
                if (
                    not str(err)
    ...
    def remove_collector_imports(self):
        ...
        with open(self.filename, 'r+') as code_file:
            ...
            if content.startswith(collector_import_lines):
                ...
                clean_content = f'{collector_import_lines}'.join(
                    content_parts[1:]
                )
...
class MetadataToDict:
    ...
    def build_integration_dict(self):
        ...
        config_keys = [
            self.config_metadata_from_key(config_key)
            for config_key in self.mc.conf
        ]
        commands = [
            self.command_metadata_from_function(command)
            for command in self.mc.commands
        ]
        ...
        if self.mc.integration_name_x2:
            integration_dict['commonfields'][
                'name_x2'
            ] = self.mc.integration_name_x2
    ...
    def command_metadata_from_function(self, command: CommandMetadata) -> dict:
        ...
        if self.verbose:
            click.secho(
                f'Completed parsing metadata for command {command.name}.'
            )
    ...
    def organize_inputs_from_declaration(
        ...
        for in_arg in in_args:
            ...
            if input_arg.name.lower() not in restored_args:
                ...
                arg_type = (
                    declared_arg.annotation
                    if declared_arg
                    else arg_type_from_doc
                )
                default = (
                    declared_arg.default
                    if declared_arg
                    and declared_arg.default != inspect.Parameter.empty
                    else None
                )
                options = []
                secret = False
                ...
                if 'Args' in section:
                    ...
                    for arg_line in arg_lines:
                        (
                            in_arg,
                            in_arg_type,
                        ) = MetadataToDict.parse_in_argument_lines(
    ...
    def parse_in_argument_lines(
        ...
        if len(argument_sections) < 1:
            ...
            return (
                InputArgument(name=name, description=description),
                input_type,
            )
    ...
    def parse_out_argument_lines(
        ...
        argument_sections = re.findall(
            regex_arguments, argument_line, re.MULTILINE
        )
        if len(argument_sections) < 1:
            return None
        ...
        return OutputArgument(
            name=name, output_type=out_type, description=description
        )
