...
def save_code_as_integration(
    ...
    first_indent = len(code_snippet_lines[0]) - len(
        code_snippet_lines[0].lstrip()
    )
...
class TestImportDependencies:
    ...
    def test_generation_with_implicit_imports_in_declarations(
        self, tmp_path, repo
    ):
...
class TestConfigurationGeneration:
    ...
    def test_generate_conf_keys(
        ...
        def code_snippet():
            metadata_collector = YMLMetadataCollector(
                integration_name='some_name',  # noqa: F841
...
class TestCommandGeneration:
    ...
    def test_restored_args_not_in_command_metadata(self, tmp_path, repo):
        ...
        def code_snippet():
            ...
            @metadata_collector.command(
                ...
                outputs_list=[
                    OutputArgument(
                        name='out1', output_type=str, description='desc1'
                    )
                ],
            ...
            def funky_command(
                client, command_name, outputs_prefix, execution, args
            ):
                """Some funky command"""
                print('func')
        ...
        assert (
            'out1'
            in generated_dict['script']['commands'][0]['outputs'][0][
                'contextPath'
            ]
        )
    ...
    def test_restored_args(self, tmp_path, repo):
        ...
        (
            ...
            outputs_prefix,
            execution,
        ) = integration.funky_command('theClient')
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                {'options': ['a', 'b']},
                {'auto': 'PREDEFINED', 'predefined': ['a', 'b']},
            ),
        ],
        ids=[
            'name',
    ...
    def test_inputs_from_declaration(
        self, tmp_path, repo, docstring, expected_update
    ):
        """
        Given
    ...
    def test_outputs_from_declaration(
        self, tmp_path, repo, docstring, expected_update
    ):
        """
        Given
    ...
    def test_outputs_from_output_list(self, tmp_path, repo):
        ...
        def code_snippet():
            ...
            @metadata_collector.command(
                ...
                outputs_prefix='some',
                outputs_list=[
                    OutputArgument(name='some_out', description='some desc')
                ],
            )
            def funky_command():
                """Some other description"""
    ...
    def test_input_list_overrides_docstring(self, tmp_path, repo):
        ...
        def code_snippet():
            ...
            @metadata_collector.command(
                ...
                inputs_list=[
                    InputArgument(
                        name='listed_name', description='listed desc'
                    )
                ],
            )
            def funky_command():
...
class TestYMLGeneration:
    ...
    def test_yml_file_making(self, tmp_path, repo):
        ...
        yml_generator = YMLGenerator(
            filename=integration.code.path, force=True
        )
        yml_generator.generate()
