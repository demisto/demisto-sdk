...
class ContributionConverter:
        ...
        if script:
            try:
                if pack_version_reg := re.search(
                    ...
                    logger.info(f"Found a pack version {pack_version_reg.groups()[0]} using ###|//")
                    return pack_version_reg.groups()[0]
                elif pack_version_reg := re.search(
                    r"pack version = (\d+\.\d+\.\d+)'\)", script
                ):
                    logger.info(f"Found a pack version {pack_version_reg.groups()[0]} using pack version =")
                    return pack_version_reg.groups()[0]
            except Exception as e:
    ...
    def create_contribution_items_version_note(self):
        ...
        if self.contribution_items_version:
            logger.info(f"Creating a warning message since the contributed version isn't the latest version. {self.contribution_items_version}")
                ...
                try:
                    ...
                    try:
                        ...
                        if isinstance(content_item, IntegrationScript):
                            logger.info("The content item is of type IntegrationScript.")
                            ...
                            logger.info(f"Extracted the pack versions {contributor_item_version=}, {current_pack_version=}")
                            if contributor_item_version != "0.0.0" and Version(
                                current_pack_version
                            ) > Version(contributor_item_version):
                                logger.info("The contributed version isn't the latest version.")
