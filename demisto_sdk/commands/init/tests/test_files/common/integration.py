...
""" CONSTANTS """
LOG_LINE = (
    "HelloWorldDebugLog: "  # Make sure to use a line easily to search and read in logs.
)
...
class Client(BaseClient):
    ...
    def get_alert_list(
        self, limit: int, severity: str = None, last_id: int = 0
    ) -> list[dict]:
        """For developing walkthrough purposes, this is a dummy response.
           For real API calls, see the specific_api_endpoint_call_example method.
    ...
    def get_alert_list_for_fetch(
        self, limit, start_time: datetime, last_id: int = 0, severity: str = "low"
    ) -> list[dict]:
        ...
        def mock_time(item):
            ...
            item["date"] = datetime.strftime(
                start_time + timedelta(minutes=1), DATE_FORMAT
            )
...
def validate_api_key(api_key: str) -> None:
    ...
    if api_key != DUMMY_API_KEY:
        raise DemistoException(
            "Invalid Credentials. Please Verify your Connection parameters."
        )
...
def dedup_by_ids(
    alerts: list[dict], ids_to_compare: list[int]
) -> tuple[list[dict], int]:
...
def ip_reputation_command(
    ...
    for ip in ips:
        ...
        readable_attributes = tableToMarkdown(
            "Attributes", ip_data["attributes"], is_auto_json_transform=True
        )
...
def alert_list_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    if (
        alert_id
    ):  # If alert_id is provided, we only need one call to API and pagination is not needed.
...
def main() -> None:  # pragma: no cover
    ...
    try:
        ...
        if command == "test-module":
            ...
            default_threshold_ip = (
                arg_to_number(params.get("threshold_ip"))
                or DEFAULT_INDICATORS_THRESHOLD
            )
            return_results(
                ip_reputation_command(client, args, default_threshold_ip, reliability)
            )
