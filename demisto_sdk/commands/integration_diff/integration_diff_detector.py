...
class IntegrationDiffDetector:
    def __init__(
        self, new: str = '', old: str = '', docs_format: bool = False
    ):
    ...
    def get_differences(self) -> dict:
        ...
        parameters = self.get_different_params(
            self.old_yaml_data['configuration'],
            self.new_yaml_data['configuration'],
        )
        ...
        for argument in old_command_arguments:
            if argument not in new_command_arguments:
                ...
                new_command_argument = (
                    IntegrationDiffDetector.check_if_element_exist(
                ...
                if not new_command_argument:
                    ...
                    changed_fields = [
                        ...
                        or (
                            ...
                            and new_command_argument[field]
                        )
                    ]
    ...
    def check_changed_fields(
        ...
        for field in fields_to_check:
            if field in changed_fields:
                ...
                if (field == 'required' or field == 'isArray') and not element[
                    field
                ]:
                    continue
    ...
    def get_different_params(self, old_params, new_params) -> list:
        ...
        for old_param in old_params:
            if old_param not in new_params:
                param = self.check_if_element_exist(
                    old_param, new_params, 'display'
                )
                ...
                if not param:
                    ...
                    changed_fields = [
                        ...
                        if (
                            field in old_param
                            and param[field] != old_param[field]
                        )
    ...
    def print_items_in_docs_format(self, secho_result: bool = True) -> str:
        ...
        if 'arguments' in self.missing_items_report:
            ...
            if removed_arguments:
                ...
                argument_per_command = self.get_elements_per_command(
                    removed_arguments
                )
    ...
    def print_items(self) -> bool:
        ...
        if not self.missing_items_report:
            click.secho(
                'The integrations are backwards compatible', fg='green'
            )
            return False
