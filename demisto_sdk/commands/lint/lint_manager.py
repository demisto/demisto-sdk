...
class LintManager:
    ...
    def _gather_facts() -> Dict[str, Any]:
        ...
        try:
            ...
            print_warning(
                "You are running demisto-sdk lint not in content repositorty!"
            )
            ...
            for py_num in ["2", "3"]:
                ...
                with open(file=pipfile_lock_path) as f:
                    ...
                    facts[f"requirements_{py_num}"] = [
                        key + value["version"] for key, value in lock_file.items()
                    ]
                    logger.debug(
        ...
        try:
            ...
            print_warning(
                "Can't communicate with Docker daemon - check your docker Engine is ON - Skiping lint, "
    ...
    def _get_packages(
        self, content_repo: git.Repo, input: str, git: bool, all_packs: bool
    ) -> List[Path]:
    ...
    def _get_all_packages(content_dir: str) -> List[str]:
        ...
        content_main_pkgs: set = set(
            Path(content_dir).glob(
                ["Integrations/*/", "Scripts/*/", "Beta_Integrations/*/"]
            )
        )
    ...
    def _filter_changed_packages(
        content_repo: git.Repo, pkgs: List[Path]
    ) -> List[Path]:
        ...
        last_common_commit = content_repo.merge_base(
            content_repo.active_branch.commit, content_repo.remote().refs.master
        )
        changed_from_master = {
            content_repo.working_dir / Path(item.b_path).parent
    ...
    def run_dev_packages(
        ...
        self._create_report(pkgs_status=pkgs_status, path=json_report)
    ...
    def report_summary(self, lint_status: dict):
     ...
     """
        ...
        print(
            f"Packages PASS: {Colors.Fg.green}{len(self._pkgs) - len(failed)}{Colors.reset}"
        )
    ...
    def _create_report(pkgs_status: dict, path: str):
        if path:
            ...
            json.dump(fp=json_path.open(mode="w"), obj=pkgs_status, indent=4)
