...
class LintManager:
    ...
    def __init__(
        ...
        if check_dependent_api_module:
            print(
                'Checking for packages dependent on the modified API module',
                end='... ',
            )
        ...
        if json_file_path:
            if os.path.isdir(json_file_path):
                json_file_path = os.path.join(
                    json_file_path, 'lint_outputs.json'
                )
    ...
    def _gather_facts() -> Dict[str, Any]:
        ...
        try:
            ...
            python2_requirements = (
                pipfile_dir / 'pipfile_python2/dev-requirements.txt'
            )
        ...
        try:
            ...
            if (
                os.getenv('CI')
                and os.getenv('CIRCLE_PROJECT_REPONAME') == 'content'
            ):
    ...
    def _get_packages(
        ...
        if all_packs or git:
            ...
            for item in input:
                ...
                if is_pack:
                    pkgs.extend(
                        LintManager._get_all_packages(content_dir=item)
                    )
                else:
    ...
    def _filter_changed_packages(
        ...
        if (
            base_branch == 'master'
            and content_repo.active_branch.name == 'master'
        ):
            # case 1: comparing master against the latest previous commit
            last_common_commit = (
                content_repo.remote().refs.master.commit.parents[0]
            )
            print(
    ...
    def execute_all_packages(
        ...
        try:
            with concurrent.futures.ThreadPoolExecutor(
                ...
                for pack in sorted(self._pkgs):
                    linter: Linter = Linter(
                        ...
                        else Path(self._facts['content_repo'].working_dir),
                ...
                for i, future in enumerate(
                    ...
                    if pkg_status['warning_code']:
                        ...
                        if (
                            not return_warning_code
                            & pkg_status['warning_code']
                        ):
            ...
            if Version(platform.python_version()) > Version('3.9'):
                ...
                logger.info(
                    'Using Python under 3.8, we will cancel futures manually.'
                )
                executor.shutdown(
                    wait=True
    ...
    def run(
        ...
        self._create_failed_packs_report(
            lint_status=lint_status, path=failure_report
        )
    ...
    def _report_results(
        ...
        if not no_coverage:
            if coverage_report:
                generate_coverage_report(
                    html=True, xml=True, cov_dir=coverage_report
                )
            else:
                generate_coverage_report()
                    ...
                    self.linters_error_list.append(
                        {
                            ...
                            'pack': fail_pack,
                            'type': 'error',
                            'messages': pkgs_status[fail_pack][
                                f'{check}_errors'
                            ],
                        }
                    )
        ...
        if not all_packs:
            for check in [
                ...
                if EXIT_CODES[check] & return_warning_code:
                    ...
                    print(
                        f"{Colors.Fg.orange}{'#' * len(sentence)}{Colors.reset}\n"
                    )
    ...
    def report_unit_tests(
        ...
        for pkg, status in pkgs_status.items():
            if status.get('images'):
                if (
                    ...
                    print_v(
                        wrapper_pack.fill(
                            f'{Colors.Fg.green}{pkg}{Colors.reset}'
                        ),
                        log_verbose=self._verbose,
                    )
                        ...
                        if not image.get('image_errors'):
                            ...
                            if tests:
                                ...
                                for test_case in tests:
                                    outcome = test_case.get('call', {}).get(
                                        'outcome'
                                    )
                                    if outcome != 'failed':
                                        name = re.sub(
        ...
        if EXIT_CODES['pytest'] & return_exit_code:
            ...
            for fail_pack in lint_status['fail_packs_pytest']:
                print(
                    wrapper_pack.fill(
                        f'{Colors.Fg.red}{fail_pack}{Colors.reset}'
                    )
                )
                for image in pkgs_status[fail_pack]['images']:
                    tests = (
                    ...
                    if tests:
                        for test_case in tests:
                            if (
                                test_case.get('call', {}).get('outcome')
                                == 'failed'
                            ):
                                name = re.sub(
                                    pattern=r'\[.*\]',
    ...
    def report_failed_image_creation(
        ...
        if EXIT_CODES['image'] & return_exit_code:
            ...
            for fail_pack in lint_status['fail_packs_image']:
                print(
                    wrapper_pack.fill(
                        f'{Colors.Fg.cyan}{fail_pack}{Colors.reset}'
                    )
                )
                for image in pkgs_status[fail_pack]['images']:
    ...
    def report_summary(
        ...
        for fail_pack in failed:
            print(
                f'{Colors.Fg.red}{wrapper_fail_pack.fill(fail_pack)}{Colors.reset}'
            )
    ...
    def flake8_error_formatter(
        self, errors: Dict, json_contents: List
    ) -> None:
        ...
        for message in error_messages:
            if message:
                file_path, line_number, column_number, _ = message.split(
                    ':', 3
                )
    ...
    def get_full_file_path_for_vulture(
        file_name: str, content_path: str
    ) -> str:
    ...
    def vulture_error_formatter(
        ...
        for message in error_messages:
            if message:
                ...
                file_path = self.get_full_file_path_for_vulture(
    ...
    def xsoar_linter_error_formatter(
        self, errors: Dict, json_contents: List
    ) -> None:
    ...
    def add_to_json_outputs(output: Dict, file_path: str, json_contents: List):
        ...
        Args:
            ...
            json_contents (List): The JSON file outputs
        """
        yml_file_path = file_path.replace('.py', '.yml').replace(
            '.ps1', '.yml'
        )
