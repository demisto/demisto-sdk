...
class Linter:
        ...
        if self._pkg_lint_status["pack_type"] == TYPE_PYTHON:
            ...
            if self._facts["docker_engine"]:
                ...
                for image in self._facts["images"]:
                    ...
                    py_num: float = get_python_version_from_image(image=image[0], timeout=self.docker_timeout, log_prompt=log_prompt, docker_client=self._docker_client)
                    image[1] = py_num
    ...
    def _run_lint_on_docker_image(self, no_pylint: bool, no_test: bool, no_pwsh_analyze: bool, no_pwsh_test: bool,
        ...
        container_name = f"{self._pack_name}-container"
        ...
        try:
            ...
            for image in self._facts["images"]:
                ...
                test_image = self._docker_image_create(docker_base_image=image)
                ...
                if image_id and not errors or True:
    ...
    def _docker_image_create_orig(self, docker_base_image: List[Any]) -> Tuple[str, str]:
        """ Create docker image:
    ...
    def _docker_run_pylint(self, container_obj: docker.models.containers.Container, test_image: str, keep_container: bool) -> Tuple[int, str]:
        ...
        try:
            ...
            container_exit_code, outputs = container_obj.exec_run(command.split(' '))
    ...
    def _docker_run_pytest(self, container_obj: docker.models.containers.Container, test_image: str, keep_container: bool, test_xml: str, no_coverage: bool = False) -> Tuple[int, str, dict]:
        ...
        try:
            ...
            uid = os.getuid() or 4000
            logger.debug(f'{log_prompt} - user uid for running lint/test: {uid}')  # lgtm[py/clear-text-logging-sensitive-data]
            # container_exit_code, outputs = container_obj.exec_run([build_pytest_command(test_xml=test_xml, json=True,cov=cov)])
            container_exit_code, outputs = container_obj.exec_run(build_pytest_command(test_xml=test_xml, json=True,cov=cov).split(' '))
