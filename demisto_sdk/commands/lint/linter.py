...
class Linter:
    ...
    def __init__(self, project_dir: str, no_test: bool = False, no_pylint: bool = False, no_flake8: bool = False,
        ...
        self.run_mypy_script = os.path.join(
            dev_scripts_dir, self.run_mypy_script_name)
    ...
    def remove_common_server_python(self):
        ...
        if self.common_server_created:
            os.remove(os.path.join(self.project_dir,
    ...
    def run_flake8(self, py_num) -> int:
        ...
        output = run_command(
            f'{python_exe} -m flake8 {self.project_dir}', cwd=self.configuration.env_dir)
        self.lock.acquire()
        print("\n========= Running flake8 on: {}===============".format(lint_files))
        if len(output) == 0:
            print_color("flake8 completed for: {}\n".format(
                lint_files), LOG_COLORS.GREEN)
            if self.lock.locked():
                self.lock.release()
            return 0
    ...
    def run_mypy(self, py_num) -> int:
        ...
        script_path = os.path.abspath(os.path.join(
            self.configuration.sdk_env_dir, self.run_mypy_script))
        output = run_command(
            ' '.join(['bash', script_path, str(py_num), lint_files]), cwd=self.project_dir)
        self.lock.acquire()
    ...
    def _docker_login(self):
        ...
        res = subprocess.run(cmd, input=docker_pass,
    ...
    def _docker_image_create(self, docker_base_image, requirements):
        ...
        try:
            ...
            print_v("Failed check and delete for lock file: {}. Error: {}".format(
                lock_file, ex))
        wait_print = True
        for x in range(60):
    ...
    def _setup_dev_files(self, py_num):
        ...
        try:
            shutil.copy(self.configuration.env_dir +
            ...
            print_v('Could not copy demistomock and CommonServer files: {}'.format(
        ...
        if py_num > 3:
            ...
            try:
                ...
                if module_name:
                    ...
                    print_v(
                        'Copying ' + os.path.join(self.configuration.env_dir, 'Scripts', module_path))
