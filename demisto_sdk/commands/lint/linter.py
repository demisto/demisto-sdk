...
from demisto_sdk.commands.lint.docker_helper import (
    Docker,
...
class Linter:
    ...
    def run_pack(
        ...
        try:
            ...
            err = (
                f'{self._pack_abs_dir}: Unexpected fatal exception: {str(ex)}'
            )
    ...
    def _gather_facts(self, modules: dict) -> bool:
        ...
        if self._pkg_lint_status['pack_type'] == TYPE_PYTHON:
            ...
            if self._facts['docker_engine']:
                ...
                test_requirements = (
                    self._pack_abs_dir / 'test-requirements.txt'
                )
                ...
                logger.info(
                    f'{log_prompt} - Using python version from yml: {pynum}'
                )
    ...
    @timer(group_name='lint')
    def _run_lint_in_host(
        self, no_bandit: bool, no_mypy: bool, no_xsoar_linter: bool
    ):
    ...
    @timer(group_name='lint')
    def _run_xsoar_linter(
        self, py_num: str, lint_files: List[Path]
    ) -> Tuple[int, str]:
        ...
        with pylint_plugin(self._pack_abs_dir):
            ...
            stdout, stderr, exit_code = run_command_os(
                command=build_xsoar_linter_command(
                    lint_files, self._facts.get('support_level', 'base')
                ),
    ...
    def _run_lint_on_docker_image(
        ...
        for image in self._facts['images']:
            logger.info(
                f'{log_promopt} - Running lint on docker image {image[0]}'
            )
            ...
            for trial in range(2):
                image_id, errors = self._docker_image_create(
                        ...
                        if (
                            ...
                            if exit_code in [RERUN, FAIL]:
                                if check in {'flake8', 'vulture'}:
                                    self._handle_lint_results(
                                        exit_code, check, output
                                    )
                                else:
            ...
            logger.info(
                f'{log_promopt} - Finished linting on docker image {image[0]}'
            )
    ...
    @timer(group_name='lint')
    def _docker_image_create(
        self, docker_base_image: List[Any]
    ) -> Tuple[str, str]:
        """Create docker image:
        ...
        pip_requirements = (
            requirements + self._facts['additional_requirements']
        )
    ...
    def _docker_remove_container(self, container_name: str):
        try:
            ...
            if platform.system() != 'Darwin' or 'Connection broken' not in str(
                err
            ):
                raise
        ...
        try:
            ...
            container: docker.models.containers.Container = (
                Docker.create_container(
                    ...
                    command=[
                        build_pytest_command(
                            test_xml=test_xml, json=True, cov=cov
                        )
                    ],
                    user=f'{uid}:4000',
                    files_to_push=[(self._pack_abs_dir, '/devwork')],
            ...
            if container_exit_code in [0, 1, 2, 5]:
                ...
                if cov:
                    ...
                    cov_data = (
                        cov_data
                        if isinstance(cov_data, bytes)
                        else cov_data.encode()
                    )
            ...
            if keep_container:
                print(f'{log_prompt} - Container name {container_name}')
                container.commit(
                    repository=container_name.lower(), tag='pytest'
                )
            else:
                try:
                    container.remove(force=True)
    ...
    def _docker_run_pwsh_analyze(
        ...
        """
        log_prompt = (
            f'{self._pack_name} - Powershell analyze - Image {test_image}'
        )
        logger.info(f'{log_prompt} - Start')
        ...
        try:
            ...
            if keep_container:
                print(f'{log_prompt} - container name {container_name}')
                container.commit(
                    repository=container_name.lower(), tag='pwsh_analyze'
                )
            else:
                try:
                    container.remove(force=True)
    ...
    def _docker_run_pwsh_test(
        ...
        """
        log_prompt = (
            f'{self._pack_name} - Powershell test - Image {test_image}'
        )
        logger.info(f'{log_prompt} - Start')
        ...
        try:
            ...
            if keep_container:
                print(f'{log_prompt} - container name {container_name}')
                container.commit(
                    repository=container_name.lower(), tag='pwsh_test'
                )
            else:
                try:
                    container.remove(force=True)
