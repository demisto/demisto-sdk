...
def test_build_mypy_command(files, py_num, content_path):
    ...
    expected_cache_dir = (
        'test_path/.mypy_cache' if content_path else '/dev/null'
    )
...
def test_build_pytest_command_3():
    ...
    command = 'pytest -ra --junitxml=/devwork/report_pytest.xml --cov-report= --cov=test'
...
def test_build_pwsh_analyze():
    ...
    file = MagicMock()
    command = (
        f'pwsh -Command Invoke-ScriptAnalyzer -EnableExit -Path {file.name}'
    )
