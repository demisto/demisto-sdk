...
@pytest.mark.parametrize(
    argnames="py_num , expected_exec", argvalues=[(3.7, "python3"), (2.7, "python")]
)
...
def test_build_bandit_command(files):
    ...
    files = [str(file) for file in files]
    expected = (
        f"python3 -m bandit -lll -iii -a file --exclude=CommonServerPython.py,demistomock.py,"
...
@pytest.mark.parametrize(
    argnames="files, py_num", argvalues=[(values[0], "2.7"), (values[1], "3.7")]
)
def test_build_mypy_command(files, py_num):
    ...
    files = [str(file) for file in files]
    expected = (
        f"python3 -m mypy --python-version {py_num} --check-untyped-defs --ignore-missing-imports "
...
def test_build_vulture_command(files, mocker):
    ...
    expected = (
        f"python -m vulture --min-confidence 20 --exclude=CommonServerPython.py,demistomock.py,"
...
def test_build_pylint_command(files):
    ...
    files = [str(file) for file in files]
    expected = (
        "python -m pylint --ignore=CommonServerPython.py,demistomock.py,CommonServerUserPython.py,"
