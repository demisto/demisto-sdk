...
class TestGenerateCoverageReport:
    ...
    def test_generate_coverage_report_with_html(self, mocker):
        mock_html_report = mocker.patch.object(
            self.coverage.Coverage, 'html_report'
        )
        self.mock_path_exists(mocker)
        generate_coverage_report(report=False, html=True)
        mock_html_report.assert_called_once_with(
            directory='coverage_report/html'
        )

    def test_generate_coverage_report_with_xml(self, mocker):
        mock_xml_report = mocker.patch.object(
            self.coverage.Coverage, 'xml_report'
        )
        self.mock_path_exists(mocker)
        generate_coverage_report(report=False, xml=True)
        mock_xml_report.assert_called_once_with(
            outfile='coverage_report/coverage.xml'
        )
