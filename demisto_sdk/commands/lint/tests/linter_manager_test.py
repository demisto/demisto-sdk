...
def mock_lint_manager(mocker):
    ...
    mocker.patch.object(
        LintManager, '_gather_facts', return_value={'content_repo': ''}
    )
    return LintManager(
        input='',
        git=False,
...
def test_report_summary_with_warnings(capsys):
    ...
    assert (
        'Packages WARNING (can either PASS or FAIL): \x1b[33m1\x1b[0m'
        in captured.out
    )
...
def test_report_summary_no_warnings(capsys):
    ...
    assert (
        'Packages WARNING (can either PASS or FAIL): \x1b[33m0\x1b[0m'
        in captured.out
    )
...
def test_create_json_output_flake8(repo, mocker):
    ...
    mocker.patch.object(
        lint_manager, 'find_type', return_value=FileType.INTEGRATION
    )
    mocker.patch.object(
        lint_manager, 'get_file_displayed_name', return_value='Display'
    )
    check = {
        'linter': 'flake8',
        'pack': 'myPack',
...
def test_create_json_output_vulture(repo, mocker):
    ...
    mocker.patch.object(
        lint_manager,
        'find_file',
        return_value='Packs/myPack/Integrations/INT1/INT1.py',
    )
    mocker.patch.object(
        lint_manager, 'get_file_displayed_name', return_value='Display'
    )
