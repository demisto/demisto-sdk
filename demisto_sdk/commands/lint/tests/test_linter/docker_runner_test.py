...
class TestPylint:
    def test_run_pylint_no_errors(self, mocker, linter_obj: Linter):
        ...
        linter_obj._docker_client.containers.run(
            'test-image'
        ).wait.return_value = {'StatusCode': exp_container_exit_code}
        linter_obj._docker_client.containers.run(
            'test-image'
    ...
    def test_run_pylint_with_errors(
        ...
        linter.Docker.create_container().logs.return_value = (
...
class TestPytest:
    ...
    def test_run_pytest(
        ...
        exp_test_json = (
            mocker.MagicMock()
            if exp_container_exit_code in [0, 1, 2, 5]
            else {}
        )
