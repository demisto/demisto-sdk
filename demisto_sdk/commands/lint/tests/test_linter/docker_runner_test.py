    ...
    def test_build_image_no_errors(self, linter_obj: Linter, mocker):
        ...
        mocker.patch.dict(
            linter_obj._facts,
            {
                ...
                "docker_engine": True,
                "env_vars": {"CI": True, "DEMISTO_LINT_UPDATE_CERTS": "yes"},
            },
        )
        ...
        linter_obj._docker_client.containers.create().commit().short_id = (
            exp_test_image_id
        )

        act_test_image_id, act_errors = linter_obj._docker_image_create(
            docker_base_image=[exp_test_image_id, 3.7]
        )
...
class TestPylint:
    def test_run_pylint_no_errors(self, mocker, linter_obj: Linter):
        ...
        # Docker client mocking
        mocker.patch.object(linter_obj, "_docker_client")
        linter_obj._docker_client.containers.run().wait.return_value = {
            "StatusCode": exp_container_exit_code
        }
        linter_obj._docker_client.containers.run().logs.return_value = exp_container_log.encode(
            "utf-8"
        )
        act_container_exit_code, act_container_log = linter_obj._docker_run_pylint(
...
class TestPytest:
    ...
    def test_run_pytest(
        ...
        (
            ...
            test_image="test-image", keep_container=False, test_xml=""
        )
...
class TestRunLintInContainer:
    ...
    @pytest.mark.parametrize(
        argnames="no_test, no_pylint, no_pwsh_analyze, no_pwsh_test, pack_type",
        argvalues=[
            (True, True, False, False, TYPE_PYTHON),
    ...
    def test_run_one_lint_check_success(
        ...
        mocker.patch.dict(linter_obj._pkg_lint_status, {"pack_type": pack_type, })
