...
class TestFlake8:
    ...
    def test_run_flake8_usage_stderr(
        self, linter_obj: Linter, lint_files: List[Path], mocker
    ):
        from demisto_sdk.commands.lint import linter
...
class TestVulture:
    ...
    def test_run_vulture_fail_lint(
        self, linter_obj: Linter, lint_files: List[Path], mocker
    ):
        from demisto_sdk.commands.lint import linter
    ...
    def test_run_vulture_usage_stderr(
        self, linter_obj: Linter, lint_files: List[Path], mocker
    ):
        from demisto_sdk.commands.lint import linter
...
class TestRunLintInHost:
    ...
    @pytest.mark.parametrize(
        argnames="no_flake8, no_bandit, no_mypy, no_vulture",
        argvalues=[
    ...
    def test_run_all_lint_fail_all(self, mocker, linter_obj, lint_files):
        ...
        linter_obj._run_vulture.return_value = (0b1, "Error")
        linter_obj._run_lint_in_host(
            no_flake8=False, no_bandit=False, no_mypy=False, no_vulture=False
        )
        linter_obj._run_flake8.assert_called_once()
        assert linter_obj._pkg_lint_status.get("flake8_errors") == "Error"
        linter_obj._run_bandit.assert_called_once()
