...
class OpenAPIIntegration:
    ...
    def generate_python_code(self) -> str:
        ...
        for command in self.configuration["commands"]:
            function, req_function = self.get_python_command_and_request_functions(
                command
            )
    ...
    def get_python_command_and_request_functions(self, command: dict) -> tuple:
        ...
        command["path"] = (
            f"'{command['path']}'" if "'" not in command["path"] else command["path"]
        )
        command["path"] = (
            f"f{command['path']}" if "{" in command["path"] else command["path"]
        )
            ...
            if arg["properties"]:
                for k, v in arg["properties"].items():
                    ...
                    if "None" not in prop_default:
                        this_prop_argument = (
                            this_prop_argument.replace("$ARGTYPE$", f"{prop_arg_type}(")
                            + ")"
                        )
                    else:
    ...
    def get_yaml_params(self) -> list:
        ...
        if BASIC_AUTH_TYPE in self.configuration["auth"]:
            params.append(
                XSOARIntegration.Configuration(
                    display="Username", name="credentials", required=True, type_=9
                )
            )
            ...
            for arg in command["arguments"]:
                ...
                if arg["properties"]:
                    for k, v in arg["properties"].items():
                        ...
                        prop_required = (
                            True if required is True and v.get("required") else False
                        )
                        options = None
                        auto = None
                        if v.get("options"):
                    ...
                    args.append(
                        XSOARIntegration.Script.Command.Argument(
                            arg_name, description, required, auto, options, is_array
                        )
                    )
                        ...
                        prop_arr.append(
                            {
                                ...
                                "type": extracted_object.get("type", "Unknown"),
                        ...
                        if ref_props:
                            for k, prop in ref_props[0].items():
                                if k in self.root_objects:
                                    ...
                                    if path:
                                        context_path = self.clean_function_name(
        ...
        for arg in arguments:
            ...
            if "schema" in arg:
                ...
                for ref in refs:
                    ...
                    # Which are written in string and caused errors on ref_props[0].items()
                    ref_args = [
                        ref_arg for ref_arg in ref_args if isinstance(ref_arg, dict)
                    ]
                                ...
                                for complex_ref in complex_refs:
                                    for ck, cv in complex_ref.items():
                                        ...
                                        new_ref_arg["properties"][ck]["required"] = (
                                            True
                                            if ck
                                            in self.reference.get(c_ref, {}).get(
                                                "required", []
                                            )
                                            else False
                                        )
                                        new_ref_arg["properties"][ck]["options"] = (
    ...
    def save_image_and_desc(self, directory: str) -> tuple:
        ...
        try:
            shutil.copy(
                os.path.join(
                    os.path.dirname(__file__), "resources", "Generated_image.png"
                ),
                image_path,
            )
            shutil.copy(
                os.path.join(
                    os.path.dirname(__file__), "resources", "Generated_description.md"
                ),
                desc_path,
            )
    ...
    def init_arg(self, arg: dict):
        ...
        new_arg["enums"] = (
            [str(x) for x in arg.get("enum", [])] if arg.get("enum", None) else None
        )
    ...
    def extract_values(obj: Union[dict, list], key: str) -> list:
        ...
        def extract(
            extracted_object: Union[dict, list], values: list, key_to_extract: str
        ) -> list:
        ...
        if duplicate_names:
            for func in functions:
                ...
                if name in duplicate_names:
                    ...
                    path_name = "_".join(
                        [re.sub(r"{[^)]*\}", "", x) for x in function_path.split("/")]
                    )
