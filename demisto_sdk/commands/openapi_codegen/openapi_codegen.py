...
class OpenAPIIntegration:
        ...
        for path, function in self.json["paths"].items():
            try:
                for method, data in function.items():
                    ...
                    self.add_function(path, method, data, function.get("parameters", []))
            except Exception as e:
    ...
    def generate_python_code(self) -> str:
        ...
        for command in self.functions:
            ...
            function = BASE_LIST_FUNCTIONS.replace("$FUNCTIONNAME$", f"{prefix}{command['name']}".replace("_", "-"))
    ...
    def get_python_command_and_request_functions(self, command: dict) -> tuple:
        ...
        if arguments_found:
            ...
            function = "\n".join([x for x in function.split("\n") if "$ARGUMENTS$" not in x])
        ...
        if headers:
            ...
            for header in headers:
                for k, v in header.items():
                    new_headers.append(BASE_HEADER.replace("$HEADERKEY$", f"'{k}'").replace("$HEADERVALUE$", f"'{v}'"))
    ...
    def get_yaml_params(self) -> list:
        ...
        if BEARER_AUTH_TYPE in self.configuration["auth"]:
            params.append(XSOARIntegration.Configuration(display="API Key", name="api_key", required=True, type_=4))
        if BASIC_AUTH_TYPE in self.configuration["auth"]:
            params.append(
                XSOARIntegration.Configuration(display="Username", name="credentials", required=True, type_=9)
            )
                ...
                if extracted_object.get("type") and type(extracted_object.get("type")) == str:
                    if extracted_object.get("type") in ["array", "object"]:
                        ...
                        if refs:
                            ref = refs[0].split("/")[-1]
                            prop_arr = extract(
                                self.extract_values(self.reference.get(ref, {}), "properties"),
                                prop_arr,
                                current_context,
                            )
                    ...
                    for ref_arg in ref_args:
                        for k, v in ref_arg.items():
                            new_ref_arg = {
                                "name": k,
                                "in": arg.get("in"),
                                "required": True if k in self.reference.get(ref, {}).get("required", []) else False,
                            }
                            if "$ref" in ref_arg[k]:
                                ...
                                for complex_ref in complex_refs:
                                    for ck, cv in complex_ref.items():
                                        ...
                                        new_ref_arg["properties"][ck]["required"] = (
                                            True if ck in self.reference.get(c_ref, {}).get("required", []) else False
                                        )
                                        new_ref_arg["properties"][ck]["options"] = (
                                            [str(x) for x in cv.get("enum")] if arg.get("enum", None) else None
                                        )
                            else:
    ...
    def save_image_and_desc(self, directory: str) -> tuple:
        ...
        try:
            shutil.copy(
                os.path.join(os.path.dirname(__file__), "resources", "Generated_image.png"),
                image_path,
            )
            shutil.copy(
                os.path.join(os.path.dirname(__file__), "resources", "Generated_description.md"),
                desc_path,
            )
