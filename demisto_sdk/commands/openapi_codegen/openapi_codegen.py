...
class OpenAPIIntegration:
    def __init__(
        ...
        self.unique_keys = (
            unique_keys.split(',') if unique_keys is not None else []
        )
        self.root_objects = (
            root_objects.split(',') if root_objects is not None else []
        )
    ...
    def load_file(self):
        ...
        self.description = self.clean_description(
            self.json.get('info', {}).get('description', '')
        )
        self.definitions = self.json.get('definitions', {})
        self.components = self.json.get('components', {})
        self.reference = (
            self.definitions or self.components.get('schemas', {}) or {}
        )
            ...
            try:
                for method, data in function.items():
                    ...
                    self.print_with_verbose(
                        f'Adding command for the path: {path}'
                    )
                    self.add_function(
                ...
                print_error(
                    f'Failed adding the command for the path {path}: {e}'
                )
                raise
    ...
    def generate_configuration(self):
        ...
        for function in self.functions:
            ...
            if (
                function['consumes']
                and JSON_TYPE_HEADER not in function['consumes']
            ...
            if (
                function['produces']
                and JSON_TYPE_HEADER not in function['produces']
    ...
    def generate_python_code(self) -> str:
        ...
        for command in self.functions:
            ...
            function = BASE_LIST_FUNCTIONS.replace(
                '$FUNCTIONNAME$',
                f"{prefix}{command['name']}".replace('_', '-'),
            )
    ...
    def get_python_command_and_request_functions(self, command: dict) -> tuple:
        ...
        self.print_with_verbose(
            f'Adding the function {function_name} to the code...'
        )
        ...
        (
            ...
            body_data,
            params_data,
        ) = self.process_command_arguments(command)
        if arguments_found:
        ...
        for param in re.findall(
            ...
            if param in ILLEGAL_CODE_NAMES:
                command['path'] = command['path'].replace(
        ...
        for arg in command['arguments']:
            ...
            if arg['properties']:
                for k, v in arg['properties'].items():
                    ...
                    prop_arg_type = ARGUMENT_TYPES.get(
                        v.get('type', 'string'), 'str'
                    )
                    ...
                    this_prop_argument = this_prop_argument.replace(
                        '$SARGNAME$', prop_arg_name
                    )
                    if 'None' not in prop_default:
                        this_prop_argument = (
                            this_prop_argument.replace(
                                '$ARGTYPE$', f'{prop_arg_type}('
                            )
                            + ')'
                        )
                    else:
                ...
                all_props = self.format_params(
                    arg_props, BASE_PROPS, '$PROPS$'
                )
            ...
            if 'query' in arg['in']:
                params_data.append({arg['name']: ref_arg_name})
            elif arg['in'] in ['formData', 'body']:
                body_data.append({arg['name']: ref_arg_name})
    ...
    def generate_yaml(self) -> XSOARIntegration:
        ...
        commonfields = XSOARIntegration.CommonFields(
        ...
        int_script = XSOARIntegration.Script(
            '',
            self.configuration['code_type'],
            self.configuration['code_subtype'],
            self.configuration['docker_image'],
            self.configuration.get('fetch_incidents', False),
            commands,
        )
    ...
    def get_yaml_params(self) -> list:
        ...
        if BEARER_AUTH_TYPE in self.configuration['auth']:
            params.append(
                XSOARIntegration.Configuration(
                    display='API Key', name='api_key', required=True, type_=4
                )
            )
        if BASIC_AUTH_TYPE in self.configuration['auth']:
            params.append(
        ...
        for command in self.configuration['commands']:
            ...
            for arg in command['arguments']:
                ...
                if arg['properties']:
                    for k, v in arg['properties'].items():
                        ...
                        prop_required = (
                            True
                            if required is True and v.get('required')
                            else False
                        )
                        options = None
                        auto = None
            ...
            for output in command['outputs']:
                ...
                outputs.append(
                    XSOARIntegration.Script.Command.Output(
                        output_type, output_name, output_description
                    )
                )
            ...
            commands.append(
                XSOARIntegration.Script.Command(
                    command_name, command_description, args, outputs
                )
            )
    ...
    def extract_properties(
        self, obj: Union[dict, list], context: list
    ) -> list:
        """
        ...
        def extract(
            extracted_object: Union[dict, list],
            prop_arr: list,
                ...
                if (
                    ...
                    if extracted_object.get('type') in ['array', 'object']:
                        ...
                        if refs:
                            ...
                            prop_arr = extract(
                                self.extract_values(
                                    self.reference.get(ref, {}), 'properties'
                                ),
                                prop_arr,
                                current_context,
                            )
                        elif extracted_object.get('items'):
                            for k, v in extracted_object.get(
                                'items', {}
                            ).items():
                                prop_arr = extract(
                                    v, prop_arr, current_context
                                )
                        elif extracted_object.get('properties'):
                            prop_arr = extract(
                                extracted_object.get('properties', {}),
                                prop_arr,
                                current_context,
                            )
                            ...
                            for item in extracted_object.get('allOf', []):
                                prop_arr = extract(
                                    item, prop_arr, current_context
                                )
                    else:
                        prop_arr.append(
                            {
                                'name': '.'.join(current_context),
                                'type': extracted_object.get(
                                    'type', 'Unknown'
                                ),
                                'description': extracted_object.get(
                                    'description', ''
                                ),
                            }
                        )
                        ...
                        if ref_props:
                            for k, prop in ref_props[0].items():
                                if k in self.root_objects:
                                    ...
                                    if path:
                                        context_path = (
                                            self.clean_function_name(
                                                path[0].split('/')[-1], False
                                            )
                                        )
                                    found_root = True
                            if found_root:
                    ...
                    for ref_arg in ref_args:
                        for k, v in ref_arg.items():
                            ...
                            if '$ref' in ref_arg[k]:
                                ...
                                complex_refs = self.extract_values(
                                    self.reference.get(c_ref, {}), 'properties'
                                )
                                for complex_ref in complex_refs:
                                    for ck, cv in complex_ref.items():
                                        ...
                                        new_ref_arg['properties'][ck][
                                            ...
                                            [str(x) for x in cv.get('enum')]
                                            if arg.get('enum', None)
                                            else None
                                        )
                            else:
    ...
    def add_function(self, path: str, method: str, data: dict, params: list):
        ...
        """
        new_function: dict = {
            'path': '/'.join(x.split(' ')[0] for x in path.split('/')).strip(
                '/'
            ),
            'method': method,
        }
        ...
        if not name:
            name = '_'.join(
                [re.sub(r'{[^)]*\}', '', x) for x in path.split('/')]
            )
    ...
    def handle_duplicates(self, functions: List):
        ...
        duplicate_names = [
            n for n in duplicate_names if duplicate_names.count(n) > 1
        ]
        if duplicate_names:
