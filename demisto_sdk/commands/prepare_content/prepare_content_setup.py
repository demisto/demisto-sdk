...
from demisto_sdk.commands.common.tools import parse_marketplace_kwargs, find_type
...
def prepare_content(
        input: str = typer.Option(None, help="Comma-separated list of paths to directories or files to unify."),
        all: bool = typer.Option(False, is_flag=True,
        ...
        graph: bool = typer.Option(False, is_flag=True, help="Whether to use the content graph"),
        skip_update: bool = typer.Option(False, is_flag=True,
                                         help="Whether to skip updating the content graph (used only when graph is true)"),
        output: Path = typer.Option(None, help="The output dir to write the unified YML to"),
        custom: str = typer.Option(None, help="Add test label to unified YML id/name/display"),
        force: bool = typer.Option(False, is_flag=True, help="Forcefully overwrites the preexisting YML if one exists"),
        ignore_native_image: bool = typer.Option(False, is_flag=True,
                                                 help="Whether to ignore the addition of the native image key to the YML of a script/integration"),
        marketplace: str = typer.Option("xsoar",
                                        help="The marketplace the content items are created for, that determines usage of marketplace unique text.",
                                        type=typer.Choice([mp.value for mp in list(MarketplaceVersions)] + ["v2"]))
):
    """
    This command is used to prepare the content to be used in the platform.
    """
    assert (sum([bool(all), bool(input)]) == 1), "Exactly one of the '-a' or '-i' parameters must be provided."
    ...
    if output_path := output:
        if "." in Path(output_path).name:  # Check if the output path is a file
            if len(inputs) > 1:
                raise ValueError("When passing multiple inputs, the output path should be a directory and not a file.")
        else:
    ...
    for input_content in inputs:
        ...
        os.environ["ENV_DEMISTO_SDK_MARKETPLACE"] = marketplace.lower() if marketplace else ""
