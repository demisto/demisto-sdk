...
def test_split_internal_action_true():
    """
    Given:
        - A script action with action.internal=True (action is hidden) but no standalone set.
    When:
        - Calling AgentixActionSplitter.split().
    Then:
        - action_dict["internal"] is True (action is hidden).
        - script_dict["internal"] is True (default: generated scripts are always internal
          unless standalone=True is explicitly set; action.internal does NOT propagate to script).
    """
    from demisto_sdk.commands.prepare_content.agentix_action_splitter import (
        AgentixActionSplitter,
    )
    ...
    # action.internal=True → action is hidden
    assert action_dict.get("internal") is True
    # script is internal by default (standalone not set) — action.internal does NOT propagate
...
def test_split_action_internal_does_not_propagate_to_script_when_standalone_true():
    """
    Given:
        - A script action with action.internal=True AND standalone=True.
    When:
        - Calling AgentixActionSplitter.split().
    Then:
        - script_dict["internal"] is False/absent (standalone=True wins; action.internal
          does NOT propagate to the generated script).
        - action_dict["internal"] is True (action itself is hidden).
    """
    from demisto_sdk.commands.prepare_content.agentix_action_splitter import (
        AgentixActionSplitter,
    )
    ...
    action = _make_action(internal=True, script_config=script_cfg)
    processed_data = {
        "commonfields": {"id": "CortexIsolateEndpoint", "version": -1},
        "name": "CortexIsolateEndpoint",
        "display": "Cortex - Isolate Endpoint",
    ...
    # standalone=True → script is visible (not internal), even though action.internal=True
    assert not script_dict.get("internal")
    # action itself is still hidden
    assert action_dict.get("internal") is True


def test_strict_schema_rejects_both_underlyingcontentitem_and_script():
    """
    Given:
        - An AgentixAction strict schema dict with BOTH underlyingcontentitem AND script.
    When:
        - Constructing the strict AgentixAction model.
    Then:
        - pydantic.ValidationError is raised (mutual exclusion).
    """
    import pydantic
    ...
    with pytest.raises(pydantic.ValidationError, match="Cannot provide both"):
        StrictAgentixAction(
            **{
