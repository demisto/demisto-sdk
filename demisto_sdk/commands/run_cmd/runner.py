            ...
            if not self.prefix:
                ...
                if raw_output_json:
                    with tempfile.NamedTemporaryFile(mode='w+') as f:
                        ...
                        json_to_outputs(
                            command, json=file_path, prefix=self.prefix
                        )
                else:
                    print_error(
                ...
                with open(result, 'r+') as log_info:
                    for line in log_info:
                        if self.SECTIONS_HEADER_REGEX.match(line):
                            ...
                            print_color(
                                'Full Integration Log:', LOG_COLORS.YELLOW
                            )
                        else:
                            print(line)
            ...
            with open(self.debug_path, 'w+b') as output_file:
                for log_id in log_ids:
                    ...
                    with open(result, 'r+') as log_info:
                        for line in log_info:
                            if (
                                self.RAW_RESPONSE_HEADER.match(line)
                                and not temp_dict
                            ):
                                output_file.write(line.encode('utf-8'))
                                line = log_info.readline()
                            ...
                            if (
                                ...
                                output_file.write(line.encode('utf-8'))
                                line = log_info.readline()
                                while not self.HUMAN_READABLE_HEADER.match(
                                    line
                                ):
                                    output_file.write(line.encode('utf-8'))
                                    context = context + line
