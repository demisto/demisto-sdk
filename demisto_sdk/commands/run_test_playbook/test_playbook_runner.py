...
class TestPlaybookRunner:
    ...
    def get_all_test_playbooks(self):
        ...
        for pack in packs_list:
            if os.path.isdir(f'Packs/{pack}'):
                tpb_list.extend(
                    self.get_test_playbooks_from_folder(f'Packs/{pack}')
                )
        return tpb_list
    ...
    def run_test_playbook_by_id(self, test_playbook_id):
        ...
        if self.should_wait:
            ...
            print_color(
                f'To see results please go to : {work_plan_link}',
                LOG_COLORS.NATIVE,
            )

        return status_code

    def run_and_check_tpb_status(
        self, test_playbook_id, work_plan_link, incident_id
    ):
        status_code = SUCCESS_RETURN_CODE
        print_color(
        ...
        while elapsed_time < self.timeout:
            test_playbook_result = self.get_test_playbook_results_dict(
                incident_id
            )
        ...
        if elapsed_time >= self.timeout:
            ...
            if test_playbook_result['state'] == 'failed':
                self.print_tpb_error_details(
                    test_playbook_result, test_playbook_id
                )
                print_color(
    ...
    def create_incident_with_test_playbook(
        self, incident_name, test_playbook_id
    ):
        # type: (str, str) -> int
        ...
        if entries:
            ...
            for entry in entries:
                if (
                    ...
                    print_color(
                        f'- Task ID: {entry["taskId"]}', LOG_COLORS.RED
                    )
