...
class SecretsValidator:
    ...
    def get_secrets(self, branch_name, is_circle):
        ...
        if not run_command("git rev-parse -q --verify MERGE_HEAD") or self.input_paths:
            ...
            if secret_to_location_mapping:
                ...
                secho_and_info(secrets_found_string, "red")
    ...
    def remove_whitelisted_items_from_file(
        ...
        for item in secrets_white_list:
            try:
                ...
                secho_and_info(error_string, "red")
    ...
    def find_secrets(self):
        secho_and_info("Starting secrets detection", "green")
        ...
        if secrets_found:
            ...
            secho_and_info(
                "Finished validating secrets, no secrets were found.", "green"
            )
            return False
