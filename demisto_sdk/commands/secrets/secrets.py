...
class SecretsValidator(object):
    ...
    def get_secrets(self, branch_name, is_circle):
        ...
        if (
            not run_command('git rev-parse -q --verify MERGE_HEAD')
            or self.input_paths
        ):
            ...
            if secret_to_location_mapping:
                ...
                for file_name in secret_to_location_mapping:
                    for line in sorted(secret_to_location_mapping[file_name]):
                        ...
                        if (
                            len(secret_to_location_mapping[file_name][line])
                            == 1
                        ):
    ...
    def get_diff_text_files(self, files_string):
        ...
        for file_name in all_files:
            ...
            if (
                ...
                if not any(
                    skipped_file in file_path for skipped_file in SKIPPED_FILES
                ):
    ...
    def search_potential_secrets(
        ...
        """
        secret_to_location_mapping: DefaultDict[
            str, defaultdict
        ...
        for file_path in secrets_file_paths:
            ...
            for line_num, line in enumerate(file_contents.split('\n')):
                ...
                for regex_secret in regex_secrets:
                    if not any(
                        ioc.lower() in regex_secret.lower()
    ...
    def create_temp_white_list(file_contents) -> set:
        ...
        for context_path in context_paths:
            ...
            context_path = [
                white_item.lower()
                for white_item in context_path
                if len(white_item) > 4
            ]
    ...
    def retrieve_related_yml(integration_path):
        ...
        if os.path.exists(yml_file):
            with io.open(
                yml_file, mode='r', encoding='utf-8'
            ) as matching_yml_file:
    ...
    def get_white_listed_items(self, is_pack, pack_name):
        ...
        if is_pack:
            ...
            (
                pack_white_list,
                _,
                pack_files_white_list,
            ) = self.get_packs_white_list(pack_whitelist_path, pack_name)
    ...
    def get_file_contents(self, file_path, file_extension):
        try:
            ...
            print(
                'Failed opening file: {}. Exception: {}'.format(file_path, ex)
            )
            raise
