...
class TestSecrets:
    ...
    validator = SecretsValidator(
        is_circle=True,
        white_list_path=os.path.join(FILES_PATH, WHITE_LIST_FILE_NAME),
    )
    ...
    def teardown_class(cls):
        ...
        if os.path.exists(TestSecrets.TEMP_DIR):
            shutil.rmtree(
                TestSecrets.TEMP_DIR, ignore_errors=False, onerror=None
            )
...
"""
    ...
    def test_remove_whitelisted_items_from_file_substring(self):
        ...
        assert (
            white_list
            not in self.validator.remove_whitelisted_items_from_file(
                file_contents, {white_list}
            )
        )
    ...
    def test_remove_secrets_disabled_line(self):
        ...
        """
        file_contents = self.validator.remove_secrets_disabled_line(
            file_contents
        )
        ...
        8.8.8.4
        """
        file_contents1 = self.validator.remove_secrets_disabled_line(
            file_contents1
        )
