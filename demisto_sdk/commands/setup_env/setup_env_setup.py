...
import typer
from demisto_sdk.commands.setup_env.setup_environment import setup_env, IDEType
...
def setup_env_command(
    ...
    """Setup the integration environments."""

    if ide.lower() == "auto-detect":
        if (Path("CONTENT_PATH") / ".vscode").exists():
            typer.echo("Visual Studio Code IDEType has been detected and will be configured.")
            ide_type = IDEType.VSCODE
        elif (Path("CONTENT_PATH") / ".idea").exists():
            typer.echo("PyCharm / IDEA IDEType has been detected and will be configured.")
