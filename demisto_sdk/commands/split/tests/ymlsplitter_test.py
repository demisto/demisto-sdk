...
def test_extract_to_package_format_modeling_rule(tmpdir):
    ...
    # check code
    with open(
        out.join("OktaModelingRule").join("OktaModelingRule.xif"), "r", encoding="utf-8"
    ) as f:
        file_data = f.read()
...
def test_extract_to_package_format_pwsh(tmpdir):
    ...
    with open(
        out.join("PowerShellRemotingOverSSH").join("PowerShellRemotingOverSSH.yml"), "r"
    ) as f:
        yaml_obj = yaml.load(f)
...
def test_extract_to_package_format_py(pack, mocker, tmp_path):
    ...
    mocker.patch(
        "demisto_sdk.commands.split.ymlsplitter.get_python_version", return_value="2.7"
    )
    mocker.patch(
