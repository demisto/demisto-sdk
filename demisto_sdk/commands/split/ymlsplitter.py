...
INTEGRATIONS_DOCS_REFERENCE = (
    'https://xsoar.pan.dev/docs/reference/integrations/'
)
...
class YmlSplitter:
    ...
    def __init__(
        ...
        no_basic_fmt: bool = False,
        new_module_file: bool = False,
    ):
        self.input = Path(input).resolve()
        self.output = (
            Path(output) if output else Path(self.input.parent)
        ).resolve()
    ...
    def get_output_path(self):
        ...
        if self.autocreate_dir and output_path.name in {
            'Integrations',
            'Scripts',
            'ModelingRules',
            'ParsingRules',
    ...
    def extract_to_package_format(self) -> int:
        ...
        try:
            ...
            return 1
        self.print_logs(
            'Starting migration of: {} to dir: {}'.format(
        ...
        self.print_logs(
            'Creating yml file: {} ...'.format(yaml_out),
            log_color=LOG_COLORS.NATIVE,
        )
        ...
        if self.file_type in ('modelingrule', 'parsingrule'):
            ...
            if code_type == TYPE_PYTHON:
                if self.basic_fmt:
                    self.print_logs(
                        'Running autopep8 on file: {} ...'.format(code_file),
                        log_color=LOG_COLORS.NATIVE,
                    )
                    try:
                        subprocess.call(
                ...
                if self.pipenv:
                    try:
                        ...
                        py_ver = get_python_version(
                            docker, self.config.log_verbose
                        )
                        pip_env_dir = get_pipenv_dir(
                            py_ver, self.config.envs_dirs_base
                        )
                        self.print_logs(
                            'Copying pipenv files from: {}'.format(
                                pip_env_dir
                            ),
                            log_color=LOG_COLORS.NATIVE,
                        )
                        shutil.copy(
                            '{}/Pipfile'.format(pip_env_dir), output_path
                        )
                        shutil.copy(
                            '{}/Pipfile.lock'.format(pip_env_dir), output_path
                        )
                        env = os.environ.copy()
                        ...
                        try:
                            ...
                            self.print_logs(
                                'Installing all py requirements from docker: [{}] into pipenv'.format(
                                    docker
                                ),
                                LOG_COLORS.NATIVE,
                            )
                        ...
                        if found_readme:
                            next_steps += '    git rm {}\n'.format(
                                self.input.parent
                                / f'{self.input.stem}_README.md'
                            )
                        next_steps += '    git add {}\n'.format(arg_path)
                        self.print_logs(
                            next_steps, log_color=LOG_COLORS.NATIVE
                        )
    ...
    def extract_code(self, code_file_path) -> int:
        ...
        self.print_logs(
            'Extracting code to: {} ...'.format(code_file_path),
            log_color=LOG_COLORS.NATIVE,
        )
    ...
    def extract_image(self, output_path) -> int:
        ...
        self.print_logs(
            'Extracting image to: {} ...'.format(output_path),
            log_color=LOG_COLORS.NATIVE,
        )
    ...
    def remove_integration_documentation(self, detailed_description):
        if '[View Integration Documentation]' in detailed_description:
            ...
            documentation = (
                f'[View Integration Documentation]({integration_doc_link})'
            )
    ...
    def extract_long_description(self, output_path) -> int:
        ...
        if long_description:
            long_description = self.remove_integration_documentation(
                long_description
            )
            self.print_logs(
                'Extracting long description to: {} ...'.format(output_path),
                log_color=LOG_COLORS.NATIVE,
            )
    ...
    def extract_rules(self, output_path) -> int:
        ...
        if rules:
            self.print_logs(
                'Extracting rules to: {} ...'.format(output_path),
                log_color=LOG_COLORS.NATIVE,
            )
    ...
    def extract_rule_schema_and_samples(self, output_path) -> int:
        ...
        if schema:
            self.print_logs(
                'Extracting rules schema to: {} ...'.format(output_path),
                log_color=LOG_COLORS.NATIVE,
            )
            with open(output_path, 'w', encoding='utf-8') as rules_file:
        ...
        if samples:
            self.print_logs(
                'Extracting rules samples to: {} ...'.format(output_path),
                log_color=LOG_COLORS.NATIVE,
            )
            with open(output_path, 'w', encoding='utf-8') as rules_file:
    ...
    def replace_imported_code(self, script):
        ...
        if (
            ...
            for match in matches:
                ...
                self.print_logs(
                    f'Replacing code block with `{imported_line}`',
                    LOG_COLORS.NATIVE,
                )
