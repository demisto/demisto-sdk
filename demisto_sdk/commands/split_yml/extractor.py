...
class Extractor:
    ...
    def get_output_path(self):
        ...
        if self.autocreate_dir and (
            output_path.endswith("Integrations") or output_path.endswith("Scripts")
        ):
            code_name = self.yml_data.get("name")
            if not code_name:
                raise ValueError(
                    f"Failed determining Integration/Script name when trying to auto create sub dir at: {output_path}"
    ...
    def extract_to_package_format(self) -> int:
        ...
        try:
            ...
            return 1
        self.print_logs(
            "Starting migration of: {} to dir: {}".format(self.input, output_path),
            log_color=LOG_COLORS.NATIVE,
        )
        os.makedirs(output_path, exist_ok=True)
        base_name = (
            os.path.basename(output_path) if not self.base_name else self.base_name
        )
        ...
        lang_type: str = script[
            "type"
        ] if self.file_type == "integration" else self.yml_data["type"]
        ...
        self.print_logs(
            "Creating yml file: {} ...".format(yaml_out), log_color=LOG_COLORS.NATIVE
        )
        ryaml = YAML()
        ...
        if code_type == TYPE_PYTHON:
            ...
            if self.pipenv:
                self.print_logs(
                    "Running isort on file: {} ...".format(code_file), LOG_COLORS.NATIVE
                )
                try:
                ...
                self.print_logs(
                    "Copying pipenv files from: {}".format(pip_env_dir),
                    log_color=LOG_COLORS.NATIVE,
                )
                ...
                try:
                    ...
                    os.unlink(fp.name)
                    self.print_logs(
                        "Installing flake8 for linting", log_color=LOG_COLORS.NATIVE
                    )
                    subprocess.call(
                        ["pipenv", "install", "--dev", "flake8"], cwd=output_path
                    )
                except FileNotFoundError:
                    self.print_logs(
    ...
    def extract_code(self, code_file_path) -> int:
        ...
        if common_server:
            common_server = (
                "CommonServerPython" not in self.input
                and "CommonServerPowerShell" not in self.input
            )
