...
class Docker:
    ...
    def _build_stats_cmd(cls, server_ip, docker_images):
        ...
        docker_command = (
            'sudo docker stats --no-stream --no-trunc --format "{}"'.format(
                cls.COMMAND_FORMAT
            )
        )
    ...
    def _parse_stats_result(cls, stats_lines, logging_module=logging):
        ...
        try:
            containers_stats = [
                json.loads(c) for c in stats_lines.splitlines()
            ]

            for container_stat in containers_stats:
                memory_usage_stats = (
                    container_stat.get(cls.MEMORY_USAGE, '')
                    .split('/')[0]
                    .lower()
                )
                ...
                if 'kib' in memory_usage_stats:
                    mib_usage = (
                        float(memory_usage_stats.replace('kib', '').strip())
                        / 1024
                    )
                    ...
                    mib_usage = float(
                        memory_usage_stats.replace('mib', '').strip()
                    )
                ...
                stats_result.append(
                    {
                        ...
                        'container_name': container_stat.get(
                            cls.CONTAINER_NAME
                        ),
    ...
    def get_docker_pid_info(cls, server_ip, container_id, logging_module):
        ...
        if stderr:
            ignored_warning_message = 'Connection to {} closed'.format(
                server_ip
            )
    ...
    def check_resource_usage(
        ...
        containers_stats = cls.docker_stats(
            server_ip, docker_images, logging_module
        )
        error_message = ''
        ...
        for container_stat in containers_stats:
            ...
            image_name = image_full.split(':')[
                0
            ]  # just the name such as demisto/slack
