...
from demisto_sdk.commands.test_content.constants import (LOAD_BALANCER_IP,
                                                         SSH_USER)
...
class TestContext:
    def __init__(self,
        ...
        self.tunnel_command = \
            f'ssh -i ~/.ssh/oregon-ci.pem -4 -o StrictHostKeyChecking=no -f -N "{SSH_USER}@{LOAD_BALANCER_IP}" ' \
    ...
    def _notify_failed_test(self):
        text = f'{self.build_context.build_name} - {self.playbook} Failed\n' \
               f'for more details run: `{self.tunnel_command}` and enter the following link\n' \
