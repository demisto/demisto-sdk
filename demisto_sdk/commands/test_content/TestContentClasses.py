    ...
    def create_incident(self, client: DefaultApi) -> Optional[Incident]:
        ...
        while found_incidents < 1:
            ...
            if time.time() > timeout:
                if IS_XSIAM:
                    self.build_context.logging_module.error(f'Got timeout for searching incident with id '
                                                            f'{incident_name}')
                else:
                    self.build_context.logging_module.error(f'Got timeout for searching incident name {inc_id}')
