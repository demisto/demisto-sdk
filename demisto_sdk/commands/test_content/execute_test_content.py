...
def _add_pr_comment(comment, logging_module):
    ...
    query = '?q={}+repo:demisto/content+org:demisto+is:pr+is:open+head:{}+is:open'.format(
        sha1, branch_name
    )
    ...
    is_skipped_tests_flow = (
        'The following integrations/tests were collected' in comment
    )
        ...
        if res and res.get('total_count', 0) == 1:
            ...
            if issue_url:
                ...
                response = requests.get(
                    issue_url, headers=headers, verify=False
                )
                issue_comments = _handle_github_response(
                    response, logging_module
                )
                for existing_comment in issue_comments:
                    if (
                        is_skipped_tests_flow
                        and SKIPPED_CONTENT_COMMENT
                        in existing_comment.get('body', '')
                    ) or (
                        not is_skipped_tests_flow
                        and COVERAGE_REPORT_COMMENT
                        in existing_comment.get('body', '')
                    ):
                        comment_url = existing_comment.get('url')
                        requests.delete(
                            comment_url, headers=headers, verify=False
                        )
...
def execute_test_content(**kwargs):
    ...
    for server_ip, port in build_context.instances_ips.items():
        ...
        threads_list.append(
            Thread(target=tests_execution_instance.execute_tests)
        )

    logging_manager.info(
        'Finished creating configurations, starting to run tests.'
    )
