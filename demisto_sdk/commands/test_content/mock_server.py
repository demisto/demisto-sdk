...
class MITMProxy:
    ...
    def reset_mock_files(self):
        ...
        try:
            output = self.ami.check_output("cd content-test-data && git reset --hard".split(), stderr=STDOUT)
    ...
    def configure_proxy_in_demisto(self, username, password, server, api_key=None, auth_id=None, proxy=""):
        ...
        response = demisto_client.generic_request_func(self=client, path="/system/config", method="POST", body=data)
    ...
    def has_mock_file(self, playbook_or_integration_id):
        command = [
            "[",
            "-f",
            os.path.join(self.current_folder, get_mock_file_path(playbook_or_integration_id)),
            "]",
        ]
    ...
    def has_mock_folder(self, playbook_or_integration_id):
        command = [
            "[",
            "-d",
            os.path.join(self.current_folder, get_folder_path(playbook_or_integration_id)),
            "]",
        ]
        return self.ami.call(command) == 0
    ...
    def normalize_mock_file(self, playbook_or_integration_id: str, path: str = None):
        ...
        try:
            ...
            self.logging_module.debug("There may have been a problem while normalizing the mock file.")
    ...
    def prepare_proxy_start(self, path: str, playbook_or_integration_id: str, record: bool) -> bool:
        """
...
@contextmanager
def run_with_mock(proxy_instance: MITMProxy, playbook_or_integration_id: str, record: bool = False) -> Iterator[dict]:
    """
