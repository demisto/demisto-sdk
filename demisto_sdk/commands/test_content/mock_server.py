...
class AMIConnection:
    ...
    def copy_file(self, src, dst=REMOTE_HOME, **kwargs):
        silence_output(
            check_call,
            ["scp", src, f"{SSH_USER}@{self.internal_ip}:{dst}"],
            stdout="null",
            **kwargs,
        )
...
class MITMProxy:
    ...
    def __init__(
        ...
        is_branch_master = branch_name == DEMISTO_DEFAULT_BRANCH
        self.internal_ip = internal_ip
    ...
    def reset_mock_files(self):
        ...
        try:
            output = self.ami.check_output(
                "cd content-test-data && git reset --hard".split(), stderr=STDOUT
            )
            self.logging_module.debug(
    ...
    def configure_proxy_in_demisto(
        ...
        system_conf.update({"http_proxy": http_proxy, "https_proxy": https_proxy})
    ...
    def has_mock_file(self, playbook_or_integration_id):
        command = [
            ...
            "-f",
            os.path.join(
                self.current_folder, get_mock_file_path(playbook_or_integration_id)
            ),
            "]",
        ]
    ...
    def has_mock_folder(self, playbook_or_integration_id):
        command = [
            ...
            "-d",
            os.path.join(
                self.current_folder, get_folder_path(playbook_or_integration_id)
            ),
            "]",
        ]
        return self.ami.call(command) == 0
    ...
    def normalize_mock_file(self, playbook_or_integration_id: str, path: str = None):
        ...
        try:
            ...
            err_msg = (
                f'{err_msg} and the output of "{e.output}"' if e.output else err_msg
            )
            if e.stderr:
    ...
    def prepare_proxy_start(
        ...
        silence_output(
            self.ami.call, ["mkdir", os.path.join(path, folder_path)], stderr="null"
        )
...
@contextmanager
def run_with_mock(
    proxy_instance: MITMProxy, playbook_or_integration_id: str, record: bool = False
) -> Iterator[dict]:
    """
