...
def clean_filename(
    playbook_or_integration_id, whitelist=VALID_FILENAME_CHARS, replace=' ()'
):
...
class AMIConnection:
    ...
    def add_ssh_prefix(self, command, ssh_options=''):
        ...
        prefix = 'ssh {} {}@{}'.format(
            ssh_options, SSH_USER, self.internal_ip
        ).split()
    ...
    def copy_file(self, src, dst=REMOTE_HOME, **kwargs):
        silence_output(
            check_call,
            ['scp', src, '{}@{}:{}'.format(SSH_USER, self.internal_ip, dst)],
            stdout='null',
            **kwargs,
        )
    ...
    def run_script(self, script, *args):
        ...
        """
        remote_script_path = self.copy_file(
            os.path.join(self.LOCAL_SCRIPTS_DIR, script)
        )

        silence_output(
            self.check_call, ['chmod', '+x', remote_script_path], stdout='null'
        )
        silence_output(
            self.check_call, [remote_script_path] + list(args), stdout='null'
        )
...
class MITMProxy:
    ...
    def __init__(
        ...
        silence_output(
            self.ami.call, ['mkdir', '-p', tmp_folder], stderr='null'
        )
        script_filepath = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), 'timestamp_replacer.py'
        )
    ...
    def configure_proxy_in_demisto(
        ...
        self.logging_module.debug(
            f'Server configurations before proxy server configurations:\n{pformat(system_conf)}'
        )
    ...
    def has_mock_file(self, playbook_or_integration_id):
        command = [
            ...
            os.path.join(
                self.current_folder,
                get_mock_file_path(playbook_or_integration_id),
            ),
            ']',
        ]
    ...
    def has_mock_folder(self, playbook_or_integration_id):
        command = [
            ...
            os.path.join(
                self.current_folder,
                get_folder_path(playbook_or_integration_id),
            ),
            ']',
        ]
        return self.ami.call(command) == 0
    ...
    def normalize_mock_file(
        self, playbook_or_integration_id: str, path: str = None
    ):
        ...
        self.logging_module.debug(
            f'problem_keys_filepath="{problem_keys_filepath}"'
        )
        problem_key_file_exists = ['[', '-f', problem_keys_filepath, ']']
        if not self.ami.call(problem_key_file_exists) == 0:
            self.logging_module.debug(
                'Error: The problematic_keys.json file was not written to the file path'
        ...
        self.logging_module.debug(
            f'command to normalize mockfile:\n\t{command}'
        )
        ...
        try:
            ...
            err_msg = (
                f'command `{command}` exited with return code [{e.returncode}]'
            )
            err_msg = (
                f'{err_msg} and the output of "{e.output}"'
                if e.output
                else err_msg
            )
            if e.stderr:
        ...
        try:
            diff_cmd_output = (
                self.ami.check_output(diff_cmd.split()).decode().strip()
            )
    ...
    def start(
        self, playbook_or_integration_id, path=None, record=False
    ) -> None:
        ...
        if self.is_proxy_listening():
            self.logging_module.debug(
                'proxy service is already running, stopping it'
            )
            self.ami.call(['sudo', 'systemctl', 'stop', 'mitmdump'])
        self.logging_module.debug(
            f'Attempting to start proxy in {self.get_script_mode(record)} mode'
        )
    ...
    def _start_proxy_and_wait_until_its_up(self, is_record: bool) -> None:
        ...
        if was_proxy_up:
            self.logging_module.debug(
                f'Proxy service started in {self.get_script_mode(is_record)} mode'
            )
        else:
            self.logging_module.error(
                f'Proxy failed to start after {self.TIME_TO_WAIT_FOR_PROXY_SECONDS} seconds'
            )
    ...
    def prepare_proxy_start(
        self, path: str, playbook_or_integration_id: str, record: bool
    ) -> bool:
        """
        ...
        repo_problem_keys_path = os.path.join(
            self.repo_folder, folder_path, 'problematic_keys.json'
        )
        current_problem_keys_path = os.path.join(
            path, folder_path, 'problematic_keys.json'
        )
        log_file_path = os.path.join(
            path, get_log_file_path(playbook_or_integration_id, record)
        )
        mock_file_path = os.path.join(
            path, get_mock_file_path(playbook_or_integration_id)
        )
        ...
        silence_output(
            self.ami.call,
            ['mkdir', os.path.join(path, folder_path)],
            stderr='null',
        )
        ...
        if record:
            try:
                ...
                self.logging_module.debug(
                    f'Failed to move problematic_keys.json with exit code {e.returncode}'
                )

        return self._write_mitmdump_rc_file_to_host(file_content)

    def _write_mitmdump_rc_file_to_host(self, file_content: str) -> bool:
        """
        ...
        try:
            self.ami.call(
                [
                    ...
                    f"'{file_content}'",
                    '>',
                    os.path.join(AMIConnection.REMOTE_HOME, 'mitmdump_rc'),
                ]
            )
            return True
        except CalledProcessError:
...
def run_with_mock(
    ...
    if record:
        ...
        if proxy_instance.should_update_mock_repo:
            proxy_instance.logging_module.debug(
                'Cleaning content-test-data repo'
            )
    ...
    try:
        ...
        proxy_instance.logging_module.exception(
            'Unexpected failure in proxy context manager'
        )
    finally:
        proxy_instance.stop()
        if record:
            if result_holder.get(RESULT):
                ...
                proxy_instance.move_mock_file_to_repo(
                    playbook_or_integration_id
                )
                proxy_instance.successful_rerecord_count += 1
                proxy_instance.rerecorded_tests.append(
                    playbook_or_integration_id
                )
                if proxy_instance.should_update_mock_repo:
                    proxy_instance.logging_module.debug(
                ...
                proxy_instance.failed_rerecord_tests.append(
                    playbook_or_integration_id
                )
