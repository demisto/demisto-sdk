...
class SetFakeXsiamClientEnvironmentVars:
    def __init__(self, demisto_base_url: str = "https://api-fake.com",
                 demisto_api_key: str = 'fake-api-key', xsiam_auth_id: str = 'fake-auth-id',
                 xsiam_token: str = 'fake-token', collector_token: str = 'fake-collector-token'):
...
class TestTheTestModelingRuleCommandSingleRule:
        ...
        try:
            with requests_mock.Mocker() as m:
                with SetFakeXsiamClientEnvironmentVars() as fake_env_vars:
                    # Arrange
                    m.get(f'{fake_env_vars.demisto_base_url}/xsoar/contentpacks/metadata/installed', json=[])
                    # Act
                    result = runner.invoke(test_modeling_rule_cmd, [mrule_dir.as_posix(), '--non-interactive'])
                    # Assert
    ...
    def test_the_test_modeling_rule_command_fail_to_push_test_data(self, pack):
        ...
        try:
            with requests_mock.Mocker() as m:
                with SetFakeXsiamClientEnvironmentVars() as fake_env_vars:
                    # Arrange
                    m.get(f'{fake_env_vars.demisto_base_url}/xsoar/contentpacks/metadata/installed',
                          json=[{'name': pack.name, 'id': pack.name}])
                    m.post(f'{fake_env_vars.demisto_base_url}/logs/v1/xsiam',
                           status_code=500)
                    # Act
                    result = runner.invoke(test_modeling_rule_cmd, [mrule_dir.as_posix(), '--non-interactive'])
                    # Assert
    ...
    def test_the_test_modeling_rule_command_fail_to_check_dataset_exists(self, pack, monkeypatch):
        ...
        func_path = ('demisto_sdk.commands.test_content.test_modeling_rule.'
        ...
        try:
            with requests_mock.Mocker() as m:
                with SetFakeXsiamClientEnvironmentVars() as fake_env_vars:
                    ...
                    m.get(f'{fake_env_vars.demisto_base_url}/xsoar/contentpacks/metadata/installed',
                          json=[{'name': pack.name, 'id': pack.name}])
                    m.post(f'{fake_env_vars.demisto_base_url}/logs/v1/xsiam', json={},
                           status_code=200)
                    m.post(f'{fake_env_vars.demisto_base_url}/public_api/v1/xql/start_xql_query/',
            ...
            with requests_mock.Mocker() as m:
                with SetFakeXsiamClientEnvironmentVars() as fake_env_vars:
                    ...
                    m.post(
                        ...
                        [
                            {'json': {'reply': 'fake-execution-id'}, 'status_code': 200},
                            {'json': {}, 'status_code': 500},
                        ]
                    )
                    m.post(
                        ...
                        json={
                            'reply': {'status': 'SUCCESS', 'results': {'data': ['some-results']}}
                        },
                        status_code=200
                    )
                    # Act
            ...
            with requests_mock.Mocker() as m:
                with SetFakeXsiamClientEnvironmentVars() as fake_env_vars:
                    ...
                    m.post(
                        ...
                        [
                            {
                                'json': {'reply': {'status': 'SUCCESS', 'results': {'data': ['some-results']}}},
                                'status_code': 200
                            },
                            {'json': {}, 'status_code': 500},
                        ]
                ...
                with SetFakeXsiamClientEnvironmentVars() as fake_env_vars:
                    ...
                    m.get(
                        ...
                        [
                            ...
                            {'json': [{'name': pack_2.name, 'id': pack_2.name}], 'status_code': 200},
                        ]
                    )
                    m.post(f'{fake_env_vars.demisto_base_url}/logs/v1/xsiam', json={},
                           status_code=200)
                    ...
                    result = runner.invoke(
                        ...
                        [mrule_dir_1.as_posix(), mrule_dir_2.as_posix(), '--non-interactive']
                    )
                    # Assert
                    assert result.exit_code == 1
