...
def get_mocked_build_context(
    ...
    conf_path.write_text(
        json.dumps(content_conf_json or generate_content_conf_json())
    )
...
def test_non_filtered_tests_are_skipped(mocker, tmp_path):
    ...
    assert (
        'test_that_should_be_skipped'
        in build_context.tests_data_keeper.skipped_tests
    )
    assert (
        'test_that_should_run'
        not in build_context.tests_data_keeper.skipped_tests
    )
...
def test_nightly_playbook_skipping(mocker, tmp_path):
    ...
    tests = [
        generate_test_configuration(
            playbook_id='nightly_playbook', nightly=True
        )
    ]
    content_conf_json = generate_content_conf_json(tests=tests)
    build_context = get_mocked_build_context(
    ...
    assert (
        'nightly_playbook' not in build_context.tests_data_keeper.skipped_tests
    )
...
def test_playbook_with_nightly_integration_skipping(mocker, tmp_path):
    ...
    tests = [
        generate_test_configuration(
            playbook_id='playbook_with_nightly_integration',
            integrations=['nightly_integration'],
        )
    ]
    content_conf_json = generate_content_conf_json(
        tests=tests, nightly_integrations=['nightly_integration']
    )
    build_context = get_mocked_build_context(
        mocker,
        tmp_path,
    ...
    assert (
        'playbook_with_nightly_integration'
        in build_context.tests_data_keeper.skipped_tests
    )
    build_context = get_mocked_build_context(
        mocker,
...
def test_get_public_ip_from_server_url(mocker, tmp_path):
    ...
    assert (
        'Could not find private ip for the server mapped to port 4446'
