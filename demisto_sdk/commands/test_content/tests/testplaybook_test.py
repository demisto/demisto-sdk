...
def test_set_prev_server_keys(mocker, tmp_path):
    ...
    assert test_playbook._set_prev_server_keys(
        mocked_demisto_client, server_context
    )
...
@pytest.mark.parametrize(
    ...
    [(True, (None, 200, None)), (False, (None, 400, None))],
)
def test_close_incident(mocker, tmp_path, expected_res, client_res):
    ...
    test_playbook_configuration = TestConfiguration({}, 0)
    mocker.patch.object(
        demisto_client, 'generic_request_func', return_value=client_res
    )

    test_playbook = TestPlaybook(build_context, test_playbook_configuration)
    res = test_playbook.close_incident(
        client=mocked_demisto_client, incident_id='1'
    )
