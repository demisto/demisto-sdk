...
class TimestampReplacer:
    ...
    def _debug_request(self, flow: HTTPFlow) -> None:
        ...
        logging.info(f'hashed_data={ServerPlayback()._hash(flow)}')
    ...
    def replace_boundary(req: Request):
        ...
        req.content = req.content and req.content.replace(old_boundary.encode(), fixed_boundary.encode())
    ...
    def clean_json_body(self, req: Request) -> None:
        ...
        if req.method == 'POST':
            ...
            if json_data:
                try:
                    content_dict = OrderedDict(literal_eval(content))
                    self.modify_json_body(req, content_dict)
                    return
                except Exception:
    ...
    def handle_json_body(self, req: Request) -> None:
        ...
        if req.method == 'POST':
            ...
            if json_data:
                try:
                    content_dict = OrderedDict(literal_eval(content))
                    json_keys = self.determine_problematic_keys(content_dict)
