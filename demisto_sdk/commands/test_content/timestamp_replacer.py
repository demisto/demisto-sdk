...
class TimestampReplacer:
    ...
    def request(self, flow: HTTPFlow) -> None:
        ...
        if ctx.options.script_mode == "record":
            if ctx.options.detect_timestamps:
                ...
                logging.info(
                    'updating problem_keys file at "{}"'.format(self.bad_keys_filepath)
                )
    ...
    def read_in_problematic_keys(self):
        ...
        logging.info(
            'reading in problematic keys data from "{}"'.format(repo_bad_keys_filepath)
        )
    ...
    def load_problematic_keys(self):
        ...
        if path.exists(self.bad_keys_filepath):
            logging.info(
                '"{}" path exists - loading bad keys'.format(self.bad_keys_filepath)
            )
