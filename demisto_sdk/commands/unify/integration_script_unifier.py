...
class IntegrationScriptUnifier(YAMLUnifier):
    ...
    def write_yaml_with_docker(self, yml_unified, yml_data, script_obj):
        ...
        if "dockerimage45" in script_obj:
            ...
            if (
                server_version_compare(
                    yml_data.get("fromversion", DEFAULT_CONTENT_ITEM_FROM_VERSION),
                    "5.0.0",
                )
                >= 0
            ):
            ...
            if (
                server_version_compare(
                    yml_data.get("toversion", DEFAULT_CONTENT_ITEM_TO_VERSION), "5.0.0"
                )
                < 0
            ):
                raise ValueError(
    ...
    def insert_image_to_yml(self, yml_data, yml_unified):
        ...
        if image_data:
            ...
            click.secho(
                f"Failed getting image data for {self.package_path}", fg="yellow"
            )
    ...
    def get_code_file(self, script_type):
        ...
        if self.package_path.endswith(
            os.path.join("Scripts", "CommonServerUserPython")
        ):
        ...
        if self.package_path.endswith(
            os.path.join("Scripts", "CommonServerUserPowerShell")
        ):
