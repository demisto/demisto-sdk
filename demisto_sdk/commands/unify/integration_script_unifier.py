...
class IntegrationScriptUnifier(YAMLUnifier):
    ...
    def unify(self, file_name_suffix=None):
        ...
        if not self.is_script_package:
            ...
            if self.is_contributor_pack(contributor_type):
                ...
                yml_unified = self.add_contributors_support(
                    yml_unified,
                    contributor_type,
                    contributor_email,
                    contributor_url,
        ...
        output_map = self.write_yaml_with_docker(
            yml_unified, self.yml_data, script_obj
        )
        unifier_outputs = (
            list(output_map.keys()),
            self.yml_path,
            script_path,
            image_path,
            desc_path,
        ...
        print_color(
            f'Created unified yml: {list(output_map.keys())}', LOG_COLORS.GREEN
        )
    ...
    def update_hidden_parameters_value(self) -> None:
        ...
        for i, param in enumerate(self.yml_data.get('configuration', ())):
            if isinstance(hidden := (param.get('hidden')), list):
                ...
                self.yml_data['configuration'][i]['hidden'] = (
                    self.marketplace in hidden
                )
    ...
    def insert_image_to_yml(self, yml_data, yml_unified):
        ...
        if image_data:
            image_data = self.image_prefix + base64.b64encode(
                image_data
            ).decode('utf-8')
            ...
            click.secho(
                f'Failed getting image data for {self.package_path}',
                fg='yellow',
            )
    ...
    def insert_description_to_yml(self, yml_data, yml_unified):
        desc_data, found_desc_path = self.get_data(
            self.package_path, '*_description.md'
        )
    ...
    def get_code_file(self, script_type):
        ...
        if self.package_path.endswith(
            os.path.join('Scripts', 'CommonServerPython')
        ):
            return os.path.join(self.package_path, 'CommonServerPython.py')
        if self.package_path.endswith(
            os.path.join('Scripts', 'CommonServerPowerShell')
        ):
            return os.path.join(
                self.package_path, 'CommonServerPowerShell.ps1'
            )
        ...
        script_path = list(
            filter(
                ...
                sorted(
                    glob.glob(
                        os.path.join(self.package_path, '*' + script_type)
                    )
                ),
            )
        )[0]
    ...
    def insert_module_code(
        script_code: str, import_to_name: Dict[str, str]
    ) -> str:
        """
    ...
    def _get_api_module_code(module_name, module_path):
        ...
        try:
            with io.open(
                module_path, mode='r', encoding='utf-8'
            ) as script_file:
    ...
    def get_contributor_data(self):
        ...
        pack_metadata_data, pack_metadata_path = self.get_data(
            pack_path, PACK_METADATA_PATH
        )
    ...
    def add_contributors_support(
        ...
        existing_detailed_description = unified_yml.get(
            'detaileddescription', ''
        )
        if contributor_type == COMMUNITY_CONTRIBUTOR:
            contributor_description = (
                CONTRIBUTOR_COMMUNITY_DETAILED_DESC.format(author)
            )
        else:
    ...
    def get_integration_doc_link(self, unified_yml: Dict) -> str:
        ...
        """
        normalized_integration_id = self.normalize_integration_id(
    ...
    def normalize_integration_id(integration_id: str) -> str:
        ...
        """
        dasherized_integration_id = dasherize(
            underscore(integration_id)
        ).replace(' ', '-')
