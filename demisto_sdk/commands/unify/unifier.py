...
class Unifier:
    def __init__(
        ...
        self.ryaml.width = (
            50000  # make sure long lines will not break (relevant for code section)
        )
        if self.yml_path:
    ...
    def write_yaml_with_docker(self, yml_unified, yml_data, script_obj):
        ...
        if "dockerimage45" in script_obj:
            ...
            if (
                server_version_compare(yml_data.get("fromversion", "0.0.0"), "5.0.0")
                >= 0
            ):
                raise ValueError(
            ...
            # validate that this is a script/integration which targets both 4.5 and 5.0+.
            if (
                server_version_compare(yml_data.get("toversion", "99.99.99"), "5.0.0")
                < 0
            ):
                raise ValueError(
    ...
    def get_code_file(self, script_type):
        ...
        script_path = list(
            filter(
                ...
                glob.glob(os.path.join(self.package_path, "*" + script_type)),
            )
        )[0]
    ...
    def insert_module_code(
        script_code: str, module_import: str, module_name: str
    ) -> str:
        """
