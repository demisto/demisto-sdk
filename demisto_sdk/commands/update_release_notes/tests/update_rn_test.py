...
class TestRNUpdate:
    ...
    def test_build_rn_template_integration(self, mocker):
        ...
        mocker.patch.object(
            UpdateRN, 'get_master_version', return_value='1.0.0'
        )
        mocker.patch(
    ...
    def test_build_rn_template_integration_for_generic(self, mock_master):
        ...
        changed_items = {
            ...
            ('Sample Generic Type', FileType.GENERIC_TYPE): {
                ...
                'is_new_file': False,
                'path': pack_path
                + '/GenericTypes/Object/generictype-Sample.json',
            },
        }
        release_notes = update_rn.build_rn_template(changed_items)
    ...
    @mock.patch.object(UpdateRN, 'get_master_version')
    def test_build_rn_template_when_only_pack_metadata_changed(
        self, mock_master
    ):
        """
        Given:
    ...
    def test_bump_version_number_minor(self, mock_master):
        ...
        shutil.copy(
            ...
            dst=os.path.join(
                TestRNUpdate.FILES_PATH, 'fake_pack/_pack_metadata.json'
            ),
        )
    ...
    def test_bump_version_no_version(self, mock_master):
        ...
        with pytest.raises(ValueError) as execinfo:
            update_rn.bump_version_number()
        assert (
            'Received no update type when one was expected.'
            in execinfo.value.args[0]
        )
    ...
    def test_update_rn_new_mapper(repo):
        ...
        """
        pack = repo.create_pack('test_pack')
        new_mapper = pack.create_mapper(
            name='mapper', content={'description': ''}
        )
        update_rn = UpdateRN(
            pack_path=pack.path,
            update_type='minor',
    ...
    def test_update_rn_new_incident_field(repo):
        ...
        rn_desc = update_rn.build_rn_desc(
            ...
            from_version=new_incident_field.read_json_as_dict().get(
                'fromversion'
            ),
        )
    ...
    def test_update_rn_new_trigger(repo):
        ...
        """
        pack = repo.create_pack('test_pack')
        new_trigger = pack.create_trigger(
            name='incident field',
            content={
                'description': 'description for testing',
                'fromversion': '6.5.0',
        ...
        update_rn = UpdateRN(
            pack_path=pack.path,
            update_type='minor',
            added_files=new_trigger.path,
            modified_files_in_pack=set(),
        )

        rn_desc = update_rn.build_rn_desc(
            _type=FileType.TRIGGER,
            content_name=pack.name,
            is_new_file=True,
        ...
        assert (
            '##### New:' not in rn_desc
        )  # https://github.com/demisto/etc/issues/48153#issuecomment-1111988526
        assert (
            'description for testing' in rn_desc
        )  # check if release note contains description when description
        # not empty
        assert (
            '(Available from Cortex XSOAR 6.5.0).' not in rn_desc
        )  # check if release note contains fromversion
        # when exists
    ...
    def test_deprecated_rn_yml(
        self, mocker, path, file_type, deprecated, expected_res
    ):
        """
        Given:
...
def get_mock_yml_obj(path, file_type, deprecated) -> dict:
    ...
    if file_type == FileType.INTEGRATION:
        old_yml_dict = {
            'script': {
                'commands': deepcopy(new_yml_obj.script.get('commands'))
            },
...
"""
    ...
    def test_get_changed_file_name_and_type(
        ...
        mocker.patch.object(
            UpdateRN, 'get_master_version', return_value='0.0.0'
        )
        update_rn = UpdateRN(
            pack_path=pack_name,
            update_type='minor',
    ...
    def test_create_markdown(self, mocker):
        ...
        filepath = os.path.join(
            TestRNUpdate.FILES_PATH, 'ReleaseNotes/1_1_1.md'
        )
        md_string = '### Test'
    ...
    def test_update_existing_rn(self, mocker):
        ...
        new_rn = update_rn.update_existing_rn(
            self.CURRENT_RN, self.CHANGED_FILES
        )
    ...
    def test_find_added_pack_files(self, mocker):
        ...
        added_files = {
            'HelloWorld/something_new.md',
            'HelloWorld/test_data/nothing.md',
        }
        mocker.patch.object(
            UpdateRN, 'get_master_version', return_value='0.0.0'
        )
        ...
        assert update_rn.modified_files_in_pack == {
            'HelloWorld/something_new.md'
        }
    ...
    @pytest.mark.parametrize(
        'pack_current_version, git_current_version, expected_result',
        diff_package,
    )
    def test_is_bump_required(
        ...
        mocker.patch.object(
            ...
            'communicate',
            return_value=(
                json.dumps({'currentVersion': git_current_version}),
                '',
            ),
        )
    ...
    def test_update_api_modules_dependents_rn__no_id_set(self, mocker):
        ...
        assert (
            'no id_set.json is available' in print_error_mock.call_args[0][0]
        )

    def test_update_api_modules_dependents_rn__happy_flow(
        self, mocker, tmpdir
    ):
        """
        Given
        ...
        modified = {
            '/Packs/ApiModules/Scripts/ApiModules_script/ApiModules_script.yml'
        }
        added = {}
        id_set_content = {
            'integrations': [
    ...
    def test_update_docker_image_when_yml_has_changed_but_not_docker_image_property(
        ...
        assert (
            check_docker_image_changed(
                main_branch='origin/master', packfile='test.yml'
            )
            is None
        )
    ...
    def test_update_docker_image_in_yml(self, mocker):
        ...
        mocker.patch.object(
            UpdateRN, 'get_pack_metadata', return_value=pack_data
        )
        mocker.patch.object(
            UpdateRN,
        ...
        assert (
            'Updated the Docker image to: *demisto/python3:3.9.6.22914*.' in RN
        )
    ...
    def test_update_docker_image_in_yml_when_RN_aleady_exists(self, mocker):
        ...
        mocker.patch(
            'demisto_sdk.commands.common.tools.get_display_name',
            return_value='Test',
        )
        mocker.patch.object(
            UpdateRN, 'build_rn_template', return_value='##### Test\n'
        )
        mocker.patch.object(
            UpdateRN,
            'get_release_notes_path',
    ...
    @pytest.mark.parametrize(
        'is_bc, existing_conf_data, expected_conf_data',
        BUILD_RN_CONFIG_FILE_INPUTS,
    )
...
def test_docker_image_is_added_for_every_integration(mocker, repo):
    ...
    yml_mock = {
        ...
        'script': {
            'type': 'python',
            'dockerimage': 'demisto/python3:3.9.5.123',
        },
    }
...
def test_create_md_if_currentversion_is_higher(
    ...
    updated_versions_list = [
        rn[rn.rindex('/') + 1 : -3] for rn in updated_rn_folder
    ]
