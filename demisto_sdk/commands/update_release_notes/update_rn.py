...
def get_deprecated_comment_from_desc(description: str) -> str:
    ...
    """
    deprecate_line_with_replacement = re.findall(
        DEPRECATED_REGEXES[0], description
    )
    deprecate_line_no_replacement = re.findall(
        DEPRECATED_REGEXES[1], description
    )

    deprecate_line = (
        deprecate_line_with_replacement + deprecate_line_no_replacement
    )
...
def get_deprecated_rn(path: str, file_type):
    ...
    rn = ''
    old_commands = deprecated_commands(
        old_yml.get('script', {}).get('commands')
    )
...
class UpdateRN:
    ...
    def handle_existing_rn_version_path(self, rn_path: str) -> str:
        ...
        if self.existing_rn_version_path:
            self.should_delete_existing_rn = (
                self.existing_rn_version_path != rn_path
            )
            try:
    ...
    def execute_update(self) -> bool:
        ...
        for packfile in self.modified_files_in_pack:
            file_name, file_type = self.get_changed_file_name_and_type(
                packfile
            )
            if (
                'yml' in packfile
        ...
        return self.create_pack_rn(
            rn_path, changed_files, new_metadata, new_version
        )
    ...
    def build_rn_config_file(self, new_version: str) -> None:
        ...
        bc_file_data['breakingChangesNotes'] = bc_file_data.get(
    ...
    def get_master_version(self) -> str:
        ...
        try:
            master_metadata = get_remote_file(
                self.metadata_path, tag=self.main_branch
            )
        except Exception as e:
            print_error(
    ...
    def only_docs_changed(self) -> bool:
        ...
        if (
            len(changed_files) == 1 and 'README' in changed_files_copy.pop()
        ) or (
            all(
                'README' in file
                or ('.png' in file and '_image.png' not in file)
    ...
    def find_added_pack_files(self):
        ...
        for a_file in self.added_files:
            if self.pack in a_file:
                if any(
                    item in a_file
                    for item in ALL_FILES_VALIDATION_IGNORE_WHITELIST
                ):
                    continue
                else:
                    self.modified_files_in_pack.add(
    ...
    def get_release_notes_path(self, input_version: str) -> str:
        ...
        return os.path.join(
            self.pack_path, 'ReleaseNotes', f'{new_version}.md'
        )
    ...
    def bump_version_number(
        ...
        new_version = (
            ''  # This will never happen since we pre-validate the argument
        )
    ...
    def build_rn_template(self, changed_items: dict) -> str:
        ...
        if self.is_force:
            rn_string = self.build_rn_desc(
                content_name=self.pack, text=self.text
            )
        ...
        for (content_name, _type), data in sorted(
            changed_items.items(),
            key=lambda x: RN_HEADER_BY_FILE_TYPE[x[0][1]]
            if x[0] and x[0][1]
            else '',
        ):  # Sort RN by header
            ...
            rn_template_as_dict[header] = (
                rn_template_as_dict.get(header, '') + rn_desc
            )
    ...
    def build_rn_desc(
        ...
        if _type in (
            ...
            if is_new_file:
                ...
                if from_version:
                    rn_desc += (
                        f' (Available from Cortex XSOAR {from_version}).'
                    )
                rn_desc += '\n'
            else:
                rn_desc = f'##### {content_name}\n'
    ...
    def update_existing_rn(self, current_rn, changed_files) -> str:
        ...
        for (content_name, _type), data in sorted(
            ...
            if (
                _header_by_type
                and _header_by_type in current_rn_without_docker_images
            ):
    ...
    def rn_with_docker_image(
        self, rn_string: str, docker_image: Optional[str]
    ) -> str:
        """
        ...
        update_docker_image_regex = r'- Updated the Docker image to: \*.*\*\.'
        updated_rn = re.sub(
            update_docker_image_regex, docker_image_str, rn_string
        )
        self.existing_rn_changed = True
        return updated_rn
...
def check_docker_image_changed(
    ...
    try:
        diff = run_command(
            f'git diff {main_branch} -- {packfile}', exit_on_error=False
        )
    except RuntimeError as e:
