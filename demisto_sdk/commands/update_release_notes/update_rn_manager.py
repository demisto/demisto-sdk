...
class UpdateReleaseNotesManager:
    def __init__(
        ...
        if self.given_pack and self.is_all:
            raise ValueError(
                'Please remove the -g flag when specifying only one pack.'
            )
    ...
    def manage_rn_update(self):
        ...
        if self.given_pack and '/' in self.given_pack:
            self.given_pack = get_pack_name(
                self.given_pack
            )  # extract pack from path
    ...
    def filter_files_from_git(
        ...
        (
            ...
            _,
        ) = self.filter_to_relevant_files(added_files, validate_manager)
    ...
    def get_git_changed_files(self) -> Tuple[set, set, set]:
        ...
        try:
            ...
            if (
                not validate_manager.git_util
            ):  # in case git utils can't be initialized.
    ...
    def handle_api_module_change(self, modified_files: set, added_files: set):
        ...
        api_module_was_given = (
            self.given_pack and API_MODULES_PACK in self.given_pack
        )
        ...
        if api_module_was_given or (api_module_changed_in_git and self.is_all):
            ...
            self.total_updated_packs = self.total_updated_packs.union(
                updated_packs
            )
