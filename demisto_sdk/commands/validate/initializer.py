        ...
        for path, git_status in file_by_status_dict.items():
            ...
            if f"/{INTEGRATIONS_DIR}/" in path_str or f"/{SCRIPTS_DIR}/" in path_str:
                ...
                if path_str.endswith(".yml"):
                    ...
                    integration_script_index = path.parts.index(INTEGRATIONS_DIR) if INTEGRATIONS_DIR in path.parts else path.parts.index(SCRIPTS_DIR)
                    path = Path(os.path.join(*path.parts[:integration_script_index+2]))
