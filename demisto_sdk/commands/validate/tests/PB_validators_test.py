...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"input1", "inputs3"},
            {"input1", "input2"},
            "The playbook 'Phishing Investigation - Generic' contains the following inputs that are not used in any of its tasks: input2",
        ),
    ],
)
def test_is_valid_all_inputs_in_use(
    mocker, input_in_task, input_in_section, expected_result
):
    """
        Given:
    ...
    mocker.patch(
        "demisto_sdk.commands.validate.validators.PB_validators.PB118_is_input_key_not_in_tasks.collect_all_inputs_in_use",
        return_value=input_in_task,
    )
    mocker.patch(
        "demisto_sdk.commands.validate.validators.PB_validators.PB118_is_input_key_not_in_tasks.collect_all_inputs_from_inputs_section",
        return_value=input_in_section,
    )
