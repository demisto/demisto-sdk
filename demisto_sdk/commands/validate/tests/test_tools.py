...
def create_parsing_rule_object(
    ...
    parser = ParsingRuleParser(Path(parsing_rule.path), list(MarketplaceVersions), pack_supported_modules=[])
...
def create_playbook_object(
    ...
    parser = PlaybookParser(Path(playbook.path), list(MarketplaceVersions), pack_supported_modules=[])
...
def create_test_playbook_object(
    ...
    playbook.yml.update(yml_content)
    parser = TestPlaybookParser(Path(playbook.path), list(MarketplaceVersions), pack_supported_modules=[])
