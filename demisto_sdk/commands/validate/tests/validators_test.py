...
class TestValidators:
    ...
    def test_validation_of_beta_playbooks(self, mocker):
        ...
        try:
            ...
            mocker.patch.object(
                validator, 'is_script_id_valid', return_value=True
            )
    ...
    @pytest.mark.parametrize(
        'source, answer', INPUTS_is_condition_branches_handled
    )
    ...
    def test_is_release_notes_exists(
        ...
        try:
            ...
            mocker.patch.object(
                BaseValidator, 'check_file_flags', return_value=''
            )
            mocker.patch.object(
                OldReleaseNotesValidator,
                'get_master_diff',
    ...
    @pytest.mark.parametrize(
        'source, target, file_type', INPUTS_STRUCTURE_VALIDATION
    )
    ...
    def test_run_all_validations_on_file(self, mocker, file_path):
        ...
        mocker.patch.object(
            IntegrationValidator,
            'is_valid_integration_file_path',
            return_value=True,
        )
        mocker.patch.object(
        ...
        validate_manager = ValidateManager(
            file_path=file_path, skip_conf_json=True
        )
    ...
    @pytest.mark.parametrize(
        'file_path', INVALID_FILES_PATHS_FOR_ALL_VALIDATIONS
    )
    ...
    def mock_unifier():
        ...
        with patch.object(
            IntegrationScriptUnifier, '__init__', lambda a, b: None
        ):
    ...
    def test_script_valid_rn(self, mocker):
        ...
        """
        mocker.patch.object(
            ScriptValidator, 'is_valid_name', return_value=True
        )
        mocker.patch.object(
            ScriptValidator, 'is_valid_script_file_path', return_value=True
        )
        ...
        self.mock_unifier()
        validate_manager = ValidateManager(skip_conf_json=True)
        is_valid = validate_manager.validate_added_files(
            [VALID_SCRIPT_PATH], None
        )
        assert is_valid

    def test_pack_validation(self):
        validate_manager = ValidateManager(
            file_path=VALID_PACK, skip_conf_json=True
        )
    ...
    def test_verify_no_dup_rn(self, added_files: set, expected: bool):
        ...
        """
        validate_manager = ValidateManager(skip_conf_json=True)
        result = validate_manager.validate_no_duplicated_release_notes(
            added_files
        )
    ...
    def test_are_tests_configured(
        ...
        structure_validator = StructureValidator(
            file_path, predefined_scheme=file_type
        )
    ...
    def test_create_ignored_errors_list(self):
        ...
        error_list = validate_manager.create_ignored_errors_list(
            errors_to_exclude
        )
        assert {
            'BA101',
    ...
    def test_is_old_file_format_non_unified(self):
        ...
        validate_manager = ValidateManager()
        res = validate_manager.is_old_file_format(
            test_file, FileType.INTEGRATION
        )
        assert res is False
    ...
    def test_validate_no_old_format_deprecated_content(self, repo):
        ...
        with ChangeCWD(repo.path):
            ...
            assert not validate_manager.validate_no_old_format(
                deprecated_false_file
            )
    ...
    def test_setup_git_params_non_master_branch(self, mocker):
        mocker.patch.object(
            ...
            return_value='not-master-branch',
        )
        validate_manager = ValidateManager()
        validate_manager.setup_git_params()
    ...
    def test_validate_release_notes(self, mocker):
        ...
        mocker.patch.object(
            ReleaseNotesValidator, '__init__', return_value=None
        )
        mocker.patch.object(
            ReleaseNotesValidator, 'is_file_valid', return_value=True
        )
        validate_manager = ValidateManager(skip_conf_json=True)
    ...
    @pytest.mark.parametrize(
        'answer, integration_id',
        [(True, 'MyIntegration'), (False, 'MyIntegration  ')],
    )
    ...
    @pytest.mark.parametrize(
        'answer, dashboard_id',
        [(True, 'MyDashboard'), (False, 'MyDashboard  ')],
    )
    ...
    def test_ignore_files_irrelevant_for_validation_should_not_ignore(
        ...
        assert not validate_manager.ignore_files_irrelevant_for_validation(
            file_path
        )
    ...
    def test_ignore_files_irrelevant_for_validation_test_file(
        self, file_path: str
    ):
        """
        Given
        ...
        """
        validate_manager = ValidateManager(check_is_unskipped=False)
        assert validate_manager.ignore_files_irrelevant_for_validation(
            file_path
        )
    ...
    def test_ignore_files_irrelevant_for_validation_non_pack(
        self, file_path: str
    ):
        """
        Given
    ...
    def test_is_valid_file_generic_field(
        ...
        mocker.patch.object(
            ContentEntityValidator,
            'is_valid_generic_object_file',
            return_value=True,
        )
        mocker.patch.object(
            GenericFieldValidator, 'is_valid_group', return_value=True
        )
        mocker.patch.object(
            GenericFieldValidator, 'is_valid_id_prefix', return_value=True
        )
        assert (
            res_validator.is_valid_file(
    ...
    def test_is_valid_file_incident_field(
        ...
        res_validator = IncidentFieldValidator(structure)
        mocker.patch.object(
            ContentEntityValidator, 'is_valid_file', return_value=True
        )
        mocker.patch.object(
            IncidentFieldValidator, 'is_valid_type', return_value=True
        )
        mocker.patch.object(
            IncidentFieldValidator, 'is_valid_group', return_value=True
        )
        mocker.patch.object(
...
def test_is_mapping_fields_command_exist(integration):
    ...
    integration.yml.write_dict(
        {
            'script': {
                'commands': [{'name': 'get-mapping-fields'}],
                'ismappable': True,
            }
        }
    )
...
def test_get_packs_that_should_have_version_raised(repo):
    ...
    with ChangeCWD(repo.path):
        ...
        assert (
            'PackWithModifiedIntegration'
            in packs_that_should_have_version_raised
        )
        assert (
            'ExistingPackWithAddedScript'
            in packs_that_should_have_version_raised
        )
        assert (
            'PackWithModifiedOldFile' in packs_that_should_have_version_raised
        )
        assert (
            'PackWithModifiedTestPlaybook'
            not in packs_that_should_have_version_raised
        )
...
def test_check_file_relevance_and_format_path_non_formatted_relevant_file(
    mocker,
):
    """
    Given
    ...
    validator_obj = ValidateManager(
        is_external_repo=True, check_is_unskipped=False
    )
    mocker.patch(
    ...
    mocker.patch.object(
        validator_obj, 'is_old_file_format', return_value=False
    )
    input_file_path = (
...
def test_check_file_relevance_and_format_path_ignored_non_pack_files(
    input_file_path,
):
    """
    Given
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            'Packs/some_file.ps1',
            'Packs/old_file_path.ps1',
            FileType.POWERSHELL_FILE,
        ),
        (
            'Packs/some_file.js',
            'Packs/old_file_path.js',
...
def test_job_non_feed_with_selected_feeds(repo, capsys):
    ...
    pack = repo.create_pack()
    job = pack.create_job(
        is_feed=False, name='job_name', selected_feeds=['feed_name']
    )
    validate_manager = ValidateManager(
        check_is_unskipped=False, file_path=job.path, skip_conf_json=True
    )
...
def test_job_both_selected_and_all_feeds_in_job(repo, capsys):
    ...
    pack = repo.create_pack()
    job = pack.create_job(
        is_feed=True, name='job_name', selected_feeds=['feed_name']
    )
...
def test_job_unexpected_field_values_in_non_feed_job(
    repo, capsys, is_all_feeds: bool, selected_feeds: Optional[List[str]]
):
    """
    Given
            A Job object in a repo, with non-empty selectedFeeds when isFeed is set to false
...
@pytest.mark.parametrize(
    ...
    (
        (
            {'Packs/Integration/mock_file_description.md'},
            '[BA115]',
            False,
            set(),
        ),
...
def test_validate_deleted_files(
    capsys, file_set, expected_output, expected_result, added_files, mocker
):
    """
    Given
