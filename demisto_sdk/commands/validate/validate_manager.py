...
class ValidateManager:
    def __init__(
        ...
        self.handle_error = BaseValidator(
            print_as_warnings=print_ignored_errors,
    ...
    def print_final_report(self, valid):
        ...
        if valid:
            ...
            if self.always_valid:
                click.secho(
                    'Found the errors above, but not failing build',
                    fg='yellow',
                )
                return 0
    ...
    def wait_futures_complete(
        self, futures_list: List[Future], done_fn: Callable
    ):
    ...
    def run_validation_on_generic_entities(
        self, dir_path, pack_error_ignore_list
    ):
        """
        ...
        if not file_type:
            ...
            if str(file_path).endswith('.png'):
                (
                    error_message,
                    error_code,
                ) = Errors.invalid_image_name_or_location()
            if self.handle_error(
                error_message=error_message,
                error_code=error_code,
                file_path=file_path,
    ...
    def run_validations_on_file(
        ...
        if file_type in self.skipped_file_types or file_path.endswith(
            '_unified.yml'
        ):
            self.ignored_files.add(file_path)
            return True
        ...
        # test playbooks and test scripts are using the same validation.
            ...
            return self.validate_release_notes_config(
                file_path, pack_error_ignore_list
            )
        ...
        # Validate only images of packs
            ...
            return self.validate_author_image(
                file_path, pack_error_ignore_list
            )
            ...
            return self.validate_reputation(
                structure_validator, pack_error_ignore_list
            )
            ...
            return self.validate_lists(
                structure_validator, pack_error_ignore_list
            )

        elif file_type == FileType.DASHBOARD:
            return self.validate_dashboard(
                structure_validator, pack_error_ignore_list
            )
            ...
            return self.validate_widget(
                structure_validator, pack_error_ignore_list
            )

        elif file_type == FileType.TRIGGER:
            return self.validate_triggers(
                structure_validator, pack_error_ignore_list
            )
    ...
    @staticmethod
    def specify_files_from_directory(
        file_set: Set, directory_path: str
    ) -> Set:
    ...
    def validate_test_playbook(
        self, structure_validator, pack_error_ignore_list
    ):
        ...
        return test_playbook_validator.is_valid_test_playbook(
            validate_rn=False
        )
    ...
    def validate_no_release_notes_for_new_pack(self, pack_name, file_path):
        if pack_name in self.new_packs:
            (
                error_message,
                error_code,
            ) = Errors.added_release_notes_for_new_pack(pack_name)
            if self.handle_error(
                error_message=error_message,
                error_code=error_code,
                file_path=file_path,
    ...
    def validate_incident_type(
        ...
        if is_modified and self.is_backward_check:
            ...
            return incident_type_validator.is_valid_incident_type(
                validate_rn=False
            )
    ...
    def validate_generic_definition(
        self, structure_validator, pack_error_ignore_list
    ):
    ...
    @staticmethod
    def was_file_renamed_but_labeled_as_deleted(
        deleted_file_path, added_files
    ):
        ...
        if added_files:
            ...
            if deleted_file_type := find_type(
                deleted_file_path, deleted_file_dict
            ):
    ...
    def validate_deleted_files(
        self, deleted_files: set, added_files: set
    ) -> bool:
        click.secho(
                ...
                if not self.was_file_renamed_but_labeled_as_deleted(
                    ...
                    if not self.is_file_allowed_to_be_deleted(file_path):
                        ...
                        (
                            error_message,
                            error_code,
                        ) = Errors.file_cannot_be_deleted(file_path)
                        if self.handle_error(
                            error_message, error_code, file_path
                        ):
                            is_valid = False
    ...
    def validate_changed_packs_unique_files(
        ...
        changed_packs = modified_packs.union(added_packs).union(
            changed_meta_packs
        )
    ...
    def validate_no_old_format(self, old_format_files):
        ...
        for file_path in old_format_files:
            ...
            if 'toversion' not in yaml_data and not yaml_data.get(
                'deprecated'
            ):
                error_message, error_code = Errors.invalid_package_structure()
                if self.handle_error(
                    error_message, error_code, file_path=file_path
                ):
                    handle_error = False
        return handle_error
            ...
            if find_type(file) == FileType.RELEASE_NOTES:
                ...
                if pack_name not in added_rn:
                    ...
                    (
                        error_message,
                        error_code,
                    ) = Errors.multiple_release_notes_files()
                    if self.handle_error(
    ...
    def validate_no_missing_release_notes(
        ...
        changed_files = modified_files.union(old_format_files).union(
            added_files
        )
        ...
        if API_MODULES_PACK in packs_that_should_have_new_rn:
            ...
            packs_that_should_have_new_rn = (
                packs_that_should_have_new_rn.union(
        ...
        packs_that_should_have_new_rn = (
            packs_that_should_have_new_rn - self.new_packs
        )
            ...
            for pack in packs_that_have_missing_rn:
                ...
                if pack in packs_that_should_have_new_rn_api_module_related:
                    (
                        error_message,
                        error_code,
                    ) = Errors.missing_release_notes_for_pack(API_MODULES_PACK)
                else:
                    (
                        error_message,
                        error_code,
                    ) = Errors.missing_release_notes_for_pack(pack)
    ...
    def setup_git_params(self):
        ...
        if self.branch_name.startswith('21.') or self.branch_name.startswith(
            '22.'
        ):
            self.skip_pack_rn_validation = True
    ...
    def get_changed_files_from_git(self) -> Tuple[Set, Set, Set, Set, bool]:
        ...
        (
            filtered_renamed,
            _,
            renamed_files_valid_types,
        ) = self.filter_to_relevant_files(renamed_files)
        filtered_modified = filtered_modified.union(filtered_renamed)
        (
            filtered_added,
            new_files_in_old_format,
            added_files_valid_types,
    ...
    def pack_metadata_extraction(
        self, modified_files, added_files, renamed_files
    ):
        ...
        for path in file_set:
            ...
            try:
                (
                    ...
                    file_path, old_path, old_format_files
                )
    ...
    def check_file_relevance_and_format_path(
        self, file_path, old_path, old_format_files
    ):
        """
    ...
    def ignore_files_irrelevant_for_validation(self, file_path: str) -> bool:
        ...
        if any(
            ...
            for name in VALIDATION_USING_GIT_IGNORABLE_DATA
        ):
            self.ignore_file(file_path)
            return True
        return False
    ...
    def create_ignored_errors_list(errors_to_check):
        ...
        for error_code in all_errors:
            ...
            if (
                error_code not in errors_to_check
                and error_type not in errors_to_check
            ):
                ignored_error_list.append(error_code)

        return ignored_error_list

    def add_ignored_errors_to_list(
        self, config, section, key, ignored_errors_list
    ):
        if key == 'ignore':
    ...
    def is_old_file_format(file_path, file_type):
        ...
        if file_type == FileType.INTEGRATION and file_yml.get(
            'script', {}
        ).get('script', '-') not in ['-', '']:
            if (
                file_yml.get('script', {}).get('type', 'javascript')
                != 'python'
            ):
                return False
            return True
        ...
        if file_type == FileType.SCRIPT and file_yml.get(
            'script', '-'
        ) not in ['-', '']:
