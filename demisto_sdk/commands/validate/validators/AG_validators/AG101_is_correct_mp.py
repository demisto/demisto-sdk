...
ContentTypes = Union[AgentixAgent, AgentixAction, AgentixSkill, Script]


class IsCorrectMPValidator(BaseValidator[ContentTypes]):
    error_code = "AG101"
    description = f"Content items of type {', '.join(filter(None, [ContentType.AGENTIX_AGENT, ContentType.AGENTIX_ACTION, ContentType.AGENTIX_SKILL, ContentType.SCRIPT]))} with isllm=true should be uploaded to platform only."
