...
class SubplaybookMustBeInternalValidator(BaseValidator[ContentTypes]):
    error_code = "AS108"
    description = "Validate that subplaybooks have the 'internal' field set to true."
    rationale = (
        "Playbooks with the 'subplaybook' prefix in their filename, id, or name "
        "are intended to be used as sub-playbooks and must have 'internal: true' "
        "to prevent them from being directly accessible."
    )
    error_message = (
        "The playbook has the 'subplaybook' prefix but does not have 'internal: true'. "
        "Subplaybooks must be marked as internal."
    )
...
def _is_subplaybook_without_internal(content_item: ContentTypes) -> bool:
    """
    Check if a playbook is a subplaybook (has 'subplaybook' prefix) but does not have 'internal: true'.

    Args:
        content_item: The playbook content item to validate.

    Returns:
        bool: True if the playbook is a subplaybook without internal=True.
    """
