...
class IsValidToversionOnModifiedValidator(BaseValidator[ContentTypes]):
    ...
    def is_valid_item(self, item_key, modified_item: ContentTypes, new_items: dict):
        """
        Check if modified item `toversion` field was updated and if it's a valid update.

        Args:
            item_key: the modified item identifier key (the field to indicate if a new version of it was added).
            modified_item: the modified item.
            new_items: dict of all newly added items.
