...
| CO121 | `IsValidInterpolation` | Both | Ensures that each auth profile that supports interpolations has a valid interpolation\_mapping — left side exists as an id under the auth profile (serialized or deserialized), right side exists in integration, only type 9 is split between .identifier/.password. The interpolation\_mapping left-side keys must be auth-field metadata.auth.parameter values only — never engine, engine\_group, proxy, or insecure (§2.6.2). **Sub-rule E:** a left-side key must NOT target a profile field whose `metadata.event.publish` is `true` — publish and interpolation are mutually exclusive contracts (complement of CO123, which requires non-interpolated fields to publish). Reserved-LEFT failures (Sub-rule B) suppress Sub-rule E on the same pair to avoid double-reporting. | Done |
