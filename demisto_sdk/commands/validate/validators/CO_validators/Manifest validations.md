...
| CO124 | `IsValidGroupedConnectorAuth` | Grouped only | All Grouped connector profiles support interpolation. Grouped-scoped by name/rule; implementation short-circuits with `if not grouped: pass`. |  |
| CO125 | `IsAuthProfileHasEngine` | Both | Ensure that all auth profiles have an engine param |  |
