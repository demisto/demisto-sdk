...
from demisto_sdk.commands.common.constants import FIRST_FETCH, FIRST_FETCH_PARAM, MAX_FETCH, MAX_FETCH_PARAM
...
class IsValidFetchIntegrationValidator(BaseValidator[ContentTypes]):
    ...
    error_message = "The integration is a fetch integration and missing the following params: {0}."
    ...
    def is_valid(self, content_items: Iterable[ContentTypes]) -> List[ValidationResult]:
        return [
            ValidationResult(
                validator=self,
                message=self.error_message.format(", ".join(list(missing_params.keys()))),
                content_object=content_item,
            )
            for content_item in content_items
            if content_item.is_fetch and not (missing_params := self.is_Valid_fetch_integration(content_item.name, content_item.params))
        ]
    
    def is_Valid_fetch_integration(self, integration_name: str, params: List[dict]) -> dict:
        """_summary_
        ...
        if not find_param(params, FIRST_FETCH):
            self.missing_fetch_params[integration_name] = self.missing_fetch_params.get(integration_name, {})
            self.missing_fetch_params[integration_name].update({FIRST_FETCH: FIRST_FETCH_PARAM})
    ...
    def fix(self, content_item: ContentTypes) -> FixResult:
        ...
        return FixResult(
            validator=self,
            message=self.fix_message.format(", ".join(list(self.missing_fetch_params.keys()))),
            content_object=content_item,
        )
