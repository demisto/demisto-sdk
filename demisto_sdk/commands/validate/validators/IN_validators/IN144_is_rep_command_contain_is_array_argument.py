...
from demisto_sdk.commands.common.constants import BANG_COMMAND_ARGS_MAPPING_DICT, BANG_COMMAND_NAMES
...
class IsRepCommandContainIsArrayArgumentValidator(BaseValidator[ContentTypes]):
    ...
    error_message = "The following reputation commands contain default arguments without isArray set to True:\n{0}"
    ...
    def is_valid(self, content_items: Iterable[ContentTypes]) -> List[ValidationResult]:
        return [
            ValidationResult(
                validator=self,
                message=self.error_message.format("\n".join(f"The command {key} is missing the isArray value on its default argument {val}" for key, val in invalid_commands.items())),
