...
from demisto_sdk.commands.content_graph.objects.integration import Integration, Parameter
...
class IsNewRequiredParamNoDefaultIntegrationValidator(BaseValidator[ContentTypes]):
    error_code = "IN164"
    description = (
        "Ensure that new *required* parameters added to an existing integration must have a default value."
    )
    rationale = ("Adding a new required parameter or changing a non-required one to required without specifying a "
                 "default value breaks backward compatibility and will prevent users from upgrading their "
                 "integration instances.")
    error_message = ("Possible backward compatibility break: "
                ...
                results.append(
                    ValidationResult(
                        ...
                        message=self.error_message.format(
                            param_list=", ".join(f'"{p}"' for p in new_required_params_with_no_default)
                        ),
    ...
    def _param_was_required_before(self, required_param: Parameter, old_content_item: BaseContent) -> bool:
        """
