...
class IsValidDynamicSectionValidator(BaseValidator[ContentTypes]):
    error_code = "LO108"
    description = "Ensures that dynamic sections in the layout contains existing scripts."
    ...
    error_message_unknown_script = "The tab {0} contains the following script that not exists in the repo: {1}."
                            ...
                            if section.get('query'):
                                ...
                                if is_string_uuid(query):
                                    validation_results.append(
                                        ValidationResult(
                                            validator=self,
                                            message=self.error_message_uuid.format(tab.get('name', ''), query),
                                ...
                                if not script_found:
                                    validation_results.append(
                                        ValidationResult(
                                            validator=self,
                                            message=self.error_message_unknown_script.format(tab.get('name', ''), query),
                                            content_object=content_item,
                                        )
                                    )
