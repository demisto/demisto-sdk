...
class IsValidCategoriesValidator(BaseValidator[ContentTypes]):
    ...
    error_message = (
        "The categories field must have one value, from the following list: {0}."
    )
    related_field = "categories"
    is_auto_fixable = False

    def is_valid(self, content_items: Iterable[ContentTypes]) -> List[ValidationResult]:
        approved_categories = get_current_categories()
        return [
            ValidationResult(
                validator=self,
                message=self.error_message.format(", ".join(approved_categories)),
            ...
            if (
                ...
                or not validate_categories_approved(
                    content_item.pack_metadata_dict.get("categories", []), approved_categories  # type: ignore[union-attr]
                )
            )
