...
class PlaybookOnlyDefaultNextValidator(BaseValidator[ContentTypes]):
    error_code = "PB125"
    description = "Validates that a condition task doesn't has only a default next-task."
    rationale = "Validates that a condition task doesn't has only a default next-task."
    error_message = "Playbook with id:\"{playbook_id}\" has conditional tasks with an only default condition. Tasks IDs: {tasks}.\n" \
    ...
    def is_valid(self, content_items: Iterable[ContentTypes]) -> List[ValidationResult]:
        tasks_with_only_default_nexttasks = dict()

        for content_item in content_items:
            for task_id, task in content_item.tasks.items():
                if len(list(task.nexttasks or {})) == 1 and list((task.nexttasks or {}).keys())[0].lower() == "#default#":
                    tasks_with_only_default_nexttasks.setdefault(content_item.object_id, []).append(task_id)
