...
class ParsingAndModelingRuleSuffixNameValidator(BaseValidator[ContentTypes]):
    ...
    def is_valid_using_graph(self, content_items: Iterable[ContentTypes], validate_all_files: bool) -> List[ValidationResult]:
        return [
            ...
            if (
                (content_item.content_type == MODELING_RULE and not content_item.name.endswith(MODELING_RULE_NAME_SUFFIX)) or
                (content_item.content_type == MODELING_RULE and not content_item.object_id.endswith(MODELING_RULE_ID_SUFFIX)) or
                (content_item.content_type == PARSING_RULE and not content_item.name.endswith(PARSING_RULE_NAME_SUFFIX)) or
                (content_item.content_type == PARSING_RULE and not content_item.object_id.endswith(PARSING_RULE_ID_SUFFIX))
            )
        ]
