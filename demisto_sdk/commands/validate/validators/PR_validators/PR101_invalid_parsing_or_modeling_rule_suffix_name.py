...
class ParsingAndModelingRuleSuffixNameValidator(BaseValidator[ContentTypes]):
    ...
    def is_valid(self, content_items: Iterable[ContentTypes]) -> List[ValidationResult]:
        return [
            ...
            if (
                (
                    content_item.content_type == ContentType.MODELING_RULE
                    and not content_item.name.endswith(MODELING_RULE_NAME_SUFFIX)
                )
                or (
                    content_item.content_type == ContentType.MODELING_RULE
                    and not content_item.object_id.endswith(MODELING_RULE_ID_SUFFIX)
                )
                or (
                    content_item.content_type == ContentType.PARSING_RULE
                    and not content_item.name.endswith(PARSING_RULE_NAME_SUFFIX)
                )
                or (
                    content_item.content_type == ContentType.PARSING_RULE
                    and not content_item.object_id.endswith(PARSING_RULE_ID_SUFFIX)
                )
            )
        ]
