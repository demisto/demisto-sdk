...
from junitparser import JUnitXml, Failure, Skipped, Error, Element
...
class TestResult:
    ...
    def __init__(self, name: str, status: TestStatus, time: float, _type: TestType, message: Optional[str] = None):
        self.name = name
        self.status = status
        self.time = time
    ...
    def __eq__(self, other):
        return self.name == other.name and self.status == other.status and self.time == other.status and self.message == other.message
...
class PyTestSuite:
    ...
    def __init__(self, failures: int, skipped: int, num_of_tests: int, time: float, test_cases: List[Element], _type: TestType):
    ...
    def test_results(self) -> List[TestResult]:
        ...
        for tc in self.test_cases:
            ...
            test_results.append(TestResult(name=name, time=time, status=status, message=message, _type=self.type))
    ...
    def failed_tests(self) -> List[TestResult]:
        if self.failures > 0:
            return [
                test_result for test_result in self.test_results if test_result.has_failed
            ]
        return []
    ...
    def skipped_tests(self) -> List[TestResult]:
        if self.skipped > 0:
            return [
                test_result for test_result in self.test_results if test_result.has_skipped
            ]
        return []
    ...
    def passed_tests(self) -> List[TestResult]:
        if self.passed > 0:
            return [
                test_result for test_result in self.test_results if test_result.has_passed
            ]
        return []
