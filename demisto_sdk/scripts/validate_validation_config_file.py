...
def validate_all_configured_error_codes_exist(
    ...
    """
    Validate that the set of configured validation errors in sdk_validation_config.toml is equal to the set of all existing validation to ensure we don't misconfigure non-existing validations.
...
def validate_error_code_not_configured_twice(
    ...
    for intersected_error_codes, section in [
        ...
        if intersected_error_codes:
            logger.error(
                f"[VA102] The following error codes are configured twice in the {section} both under select & warning: {', '.join(intersected_error_codes)}."
            )
            exit_code = 1
    return exit_code
