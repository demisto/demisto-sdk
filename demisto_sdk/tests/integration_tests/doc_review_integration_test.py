...
def test_spell_integration_invalid(repo, mocker, monkeypatch):
    ...
    with ChangeCWD(repo.path):
        ...
        assert not str_in_call_args_list(
            logger_info.call_args_list, "No misspelled words found in "
        )
        assert all(
            [
                str_in_call_args_list(logger_info.call_args_list, current_str)
...
def test_spell_readme_invalid(repo, mocker, monkeypatch):
    ...
    with ChangeCWD(repo.path):
        ...
        assert not str_in_call_args_list(logger_info.call_args_list, "GoodCase")
        assert not str_in_call_args_list(logger_info.call_args_list, "stillGoodCase")
    ...
    with ChangeCWD(repo.path):
        ...
        assert all(
            [
                str_in_call_args_list(logger_info.call_args_list, current_str)
                for current_str in [
                    "General Pointers About Release Notes:",
                ]
            ]
        )
