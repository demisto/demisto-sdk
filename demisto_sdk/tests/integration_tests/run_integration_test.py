...
def test_integration_run_non_existing_command(
    mocker, set_environment_variables
):
    """
    Given
    ...
    """
    mocker.patch.object(
        DefaultApi, 'investigation_add_entries_sync', return_value=None
    )
...
def test_json_to_outputs_flag(mocker, set_environment_variables):
    ...
    mocker.patch.object(
        DefaultApi, 'download_file', return_value=DEBUG_FILE_PATH
    )
    ...
    run_result = CliRunner(mix_stderr=False,).invoke(
        main,
        ['run', '-q', command, '--json-to-outputs', '-p', 'Keylight', '-r'],
    )
