...
def mock_git(mocker, is_merge: bool = False):
    mocker.patch.object(
        SecretsValidator, 'get_branch_name', return_value='branch name'
    )
    mocker.patch(
...
def test_integration_secrets_incident_field_positive(mocker, repo):
    ...
    assert (
        'Finished validating secrets, no secrets were found.' in result.output
    )
...
def test_integration_secrets_integration_positive(mocker, repo):
    ...
    with ChangeCWD(integration.repo_path):
        ...
        result = runner.invoke(
            main,
            [SECRETS_CMD, '-wl', repo.secrets.path],
...
def test_integration_secrets_integration_positive_with_input_option(
    ...
    assert (
        'Finished validating secrets, no secrets were found' in result.stdout
    )


def test_integration_secrets_integration_negative_with_input_option(
    mocker, repo
):
    """
    Given
