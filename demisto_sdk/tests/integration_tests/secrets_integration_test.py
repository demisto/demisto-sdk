...
def test_integration_secrets_integration_negative(mocker):
    ...
    integration_with_secrets_path = join(
        DEMISTO_SDK_PATH,
        "tests/test_files/content_repo_example/Packs/FeedAzure/Integrations/FeedAzure/FeedAzure.yml",
    )
    mocker.patch(
        "demisto_sdk.__main__.SecretsValidator.get_all_diff_text_files",
...
def test_integration_secrets_integration_positive(mocker, tmp_path):
    ...
    whitelist = {
        ...
        "urls": [],
        "somethingelse": [],
        "generic_strings": ["365ForMarketingEmail", "feedBypassExclusionList"],
    }
    whitelist_path = tmp_path / "whitelist.txt"
    whitelist_path.write_text(json.dumps(whitelist))
    runner = CliRunner(mix_stderr=False)
    result = runner.invoke(
        main, [SECRETS_CMD, "-wl", Path(whitelist_path)], catch_exceptions=False
    )
    assert result.exit_code == 0
...
def test_integration_secrets_integration_with_regex_expression(tmp_path):
    ...
    """
    white_list_path = create_temp_file(
        tmp_path, json.dumps({"generic_strings": "***.url\n"}), "whitelist.txt"
    )
    file_contents_path = create_temp_file(
        tmp_path,
        """
    ...
    result = runner.invoke(
        main,
        [SECRETS_CMD, "--input", file_contents_path, "-wl", white_list_path],
...
def test_integration_secrets_integration_positive_with_input_option():
    ...
    result = CliRunner(mix_stderr=False).invoke(
        main, [SECRETS_CMD, "--input", integration_secrets_path]
    )
...
def test_integration_secrets_integration_negative_with_input_option(tmp_path):
    ...
    """
    integration_secrets_path = create_temp_file(
        tmp_path, "ThunderBolt@ndLightningVeryV3ryFr1eghtningM3\n"
    )
    result = CliRunner(mix_stderr=False).invoke(
        main, [SECRETS_CMD, "--input", integration_secrets_path]
    )
