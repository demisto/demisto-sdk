...
THINKCANARY_RN_FOLDER = join(
    git_path(), 'Packs', 'ThinkCanary', 'ReleaseNotes'
)
...
def test_update_release_notes_new_integration(demisto_client, mocker):
    ...
    mocker.patch(
        'demisto_sdk.commands.common.tools.get_pack_name',
        return_value='FeedAzureValid',
    )
    ...
    assert (
        'Finished updating release notes for FeedAzureValid.' in result.stdout
    )
...
def test_update_release_notes_unified_yml_integration(demisto_client, mocker):
    ...
    runner = CliRunner(mix_stderr=False)
    old_files = {
        join(VMWARE_PACK_PATH, 'Integrations', 'integration-VMware.yml')
    }
    ...
    result = runner.invoke(
        main, [UPDATE_RN_COMMAND, '-i', join('Packs', 'VMware')]
    )
    assert result.exit_code == 0
    assert not result.exception
    assert (
...
def test_update_release_notes_non_content_path(demisto_client, mocker):
    ...
    assert (
        'You are not running' in result.stdout
...
def test_update_release_notes_master_ahead_of_current(
...
def test_update_release_notes_specific_version_invalid(demisto_client, repo):
    ...
    result = runner.invoke(
        main,
        [UPDATE_RN_COMMAND, '-i', join('Packs', 'ThinkCanary'), '-v', '3.x.t'],
    )
    assert (
        'The format of version should be in x.y.z format, e.g: <2.1.3>'
        in result.stdout
    )


def test_update_release_notes_specific_version_valid(
    demisto_client, mocker, repo
):
    """
    Given
...
def test_force_update_release(demisto_client, mocker, repo):
    ...
    result = runner.invoke(
        main,
        [UPDATE_RN_COMMAND, '-i', join('Packs', 'ThinkCanary'), '--force'],
    )
