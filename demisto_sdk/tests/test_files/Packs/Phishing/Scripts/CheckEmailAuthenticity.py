...
def main():
    try:
        ...
        if auth is None and spf is None:
            context = {"{}.AuthenticityCheck".format(email_key): "undetermined"}
        ...
        md += tableToMarkdown(
            "DKIM", dkim_data, ["Validation-Result", "Reason", "Signing-Domain"]
        )
        md += tableToMarkdown(
            "DMARC", dmarc_data, ["Validation-Result", "Tags", "Signing-Domain"]
        )
