...
def vulndb_vulnerability_to_entry(vuln):
    ...
    cve_ext_reference_values = [
        ext_reference["value"] for ext_reference in vuln.get("ext_references", [])
    ]
    ...
    classification_details = [
        {
            "Longname": classification.get("classification", default_classification)[
                "longname"
            ],
...
def vulndb_get_vuln_by_vendor_id_command():
    ...
    res = http_request(
        f"{API_URL}/vulnerabilities/find_by_vendor_id?vendor_id={vendor_id}", max_size
    )
...
def vulndb_get_updates_by_dates_or_hours_command():
    ...
    if start_date:
        ...
        res = http_request(
            f"{API_URL}/vulnerabilities/find_by_time?hours_ago={hours_ago}", max_size
        )
    else:
...
def vulndb_get_product_command():
    ...
    if vendor_id is not None and vendor_name is not None:
        ...
        res = http_request(
            f"{API_URL}/products/by_vendor_name?vendor_name={vendor_name}", max_size
        )
    else:
...
def vulndb_get_version_command():
    ...
    if product_id is not None and product_name is not None:
        ...
        res = http_request(
            f"{API_URL}/versions/by_product_id?product_id={product_id}", max_size
        )
    elif product_name:
        res = http_request(
            f"{API_URL}/versions/by_product_name?product_name={product_name}", max_size
        )
