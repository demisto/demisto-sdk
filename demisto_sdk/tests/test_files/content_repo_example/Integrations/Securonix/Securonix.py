...
def parse_data_arr(
    data_arr: Any, fields_to_drop: list = [], fields_to_include: list = []
):
    ...
    if isinstance(data_arr, list):
        ...
        for data in data_arr:
            readable = {
                reformat_outputs(i): j
                for i, j in data.items()
                if i not in fields_to_drop
            }
            if fields_to_include:
                readable = {
                    i: j for i, j in readable.items() if i in fields_to_include
                }
            readable_arr.append(readable)
            outputs_arr.append(
                {k.replace(' ', ''): v for k, v in readable.copy().items()}
            )
        return readable_arr, outputs_arr

    readable = {
        reformat_outputs(i): j
        for i, j in data_arr.items()
        if i not in fields_to_drop
    }
    if fields_to_include:
        readable = {
...
def incident_priority_to_dbot_score(priority_str: str):
    ...
    if priority == 'high':
        return 3
    demisto.info(
        f'Securonix incident priority: {priority} is not known. Setting as unknown(DBotScore of 0).'
    )
    return 0
...
class Client(BaseClient):
    ...
    def http_request(
        ...
        try:
            ...
            if not result.ok:
                raise ValueError(
                    f'Error in API call to Securonix {result.status_code}. Reason: {result.text}'
                )
            try:
    ...
    def _generate_token(self) -> str:
        ...
        token = self.http_request(
            'GET', '/token/generate', headers=headers, response_type='text'
        )
        return token
    ...
    def get_default_assignee_for_workflow_request(self, workflow: str) -> Dict:
        ...
        default_assignee = self.http_request(
            'GET',
            '/incident/get',
            headers={'token': self._token},
            params=params,
    ...
    def list_resource_groups_request(self) -> Dict:
        ...
        resource_groups = self.http_request(
            'GET',
            '/list/resourceGroups',
            headers={'token': self._token},
            response_type='xml',
    ...
    def list_activity_data_request(
        self, from_: str, to_: str, query: str = None
    ) -> Dict:
        """List activity data.
        ...
        activity_data = self.http_request(
            'GET',
    ...
    def list_violation_data_request(
        ...
        if query:
            params['query'] = f'{params["query"]} AND {query}'
        violation_data = self.http_request(
            'GET',
            '/spotter/index/search',
            headers={'token': self._token},
            params=params,
    ...
    def list_incidents_request(
        self, from_epoch: str, to_epoch: str, incident_types: str
    ) -> Dict:
    ...
    def perform_action_on_incident_request(
        self, incident_id, action: str
    ) -> Dict:
        """get incident available actions by sending a GET request.
        ...
        possible_action = self.http_request(
            'GET',
            '/incident/get',
            headers={'token': self._token},
            params=params,
    ...
    def add_comment_to_incident_request(
        self, incident_id: str, comment: str
    ) -> Dict:
    ...
    def list_watchlist_request(self):
        ...
        Returns:
            Response from API.
        """
        watchlists = self.http_request(
            'GET', '/incident/listWatchlist', headers={'token': self._token}
        )
    ...
    def create_watchlist_request(self, watchlist_name: str) -> Dict:
        ...
        Returns:
            Response from API.
        """
        params = {'watchlistname': watchlist_name}
        watchlist = self.http_request(
            'POST',
            '/incident/createWatchlist',
            headers={'token': self._token},
            params=params,
            response_type='text',
        )
    ...
    def check_entity_in_watchlist_request(
        self, entity_name: str, watchlist_name: str
    ) -> Dict:
        ...
        response = self.http_request(
            'GET',
            '/incident/checkIfWatchlisted',
            headers={'token': self._token},
            params=params,
        )
        return response

    def add_entity_to_watchlist_request(
        self,
        watchlist_name: str,
        entity_type: str,
        entity_name: str,
...
def list_workflows(client: Client, *_) -> Tuple[str, Dict, Dict]:
    ...
    human_readable = tableToMarkdown(
        ...
        t=workflows_readable,
        headers=['Workflow', 'Type', 'Value'],
        removeNull=True,
    )
    entry_context = {
        'Securonix.Workflows(val.Workflow == obj.Workflow)': workflows_outputs
    }
    return human_readable, entry_context, workflows


def get_default_assignee_for_workflow(
    ...
    default_assignee = client.get_default_assignee_for_workflow_request(
        workflow
    )
    workflow_output = {
        'Workflow': workflow,
    ...
    entry_context = {
        'Securonix.Workflows(val.Workflow === obj.Workflow)': workflow_output
    }
...
def list_possible_threat_actions(client: Client, *_) -> Tuple[str, Dict, Dict]:
    ...
    human_readable = (
        f'Possible threat actions are: {", ".join(threat_actions)}.'
    )
...
def list_policies(client: Client, *_) -> Tuple[str, Dict, Dict]:
    ...
    human_readable = tableToMarkdown(
        name='Policies:', t=policies_readable, headers=headers, removeNull=True
    )
...
def list_resource_groups(client: Client, *_) -> Tuple[str, Dict, Dict]:
    ...
    resource_groups_arr = resource_groups.get('resourceGroups').get(
        'resourceGroup'
    )

    resource_groups_readable, resource_groups_outputs = parse_data_arr(
        resource_groups_arr
    )
    headers = ['Name', 'Type']
    human_readable = tableToMarkdown(
        name='Resource groups:',
        t=resource_groups_readable,
        headers=headers,
        removeNull=True,
    )
    entry_context = {
        'Securonix.ResourceGroups(val.Name === obj.Name)': resource_groups_outputs
    }
...
def list_users(client: Client, *_) -> Tuple[str, Dict, Dict]:
    ...
    entry_context = {
        'Securonix.Users(val.EmployeeID === obj.EmployeeID)': users_outputs
    }
...
def list_activity_data(client: Client, args) -> Tuple[str, Dict, Dict]:
    ...
    activity_readable, activity_outputs = parse_data_arr(
        activity_events, fields_to_include=fields_to_include
    )
    ...
    entry_context = {
        'Securonix.ActivityData(val.EventID === obj.EventID)': activity_outputs
    }
...
def list_violation_data(client: Client, args) -> Tuple[str, Dict, Dict]:
    ...
    violation_readable, violation_outputs = parse_data_arr(
        violation_events, fields_to_include=fields_to_include
    )
    headers = ['EventID', 'Eventtime', 'Message', 'Policyname', 'Accountname']
    human_readable = tableToMarkdown(
        name='Activity data:',
        t=violation_readable,
        headers=headers,
        removeNull=True,
    )
    entry_context = {
        'Securonix.ViolationData(val.Uniquecode === obj.Uniquecode)': violation_outputs
    }
...
def list_incidents(client: Client, args: Dict) -> Tuple[str, Dict, Dict]:
    ...
    incident_types = (
        str(args.get('incident_types'))
        if 'incident_types' in args
        else 'opened'
    )
    incidents = client.list_incidents_request(
        from_epoch, to_epoch, incident_types
    )
    ...
    human_readable = tableToMarkdown(
        name='Incidents:',
        t=incidents_readable,
        headers=headers,
        removeNull=True,
    )
    entry_context = {
        'Securonix.Incidents(val.IncidentID === obj.IncidentID)': incidents_outputs
    }
...
def get_incident(client: Client, args: Dict) -> Tuple[str, Dict, Dict]:
    ...
    human_readable = tableToMarkdown(
        name='Incident:', t=incident_readable, removeNull=True
    )
    entry_context = {
        'Securonix.Incidents(val.IncidentID === obj.IncidentID)': incident_outputs
    }
...
def get_incident_status(client: Client, args: Dict) -> Tuple[str, Dict, Dict]:
    ...
    return (
        f'Incident {incident_id} status is {incident_status}.',
        entry_context,
        incident,
    )
...
def get_incident_workflow(
    ...
    return (
        f'Incident {incident_id} workflow is {incident_workflow}.',
        entry_context,
        incident,
    )


def get_incident_available_actions(
    client: Client, args: Dict
) -> Tuple[str, Dict, Dict]:
    ...
    incident_actions = client.get_incident_available_actions_request(
        incident_id
    )
    if not incident_actions:
        return (
            f'Incident {incident_id} does not have any available actions.',
            {},
            incident_actions,
        )
    actions = []
    ...
    return (
        f'Incident {incident_id} available actions: {actions}.',
        entry_context,
        incident_actions,
    )


def perform_action_on_incident(
    client: Client, args: Dict
) -> Tuple[str, Dict, Dict]:
    """Perform action on an incident.
    ...
    incident_result = client.perform_action_on_incident_request(
        incident_id, action
    )
    ...
    return (
        f'Action {action} was performed on incident {incident_id}.',
        {},
        incident_result,
    )
...
def add_comment_to_incident(
    ...
    if not incident:
        raise Exception(
            f'Failed to add comment to the incident {incident_id}.'
        )
    return (
        f'Comment was added to the incident {incident_id} successfully.',
        {},
        incident,
    )
...
def create_incident(client: Client, args: Dict) -> Tuple[str, Dict, Dict]:
    ...
    criticality = (
        str(args.get('criticality')) if 'criticality' in args else None
    )

    if 'create incident' in action_name and not workflow:
        raise Exception(
            f'Creating an incident with the action: {action_name}, Supply a workflow.'
        )
    ...
    if message:
        if isinstance(message, list) and 'Invalid' in message[0]:
            message = message[0]
            raise Exception(
                f'Failed to create the incident with message:\n{message}'
            )
    ...
    human_readable = tableToMarkdown(
        name='Incident was created successfully',
        t=incident_readable,
        headers=headers,
        removeNull=True,
    )
...
def get_watchlist(client: Client, args) -> Tuple[str, Dict, Dict]:
    ...
    watchlist_readable, watchlist_events_outputs = parse_data_arr(
        watchlist_events, fields_to_drop=fields_to_drop
    )
    watchlist_outputs = {
        'Watchlistname': watchlist_name,
    ...
    entry_context = {
        'Securonix.Watchlists(val.Watchlistname === obj.Watchlistname)': watchlist_outputs
    }
...
def create_watchlist(client: Client, args) -> Tuple[str, Dict, Dict]:
    ...
    if 'successfully' not in response:
        raise Exception(
            f'Failed to list watchlists.\nResponse from Securonix is:{str(response)}'
        )
    human_readable = f'Watchlist {watchlist_name} was created successfully.'
    entry_context = {
        'Securonix.Watchlists(val.Watchlistname === obj.Watchlistname)': watchlist_name
    }
    return human_readable, entry_context, response
...
def check_entity_in_watchlist(client: Client, args) -> Tuple[str, Dict, Dict]:
    ...
    watchlist_name = args.get('watchlist_name')
    watchlist = client.check_entity_in_watchlist_request(
        entity_name, watchlist_name
    )
    ...
    entry_context = {
        'Securonix.EntityInWatchlist(val.Entityname === obj.Entityname)': output
    }
    return human_readable, entry_context, watchlist
...
def add_entity_to_watchlist(client: Client, args) -> Tuple[str, Dict, Dict]:
    ...
    response = client.add_entity_to_watchlist_request(
        watchlist_name, entity_type, entity_name, expiry_days
    )
...
def fetch_incidents(
    ...
    if not last_run:  # if first time running
        new_last_run = {
            'time': parse_date_range(fetch_time, date_format=timestamp_format)[
                0
            ]
        }
    else:
    ...
    from_epoch = date_to_timestamp(
        new_last_run.get('time'), date_format=timestamp_format
    )
    to_epoch = date_to_timestamp(datetime.now(), date_format=timestamp_format)
    # Get incidents from Securonix
    demisto.info(
        f'Fetching Securonix incidents. From: {from_epoch}. To: {to_epoch}'
    )
        ...
        demisto_incidents = [
            {
                ...
                'occurred': timestamp_to_datestring(
                    incident.get('lastUpdateDate')
                ),
                'severity': incident_priority_to_dbot_score(
                    incident.get('priority')
                ),
                'rawJSON': json.dumps(incident),
            }
            for incident in incidents_items
            if incident.get('incidentId') > last_incident_id
        ]
...
def main():
    ...
    if not host:
        server_url = tenant
        if not tenant.startswith('http://') and not tenant.startswith(
            'https://'
        ):
    ...
    try:
        ...
        if command == 'fetch-incidents':
            ...
            incident_types = (
                params.get('incident_types')
                if 'incident_types' in params
                else 'opened'
            )
