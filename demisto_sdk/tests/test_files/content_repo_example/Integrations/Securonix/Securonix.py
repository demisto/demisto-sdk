...
def reformat_outputs(text: str) -> str:
    ...
    if text.startswith("U_") or text.startswith("u_"):
        text = text[2:]
    return (
        "".join(" " + char if char.isupper() else char.strip() for char in text)
        .strip()
        .title()
    )
...
def parse_data_arr(
    ...
    readable = {
        reformat_outputs(i): j for i, j in data_arr.items() if i not in fields_to_drop
    }
    if fields_to_include:
        readable = {i: j for i, j in readable.items() if i in fields_to_include}
...
class Client(BaseClient):
    ...
    def http_request(
        self, method, url_suffix, headers=None, params=None, response_type: str = "json"
    ):
        """
    ...
    def get_default_assignee_for_workflow_request(self, workflow: str) -> Dict:
        ...
        default_assignee = self.http_request(
            "GET", "/incident/get", headers={"token": self._token}, params=params
        )
    ...
    def list_users_request(self) -> Dict:
        ...
        users = self.http_request(
            "GET", "/list/allUsers", headers={"token": self._token}, response_type="xml"
        )
        return users
    ...
    def perform_action_on_incident_request(self, incident_id, action: str) -> Dict:
        ...
        incident = self.http_request(
            "POST", "/incident/actions", headers={"token": self._token}, params=params
        )
        return incident.get("result")
    ...
    def add_entity_to_watchlist_request(
        self, watchlist_name: str, entity_type: str, entity_name: str, expiry_days: str
    ) -> Dict:
        """Check if an entity is whitelisted by sending a GET request.
...
def list_users(client: Client, *_) -> Tuple[str, Dict, Dict]:
    ...
    human_readable = tableToMarkdown(
        name="Resource groups:", t=users_readable, headers=headers, removeNull=True
    )
    entry_context = {
...
def list_activity_data(client: Client, args) -> Tuple[str, Dict, Dict]:
    ...
    human_readable = tableToMarkdown(
        name="Activity data:", t=activity_readable, headers=headers, removeNull=True
    )
    entry_context = {
...
def list_violation_data(client: Client, args) -> Tuple[str, Dict, Dict]:
    ...
    human_readable = tableToMarkdown(
        name="Activity data:", t=violation_readable, headers=headers, removeNull=True
    )
    entry_context = {
...
def list_incidents(client: Client, args: Dict) -> Tuple[str, Dict, Dict]:
    ...
    incident_types = (
        str(args.get("incident_types")) if "incident_types" in args else "opened"
    )
    ...
    human_readable = tableToMarkdown(
        name="Incidents:", t=incidents_readable, headers=headers, removeNull=True
    )
    entry_context = {
...
def fetch_incidents(
    client: Client, fetch_time: Optional[str], incident_types: str, last_run: Dict
) -> list:
    ...
    if not last_run:  # if first time running
        new_last_run = {
            "time": parse_date_range(fetch_time, date_format=timestamp_format)[0]
        }
    else:
...
def main():
    ...
    try:
        ...
        commands: Dict[
            str,
            Callable[
                [Client, Dict[str, str]], Tuple[str, Dict[Any, Any], Dict[Any, Any]]
            ],
        ] = {
        ...
        if command == "fetch-incidents":
            ...
            incident_types = (
                params.get("incident_types") if "incident_types" in params else "opened"
            )
            incidents = fetch_incidents(
                client, fetch_time, incident_types, last_run=demisto.getLastRun()
            )
