...
class Client(BaseClient):
    ...
    def get_cookie(self, res_cookie: dict) -> str:
        ...
        KACE_LAST_USER_SECURE = res_cookie.get("KACE_LAST_USER_SECURE").__dict__.get(
            "value"
        )
        KACE_LAST_ORG_SECURE = res_cookie.get("KACE_LAST_ORG_SECURE").__dict__.get(
            "value"
        )
    ...
    def token_request(
        self, url: str, headers: Optional[dict] = None, data: Optional[str] = None
    ) -> dict:
        ...
        try:
            response = requests.request(
                "POST", url, headers=headers, data=data, verify=self._verify
            )
    ...
    def queues_list_fields_request(self, queue_number: str) -> dict:
        ...
        return self._http_request(
            "GET",
            url_suffix=f"/service_desk/queues/{queue_number}/fields",
            headers=headers,
        )

    def tickets_list_request(
        self, shaping_fields: str = None, filter_fields: str = None
    ) -> dict:
        """List of Tickets.
         Args:
    ...
    def create_ticket_request(self, data: str) -> dict:
        ...
        return self._http_request(
            "POST", url_suffix="/service_desk/tickets", headers=headers, data=data
        )
    ...
    def update_ticket_request(self, ticket_id: str, data: str) -> dict:
        ...
        return self._http_request(
            "POST",
            url_suffix=f"/service_desk/tickets/{ticket_id}",
            headers=headers,
            data=data,
        )
    ...
    def delete_ticket_request(self, ticket_id: str) -> dict:
        ...
        return self._http_request(
            "DELETE", url_suffix=f"/service_desk/tickets/{ticket_id}", headers=headers
        )
    ...
    def ticket_by_id_request(self, filtering_id: int) -> dict:
        ...
        return self._http_request(
            "GET",
            url_suffix=f"/service_desk/tickets?filtering={filter_fields}",
            headers=headers,
        )
...
def test_module(client: Client, *_) -> Tuple[str, dict, dict]:
    ...
    if list_tickets_res and demisto.params().get("isFetch"):
        parse_date_range(
            demisto.params().get("fetch_time"), date_format="%Y-%m-%dT%H:%M:%SZ"
        )
        parsed_time = datetime.utcnow() - timedelta(days=20)
        incidents, _ = parse_incidents(
            list_tickets_res, "1", "%Y-%m-%dT%H:%M:%SZ", parsed_time
        )
    return "ok", {}, {}
...
def get_queues_list_command(client: Client, args) -> Tuple[str, dict, dict]:
    ...
    context = parse_response(raw_response)
    human_readable_markdown = tableToMarkdown(
        "Quest Kace Queues", context, removeNull=True, headers=["ID", "Name", "Fields"]
    )
...
def create_ticket_command(client: Client, args: dict) -> Tuple[str, dict, dict]:
    ...
    body_from_args = create_body_from_args(
        hd_queue_id, title, summary, impact, category, status, priority, machine, asset
    )
    if custom_fields:
...
def create_body_from_args(
    hd_queue_id: Union[str, int] = None,
    title: Union[str, int] = None,
    summary: Union[str, int] = None,
    impact: Union[str, int] = None,
    category: Union[str, int] = None,
    status: Union[str, int] = None,
    priority: Union[str, int] = None,
    machine: Union[str, int] = None,
    asset: Union[str, int] = None,
) -> dict:
...
def update_ticket_command(client: Client, args) -> Tuple[str, dict, dict]:
    ...
    ticket = res.get("Tickets")
    ticket_view = tableToMarkdown(
        f"Ticket number {ticket_id} was updated successfully.\n", ticket
    )
    return ticket_view, {}, {}
...
def delete_ticket_command(client: Client, args) -> Tuple[str, dict, dict]:
    ...
    if response.get("Result") == "Success":
        context = {}
        old_context = demisto.dt(
            demisto.context(), f"QuestKace.Ticket(val.ID === {ticket_id})"
        )
        if old_context:
        ...
        return (
            f"Ticket was deleted successfully. Ticket number {ticket_id}",
            context,
            {},
        )
    else:
...
def fetch_incidents(
    ...
    if not last_run:  # if first time running
        new_last_run = {
            "last_fetch": parse_date_range(fetch_time, date_format=time_format)[0]
        }
    else:
    ...
    parsed_last_time = datetime.strptime(
        new_last_run.get("last_fetch", ""), time_format
    )
    ...
    incidents, last_incident_time = parse_incidents(
    ...
    for q in queues_id:
        ...
        for field in fields_by_queue:
            if field.get("jsonKey") not in fields:
                ...
                if (
                    field.get("jsonKey") != "related_tickets"
                    and field.get("jsonKey") != "referring_tickets"
                ):
...
def split_fields(fields: str = "") -> dict:
    ...
    if fields:
        ...
        for f in arr_fields:
            field = f.split(
                "=", 1
            )  # a field might include a '=' sign in the value. thus, splitting only once.
            if len(field) > 1:
...
try:
    ...
    try:
        ...
        # Commands dict
        commands: Dict[
            str, Callable[[Client, Dict[str, str]], Tuple[str, dict, dict]]
        ] = {
        ...
        return_error(
            f"Error from QuestKace Integration.\n"
