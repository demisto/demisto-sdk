...
class CircleCIResponse(SimpleNamespace):
    ...
    def __getattr__(self, attr):
        ...
        """
        logging.debug(
            f'could not find attribute {attr}, hence returning default value'
        )
        return self.ATTRIBUTES_DEFAULT_MAPPING.get(attr)


def parse_http_response(
    expected_valid_code: int = 200, response_type: str = 'class'
):
    """
...
class CircleCIClient:
    ...
    def __init__(
        ...
        self.auth = HTTPBasicAuth(
            username=token or os.getenv('CCI_TOKEN'), password=''
        )
    ...
    def get_job_test_metadata(self, job_number: int):
        return self.get_resource(
            url=f'project/{PROJECT_SLUG}/{job_number}/tests'
        )

    @parse_http_response()
    def get_job_details_v1(self, job_number: int):
        return self.get_resource(
            url=f'project/{PROJECT_SLUG}/{job_number}',
