...
def options_handler():
    ...
    parser.add_argument(
        '-c', '--commit', help='The commit hash', required=True
    )
    parser.add_argument(
...
def extract_changes_from_commit(
    commit_hash: str,
...
def notify_slack(
    slack_token: str, channel: str, attachments: List[Dict[str, Any]]
):
    ...
    try:
        ...
        if 'options' in locals() or 'options' in globals():
            ...
            message = [
                {
                    'fallback': title,
                    'title_link': os.path.join(
                        GITHUB_BASE_URL, options.commit
                    ),
                    'color': 'danger',
                    'title': title,
                    'fields': [
