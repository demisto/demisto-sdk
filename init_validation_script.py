...
class ValidationInitializer:
    def init(self):
        ...
        self.process_fix_result_related_fields()
    ...
    def get_error_description(self):
        ...
        self.error_description = str(
            input(
                "Please enter the error's description or press enter to leave black for now: "
            )
        )
    ...
    def get_validator_related_field(self):
        ...
        self.related_field = str(
            input(
                "Please enter the error's related_field or press enter to leave black for now: "
            )
        )
    ...
    def get_error_message(self):
        ...
        self.error_message = str(
            input(
                "Please enter the error's message or press enter to leave black for now: "
            )
        )
    ...
    def get_validator_class_name(self):
        ...
        validator_class_name = str(
            input(
                "Please enter the validator's class name in class format (i.e each word with a capital letter): "
            )
        )
        while not validator_class_name or " " in validator_class_name:
            validator_class_name = str(
                input(
    ...
    def get_git_statuses(self):
        ...
        if git_statuses_str:
            git_statuses_ls = git_statuses_str.split(",")
            ...
            self.git_statuses = f"expected_git_statuses = {str(git_statuses_ls)}"
        else:
            self.git_statuses = ""
...
Fill the content types as the numbers they appear as: """
    ...
    def get_fix_message(self):
        self.fix_message = str(
            input(
                "Please enter the fix message or press enter to leave black for now: "
            )
        )
    ...
    def process_content_types(self):
        if len(self.content_types) == 1:
            self.supported_content_types = f"ContentTypes = {CONTENT_TYPES_DICT.get(self.content_types, {}).get('content_type', '')}"
            self.imports = "from typing import Iterable, List\n\n"
        else:
            supported_content_types = [CONTENT_TYPES_DICT.get(content_type, {}).get('content_type', '') for content_type in self.content_types]
            self.supported_content_types = (
                f"ContentTypes = {','.join(supported_content_types)}"
            )
    ...
    def get_fix_info(self):
        ...
        if support_fix in ["Y", "y"]:
            ...
            self.get_fix_message()
            self.fix_result_import = "FixingResult,\n        "
        else:
            self.support_fix = False

    def get_graph_info(self):
        validate_graph = str(input("does the validation support graph? (Y/N): "))
    ...
    def get_file_name(self):
        file_name = str(input("Enter a file name in snake_case format: "))
